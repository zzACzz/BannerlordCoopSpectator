...
| `MissionScoreboardComponent` | потрібен тільки dedicated listed lobby shell для `BotData`, `PlayerPropertiesChanged(...)`, MVP/round-score readers і side-bot scoreboard bookkeeping; active `OnScoreHit(...)` path уже забраний нашим `ListedShellMissionScoreboardComponent`, host-side `kill/death/assist/score` header getters уже йдуть через `ListedShellScoreboardData` від `CoopBattlePeerStatsRuntimeState`, а reflected `MissionPeer` mirror уже звужений до `Score`-only compatibility tail | коли dedicated shell більше не буде залежати від цього native scoreboard container |
...
- host-side `kill/death/assist/score` scoreboard headers теж більше не читають тільки native `MissionPeer` поля: `ListedShellMissionScoreboardComponent` тепер використовує `ListedShellScoreboardData`, який бере ці значення з `CoopBattlePeerStatsRuntimeState` і лише fail-open падає назад у native peer fields;
- reflected `MissionPeer` stats mirror у listed scoreboard runtime вже звужений: `KillCount`, `AssistCount` і `DeathCount` більше не пишуться назад у native peer model, бо scoreboard headers і live K/D transport уже йдуть від `CoopBattlePeerStatsRuntimeState`; тимчасовим compatibility tail лишається тільки `MissionPeer.Score` для native MVP/round-end reader-ів;
