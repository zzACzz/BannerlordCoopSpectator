...
| --- | --- | --- |
| `MissionLobbyComponent` | native mission-state message surface; тепер інстанціюється вже не як vanilla `MissionCustomGame*Component`, а через наші concrete shell-класи `ListedShellMissionLobbyClientComponent` / `ListedShellMissionLobbyServerComponent`; server-side lobby state transitions, client bootstrap/state application, listed request-culture / request-change-character / class-restriction / `CreateBanner` surface, late-client K/D/bots replay, `ChangeCulture` apply і весь listed death bookkeeping уже перехоплені patch-layer-ом / coop runtime | коли coop runtime візьме на себе еквівалент mission-state message surface, death/respawn bookkeeping і peer bootstrap |
| `MultiplayerTimerComponent` | потрібен lobby lifecycle | коли буде свій lobby shell |
| `MissionScoreboardComponent` | потрібен тільки dedicated custom lobby shell; client-side native scoreboard вже прибраний зі stack-ів | коли dedicated shell більше не буде інстанціювати цей native component |
...
| `ListedShellCompatibilityModeClient` | generic listed-shell client mode: `MissionMode.Battle` + representative sync, без native gold/sound authority; тепер також тримає coop-owned `MissionStateChange` і `KillDeathCountChange` registration/apply замість native `MissionLobbyComponent` client handlers | коли listed ingress більше не залежатиме від official `TeamDeathmatch` mission shell |
| `MissionLobbySpawnContractPatch` | забирає з `MissionLobbyComponent` respawn-period, `WaitingFirstPlayers -> Playing`, `Playing -> Ending`, server-side `MissionStateChange` broadcast, client-side `MissionStateChange` application, `OnMyClientSynchronized` bootstrap, listed request-culture / request-change-character / class-restrictions / `CreateBanner` surface, native `HandleServerEventChangeCulture(...)`, late-client mission-state/K-D/bots replay і весь listed death/K-D/`BotData` path у `OnAgentRemoved(...)`; patch також тримає власний listed mission-state cache/resolver і після native registration phase викидає dead request/culture/class/banner handlers із `MissionLobbyComponent.AddRemoveMessageHandlers(...)`, тому наш runtime і сам patch-layer більше не читають `MissionLobbyComponent.CurrentMultiplayerState` як source of truth для listed flow, а native property лишається write-only compatibility output | коли listed ingress більше не потребуватиме навіть native `MissionLobbyComponent` shell |
...
- client-side `MissionLobbyComponent.HandleServerEventMissionStateChange(...)` і `OnMyClientSynchronized()` теж більше не є source of truth для listed ingress bootstrap; `MissionStateChange` registration/apply тепер уже живе в `ListedShellCompatibilityModeClient`, а state apply, client timer start, warmup removal і suppression native culture-selection request сидять у coop-owned path поверх `MissionLobbySpawnContractPatch`;
- native `MissionLobbyComponent.AfterStart()` теж більше не тримає live listed-shell bootstrap subscription на `MissionNetworkComponent.OnMyClientSynchronized`; patch-layer відписує цей dead handler одразу після старту місії, бо client bootstrap уже давно наш;
- native `MissionLobbyComponent.AddRemoveMessageHandlers(...)` для listed shell теж більше не реєструє мертвий request/culture/class/banner surface як live network handlers: `MissionLobbySpawnContractPatch` після native registration phase прибирає `RequestCultureChange`, `RequestChangeCharacterMessage`, `ChangeCulture`, `ChangeClassRestrictions`, `NetworkMessages.FromClient.CreateBanner` і `NetworkMessages.FromServer.CreateBanner` з registerer container, лишаючи в native lobby тільки реально потрібні message contracts;
- listed-shell `CreateBanner` apply/request теж більше не можуть оживити peer-driven banner mutation через native lobby container: patch-layer глушить `HandleClientEventCreateBannerForPeer(...)` і `HandleServerEventCreateBannerForPeer(...)`, а `ListedShellCompatibilityMode.AllowCustomPlayerBanners()` повертає `false`;
- native `MissionLobbyComponent.AddRemoveMessageHandlers(...)` для listed shell також більше не тримає `MissionStateChange` як live client apply handler: цей registration тепер вирізається з lobby container, а власний coop-owned handler реєструється в `ListedShellCompatibilityModeClient`;
- native `MissionLobbyComponent.AddRemoveMessageHandlers(...)` для listed shell також більше не тримає `KillDeathCountChange` як live client apply handler: apply player K/D/score payload-ів на клієнті тепер іде через `ListedShellCompatibilityModeClient`, а не через native lobby container;
...
- concrete lobby shell теж уже наш: `MissionBehaviorHelpers.TryCreateMissionLobbyComponent()` більше не повертає vanilla `MissionCustomGameClientComponent` / dedicated `MissionCustomGameServerComponent`, а інстанціює `ListedShellMissionLobbyClientComponent` / `ListedShellMissionLobbyServerComponent`, де `OnBehaviorInitialize`, `AfterStart` і `AddRemoveMessageHandlers` уже проходять через наш explicit shell, а не через Harmony postfix-и на базовому типі;
- `OnMissionTick` і `OnAgentRemoved` для listed lobby shell теж уже не перехоплюються Harmony prefix-ами на базовому `MissionLobbyComponent`: ці lifecycle вузли переведені в override-и наших `ListedShellMissionLobbyClientComponent` / `ListedShellMissionLobbyServerComponent`, які викликають coop-owned logic напряму й лише умовно падають назад у native base path;
...
- сам `MissionLobbySpawnContractPatch` для listed flow теж більше не читає `MissionLobbyComponent.CurrentMultiplayerState` як runtime input: state cache тепер ініціалізується на `OnBehaviorInitialize` і далі живе як coop-owned source of truth, а native property оновлюється тільки як compatibility output для контейнерного shell;
...
- native `MissionLobbyComponent.SendPeerInformationsToPeer(...)` більше не входить у listed late-join bootstrap contract; replay `KillDeathCountChange` / `BotsControlledChange` тепер іде через `MissionLobbySpawnContractPatch`, а не через native lobby shell;
...
- listed commander-owned bot death і `BotsControlledChange` теж більше не ставляться native `MissionLobbyComponent.OnAgentRemoved(...)`: controlling peer death counter, controlling-bot alive count, scoreboard refresh, `KillDeathCountChange` і `BotsControlledChange` тепер ідуть через `MissionLobbySpawnContractPatch`;
- pure side-bot `BotData` bookkeeping теж більше не ставиться native `MissionLobbyComponent.OnAgentRemoved(...)`: listed bot-vs-bot і bot-vs-player kills/deaths тепер оновлюють `MissionScoreboardComponent` side `BotScores`, `BotPropertiesChanged(...)` і `NetworkMessages.FromServer.BotData` напряму через `MissionLobbySpawnContractPatch`, без reflected `OnBotDies(...)` / `OnBotKills(...)` fallback у live path;
- native `HandleClientEventRequestCultureChange(...)`, `HandleClientEventRequestChangeCharacterMessage(...)`, `HandleServerEventChangeClassRestrictions(...)` і `HandleServerEventChangeCulture(...)` теж більше не входять у listed shell: `MissionLobbySpawnContractPatch` тепер глушить цей request/message surface повністю, без late-join `ChangeCulture` apply tail;
