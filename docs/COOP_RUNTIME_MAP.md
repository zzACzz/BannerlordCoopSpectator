...
- `ListedShellBaseNetworkTransportOwnershipPatch` тепер тримає весь listed `BaseNetworkComponent` interception surface в одному місці: send-side `HandleNewClientConnect(...)`, receive-side `InitializeCustomGame`, `LoadMission` і `UnloadMission` більше не розкидані по чотирьох окремих patch-класах, а маршрутизуються через один explicit ownership patch у `ListedShellNetworkBootstrapRuntime`;
