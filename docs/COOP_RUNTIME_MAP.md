...
| `MissionScoreboardComponent` | потрібен тільки dedicated listed lobby shell для `BotData`, `PlayerPropertiesChanged(...)`, MVP/round-score readers і side-bot scoreboard bookkeeping; active `OnScoreHit(...)`, host-side `kill/death/assist/score` headers і listed MVP selection уже йдуть через наш runtime state, а native `MissionPeer` stats більше не оновлюються як compatibility mirror | коли dedicated shell більше не буде залежати від цього native scoreboard container |
...
- listed-shell client apply для `UpdateRoundScores` і `BotData` теж більше не делегується в native `MissionScoreboardComponent` handlers: `ListedShellMissionScoreboardComponent` тепер сам пише `SideScore` і `BotScores`, а потім напряму тригерить `OnRoundPropertiesChanged` / `OnBotPropertiesChanged` без native apply path;
- native listed scoreboard `OnRoundEnding() -> UpdateRoundScores()` теж більше не є server-side score loop authority: `ListedShellMissionScoreboardComponent` після `AfterStart()` знімає native `OnRoundEnding` subscriber з `MultiplayerRoundComponent`, сам інкрементить `SideScore`, дописує `RoundWinnerList` і сам broadcast-ить `UpdateRoundScores`;
