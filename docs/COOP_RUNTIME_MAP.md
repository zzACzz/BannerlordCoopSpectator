...
- `MissionLobbyComponent` більше не повинен hard-read `SpawnComponent` ні для `GetSpawnPeriodDurationForPeer(...)`, ні для server-side `OnMissionTick(...)`; `WaitingFirstPlayers -> Playing`, `Playing -> Ending`, client-side `MissionStateChange` application, `OnMyClientSynchronized` bootstrap і respawn-period contract тепер перехоплює `MissionLobbySpawnContractPatch`.
...
| --- | --- | --- |
| `MissionLobbyComponent` | native mission-state message surface, late-client handling, death/respawn bookkeeping; server-side lobby state transitions і client bootstrap/state application уже перехоплені patch-layer-ом | коли coop runtime візьме на себе еквівалент mission-state message surface, death/respawn bookkeeping і peer bootstrap |
...
| `MissionLobbySpawnContractPatch` | забирає з `MissionLobbyComponent` respawn-period, `WaitingFirstPlayers -> Playing`, `Playing -> Ending`, client-side `MissionStateChange` application і `OnMyClientSynchronized` bootstrap; listed shell більше не потребує official `SpawnComponent` / `SpawningBehaviorBase` mission behavior і більше не покладається на native lobby tick/bootstrap як source of truth | коли listed ingress більше не потребуватиме навіть native `MissionLobbyComponent` shell |
