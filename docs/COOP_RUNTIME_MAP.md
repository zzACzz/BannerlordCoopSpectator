...
- `BaseNetworkComponent.HandleNewClientConnect(...)` для listed shell теж більше не покладається на native `BannerlordNetwork.LobbyMissionType == Custom/Community` як send-gate для `InitializeCustomGameMessage`; explicit listed mission marker тепер шле listed bootstrap повідомлення напряму, з official `TeamDeathmatch` id, поточною сценою і власним listed mission-session token, без `BaseNetworkComponentData.CurrentBattleIndex` fallback-а;
