...
- receive-side `BaseNetworkComponent.HandleServerEventLoadMission(...)` для listed shell теж більше не owner-ить mission-open authority; `ListedShellLoadMissionReceiveOwnershipPatch` перехоплює listed `LoadMission`, приймає mission-session token напряму і або запускає listed місію через coop-owned receive path, або глушить дубль, якщо listed shell уже активний;
- receive-side `BaseNetworkComponent.HandleServerEventUnloadMission(...)` для listed shell теж більше не owner-ить teardown authority; `ListedShellUnloadMissionReceiveOwnershipPatch` перехоплює listed `UnloadMission`, ставить `ServerEndingBeforeClientLoaded` уже в наш listed client shell, закриває multiplayer lobby mission і чекає unload без native unload coroutine;
- listed-shell `FinishedLoading` server validation теж більше не читає native `BaseNetworkComponentData.CurrentBattleIndex` навіть як fallback: `FinishedLoadingMissionReadyGatePatch` тепер owner-ить explicit listed handshake через `ListedShellMissionSessionState` і порівнює client `BattleIndex` тільки з нашим mission-session token;
- official `LobbyGameStateCustomGameClient.StartMultiplayer(...)`, `LobbyGameStateCommunityClient.StartMultiplayer(...)` і `LobbyGameStatePlayerBasedCustomServer.HandleServerStartMultiplayer()` для listed `TeamDeathmatch` тепер owner-яться напряму через `ListedShellLobbyGameStateOwnershipPatch`: explicit listed shell сам викликає `GameNetwork.StartMultiplayerOnClient(...)` або `GameNetwork.PreStartMultiplayerOnServer()` + `Module.CurrentModule.StartMultiplayerGame(...)` + `GameNetwork.StartMultiplayerOnServer(...)` без native `StartMultiplayerLobbyMission(Custom|Community)` arm;
- listed client wrapper-entry і receive ownership більше не висять на time-based `CustomGameJoinContextState`: `LobbyJoinResultSelfJoinArmPatch` тепер arm-ить явний `ListedShellClientStartOwnershipState`, `ListedShellLobbyGameStateOwnershipPatch` споживає його перед `GameNetwork.StartMultiplayerOnClient(...)`, а receive-side `InitializeCustomGame` / `LoadMission` / `UnloadMission` owner-яться вже через окремий `ListedShellTransportBootstrapState`;
...
- `ListedShellInitializeCustomGameOwnershipPatch` забирає send-side listed bootstrap з native `BaseNetworkComponent.HandleNewClientConnect(...)`: для explicit listed місії patch більше не пропускає native intermission/custom-lobby replay, а сам шле мінімальний listed bootstrap (`MultiplayerOptionsInitial`, `MultiplayerOptionsImmediate`, за потреби `MultiplayerOptionsDefault`, `InitializeCustomGameMessage`) від mission marker-а напряму, без official custom/community gate або `LobbyMissionType` rewrite;
...
- `ListedShellLoadMissionReceiveOwnershipPatch` забирає listed receive-side mission-open з native `BaseNetworkComponent.HandleServerEventLoadMission(...)`: listed `LoadMission` більше не write-ить `CurrentBattleIndex` і не веде в native mission-open coroutine, а приймає mission-session token через `ListedShellMissionSessionState`, ресетить compatibility intermission state і або owner-ить `StartMultiplayerGame(...)`, або глушить дубль, якщо listed місія вже відкрита;
- `ListedShellUnloadMissionReceiveOwnershipPatch` забирає listed receive-side teardown з native `BaseNetworkComponent.HandleServerEventUnloadMission(...)`: listed `UnloadMission` більше не веде в native custom/community client unload coroutine, а напряму виставляє `ServerEndingBeforeClientLoaded` у `ListedShellMissionLobbyClientComponent`, викликає `BannerlordNetwork.EndMultiplayerLobbyMission()`, скидає mute list і дочікується unload уже в coop-owned path;
- `ListedShellLobbyGameStateOwnershipPatch` забирає самі official custom/community/player-based wrappers: listed `TeamDeathmatch` client join більше не заходить у native `LobbyGameStateCustomGameClient.StartMultiplayer(...)` / `LobbyGameStateCommunityClient.StartMultiplayer(...)`, а listed host custom-server start більше не залежить від native `LobbyGameStatePlayerBasedCustomServer.HandleServerStartMultiplayer()`; explicit listed shell сам owner-ить `GameNetwork.StartMultiplayerOnClient(...)`, `GameNetwork.PreStartMultiplayerOnServer()`, `Module.CurrentModule.StartMultiplayerGame(...)` і `GameNetwork.StartMultiplayerOnServer(...)` для цього ingress path;
...
- `ListedShellClientStartOwnershipState` і `ListedShellTransportBootstrapState` розділяють listed client ingress на два явні етапи: join-result arm для wrapper-entry і transport arm для receive-side bootstrap; `CustomGameJoinContextState` після цього лишився тільки join-address/local-roster fallback state, а не listed startup ownership gate;
- direct listed session bring-up примітиви `GameNetwork.StartMultiplayerOnClient(...)`, `GameNetwork.PreStartMultiplayerOnServer()`, `GameNetwork.StartMultiplayerOnServer(...)`, `BannerlordNetwork.CreateServerPeer()` і listed `ClientFinishedLoading(...)` більше не розкидані по patch-класах; їх тепер централізує `ListedShellSessionTransportRuntime`, а wrapper/gate patch-і лишили собі тільки ownership-рішення;
