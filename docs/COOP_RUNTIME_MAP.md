...
| `MissionScoreboardComponent` | потрібен тільки dedicated listed lobby shell для `BotData`, `PlayerPropertiesChanged(...)`, MVP/round-score readers і side-bot scoreboard bookkeeping; active `OnScoreHit(...)` path уже забраний нашим `ListedShellMissionScoreboardComponent`, а host-side `kill/death/assist/score` header getters уже йдуть через `ListedShellScoreboardData` від `CoopBattlePeerStatsRuntimeState` | коли dedicated shell більше не буде залежати від цього native scoreboard container |
