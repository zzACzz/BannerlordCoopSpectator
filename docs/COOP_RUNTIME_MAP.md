...
- native `BannerlordNetwork.StartMultiplayerLobbyMission(...)` для custom/community client join більше не армує listed `TeamDeathmatch` у `LobbyMissionType=Custom/Community`; `ListedShellLobbyMissionTypeNeutralizationPatch` переписує цей arm у neutral/matchmaker state, якщо активний join-context already вказує на `GameType=TeamDeathmatch`;
...
- `ListedShellLobbyMissionTypeNeutralizationPatch` прибирає клієнтський custom/community arm official listed ingress: під час `LobbyGameStateCustomGameClient.StartMultiplayer(...)` / `LobbyGameStateCommunityClient.StartMultiplayer(...)` native `StartMultiplayerLobbyMission(Custom|Community)` уже не залишає listed `TeamDeathmatch` у global `LobbyMissionType`, якщо join-context каже, що це наш listed shell;
