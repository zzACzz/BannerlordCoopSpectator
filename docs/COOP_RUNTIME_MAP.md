...
- deferred non-listed `FinishedLoading` для `PendingBattleMissionStartupState` теж більше не робить live-читання native `BaseNetworkComponentData.CurrentBattleIndex` на кожній server-side валідації; explicit pending-battle startup state тепер один раз захоплює native transport token у момент резолюції місії, зв’язує його з поточною battle mission і далі порівнює client `BattleIndex` уже з цим own-ed mission-session state;
