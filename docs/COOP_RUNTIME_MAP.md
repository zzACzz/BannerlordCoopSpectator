...
| --- | --- | --- |
| `MissionLobbyComponent` | native mission-state message surface, death/respawn bookkeeping і culture/class request handlers; server-side lobby state transitions, client bootstrap/state application і late-client K/D/bots replay уже перехоплені patch-layer-ом | коли coop runtime візьме на себе еквівалент mission-state message surface, death/respawn bookkeeping і peer bootstrap |
...
| `MissionLobbySpawnContractPatch` | забирає з `MissionLobbyComponent` respawn-period, `WaitingFirstPlayers -> Playing`, `Playing -> Ending`, server-side `MissionStateChange` broadcast, client-side `MissionStateChange` application, `OnMyClientSynchronized` bootstrap і late-client `SendPeerInformationsToPeer(...)` replay; listed shell більше не потребує official `SpawnComponent` / `SpawningBehaviorBase` mission behavior і більше не покладається на native lobby tick/bootstrap як source of truth | коли listed ingress більше не потребуватиме навіть native `MissionLobbyComponent` shell |
