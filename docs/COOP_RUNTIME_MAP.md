...
- `MissionScoreboardComponent` повинен лишатись тільки на dedicated listed/custom server, бо listed lobby runtime ще веде через нього K/D, bot-death і side-bot scoreboard bookkeeping; client-side native scoreboard shell уже прибраний з `CoopBattle` і listed ingress stack-ів.
...
| `MissionScoreboardComponent` | потрібен тільки dedicated listed lobby shell для K/D, bot-death і side-bot scoreboard bookkeeping; client-side native scoreboard вже прибраний зі stack-ів | коли dedicated shell більше не буде залежати від цього native scoreboard container |
