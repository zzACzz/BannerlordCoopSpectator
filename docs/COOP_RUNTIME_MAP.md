...
- `BaseNetworkComponent.HandleNewClientConnect(...)` для listed shell теж більше не покладається на native `BannerlordNetwork.LobbyMissionType == Custom/Community` як send-gate для `InitializeCustomGameMessage`; explicit listed mission marker тепер шле listed bootstrap повідомлення напряму, з official `TeamDeathmatch` id, поточною сценою і власним listed mission-session token, а `BaseNetworkComponentData.CurrentBattleIndex` лишився тільки compatibility mirror для vanilla transport;
...
- listed-shell `FinishedLoading` server validation теж більше не читає native `BaseNetworkComponentData.CurrentBattleIndex` як source of truth: `FinishedLoadingMissionReadyGatePatch` тепер owner-ить explicit listed handshake через `ListedShellMissionSessionState` і порівнює client `BattleIndex` з нашим mission-session token, а не з native battle-index cache;
...
- `ListedShellInitializeCustomGameOwnershipPatch` забирає send-side `InitializeCustomGameMessage` з native `BaseNetworkComponent.HandleNewClientConnect(...)`: для explicit listed місії patch шле listed bootstrap від mission marker-а напряму, а не через official custom/community gate або `LobbyMissionType` rewrite;
