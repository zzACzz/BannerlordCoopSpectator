...
- direct listed session bring-up примітиви `GameNetwork.StartMultiplayerOnClient(...)`, `GameNetwork.PreStartMultiplayerOnServer()`, `GameNetwork.StartMultiplayerOnServer(...)`, `BannerlordNetwork.CreateServerPeer()`, listed `ClientFinishedLoading(...)` і server-side listed `FinishedLoading` validation більше не розкидані по patch-класах; їх тепер централізує `ListedShellSessionTransportRuntime`, а wrapper/gate patch-і лишили собі тільки ownership-рішення;
