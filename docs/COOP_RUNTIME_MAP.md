...
| --- | --- | --- |
| `MissionLobbyComponent` | native mission-state message surface, K/D/score bookkeeping, bot-death bookkeeping і culture/class request handlers; server-side lobby state transitions, client bootstrap/state application, late-client K/D/bots replay і immediate listed player-death transition уже перехоплені patch-layer-ом / coop runtime | коли coop runtime візьме на себе еквівалент mission-state message surface, death/respawn bookkeeping і peer bootstrap |
...
| `MissionLobbySpawnContractPatch` | забирає з `MissionLobbyComponent` respawn-period, `WaitingFirstPlayers -> Playing`, `Playing -> Ending`, server-side `MissionStateChange` broadcast, client-side `MissionStateChange` application, `OnMyClientSynchronized` bootstrap, late-client `SendPeerInformationsToPeer(...)` replay і listed player-death handoff після `OnAgentRemoved(...)`; listed shell більше не потребує official `SpawnComponent` / `SpawningBehaviorBase` mission behavior і більше не покладається на native lobby tick/bootstrap як source of truth | коли listed ingress більше не потребуватиме навіть native `MissionLobbyComponent` shell |
...
- listed-shell player death теж більше не чекає polling-only `lost-controlled-agent` path: `MissionLobbySpawnContractPatch` тепер перехоплює `MissionLobbyComponent.OnAgentRemoved(...)` і одразу переводить dead peer у coop-owned spectator/`DeadAwaitingRespawn` transition з authoritative respawn-timer normalization;
...
- native `MissionLobbyComponent.SendPeerInformationsToPeer(...)` більше не входить у listed late-join bootstrap contract; replay `KillDeathCountChange` / `BotsControlledChange` тепер іде через `MissionLobbySpawnContractPatch`, при цьому джерело kill/assist/score ще лишається на поточному death-bookkeeping шарі до окремого виносу `OnAgentRemoved(...)`;
- native `MissionLobbyComponent.OnAgentRemoved(...)` більше не є єдиною точкою listed player-death transition: immediate peer move у spectator holding, clear active control/runtime requests, lifecycle `DeadAwaitingRespawn` і respawn timer тепер уже ставляться нашим coop runtime, хоча самі native `KillDeathCountChange`, `Score` і bot-death апдейти ще не винесені повністю;
