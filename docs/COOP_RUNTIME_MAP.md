...
| --- | --- | --- |
| `MissionLobbyComponent` | native mission-state message surface, pure side-bot bookkeeping і culture/class request handlers; server-side lobby state transitions, client bootstrap/state application, late-client K/D/bots replay, listed player-death transition, listed player K/D/score path і listed commander-owned bot death / `BotsControlledChange` path уже перехоплені patch-layer-ом / coop runtime | коли coop runtime візьме на себе еквівалент mission-state message surface, death/respawn bookkeeping і peer bootstrap |
...
| `MissionLobbySpawnContractPatch` | забирає з `MissionLobbyComponent` respawn-period, `WaitingFirstPlayers -> Playing`, `Playing -> Ending`, server-side `MissionStateChange` broadcast, client-side `MissionStateChange` application, `OnMyClientSynchronized` bootstrap, late-client `SendPeerInformationsToPeer(...)` replay і повний listed player-death/K-D path разом із listed commander-owned bot death / `BotsControlledChange` path у `OnAgentRemoved(...)`; listed shell більше не потребує official `SpawnComponent` / `SpawningBehaviorBase` mission behavior і більше не покладається на native lobby tick/bootstrap як source of truth | коли listed ingress більше не потребуватиме навіть native `MissionLobbyComponent` shell |
...
- listed commander-owned bot death і `BotsControlledChange` теж більше не ставляться native `MissionLobbyComponent.OnAgentRemoved(...)`: controlling peer death counter, controlling-bot alive count, scoreboard refresh, `KillDeathCountChange` і `BotsControlledChange` тепер ідуть через `MissionLobbySpawnContractPatch`, а killer-side listed bot path теж уже розрізається між coop-owned peer scoring та native pure side-bot fallback;
