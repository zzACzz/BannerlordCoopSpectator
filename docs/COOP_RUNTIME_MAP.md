...
| `MissionLobbySpawnContractPatch` | забирає з `MissionLobbyComponent` respawn-period, `WaitingFirstPlayers -> Playing`, `Playing -> Ending`, server-side `MissionStateChange` broadcast, client-side `MissionStateChange` application, `OnMyClientSynchronized` bootstrap, listed request-culture / request-change-character / class-restrictions surface, native `HandleServerEventChangeCulture(...)`, late-client `SendPeerInformationsToPeer(...)` replay і весь listed death/K-D/`BotData` path у `OnAgentRemoved(...)`; patch також тримає власний listed mission-state cache/resolver і після native registration phase викидає dead request/culture/class handlers із `MissionLobbyComponent.AddRemoveMessageHandlers(...)`, тому наш runtime і сам patch-layer більше не читають `MissionLobbyComponent.CurrentMultiplayerState` як source of truth для listed flow, а native property лишається write-only compatibility output | коли listed ingress більше не потребуватиме навіть native `MissionLobbyComponent` shell |
...
- native `MissionLobbyComponent.AddRemoveMessageHandlers(...)` для listed shell теж більше не реєструє мертвий request/culture/class-restriction surface як live network handlers: `MissionLobbySpawnContractPatch` після native registration phase прибирає `RequestCultureChange`, `RequestChangeCharacterMessage`, `ChangeCulture` і `ChangeClassRestrictions` з registerer container, лишаючи в native lobby тільки реально потрібні message contracts;
