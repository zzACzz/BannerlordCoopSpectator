...
| Connectivity і local/self join | `Patches/ListedShellClientWrapperOwnershipPatch.cs`, `Patches/LocalJoinAddressPatch.cs`, `Patches/LobbyRequestJoinDiagnosticsPatch.cs` |
...
- `ListedShellClientWrapperOwnershipPatch` забирає весь listed wrapper-entry chain: Diamond join-result arm, official custom/community client `StartMultiplayer(...)` і player-based hosted `HandleServerStartMultiplayer()` більше не розкидані по двох patch-класах; explicit listed shell сам owner-ить wrapper arm, `GameNetwork.StartMultiplayerOnClient(...)`, `GameNetwork.PreStartMultiplayerOnServer()`, `Module.CurrentModule.StartMultiplayerGame(...)` і `GameNetwork.StartMultiplayerOnServer(...)` для цього ingress path;
