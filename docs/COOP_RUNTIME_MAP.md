...
- listed client receive ownership більше не висить на time-based `CustomGameJoinContextState`: `ListedShellLobbyGameStateOwnershipPatch` тепер arm-ить явний `ListedShellTransportBootstrapState` перед `GameNetwork.StartMultiplayerOnClient(...)`, а receive-side `InitializeCustomGame` / `LoadMission` / `UnloadMission` перевіряють already-owned transport bootstrap state, а не lobby/UI join-context TTL;
...
- `ListedShellTransportBootstrapState` забирає listed receive-bootstrap ownership з `CustomGameJoinContextState.ShouldOwnListedShellCustomGameBootstrap()`: explicit listed client path тепер arm-ить окремий transport bootstrap state перед `GameNetwork.StartMultiplayerOnClient(...)`, а receive-side `InitializeCustomGame`, `LoadMission` і `UnloadMission` owner-яться через нього та disarm-яться вже на coop-owned unload path;
