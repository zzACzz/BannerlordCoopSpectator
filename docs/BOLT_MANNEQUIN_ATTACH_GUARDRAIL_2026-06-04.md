...
Do **not** patch these methods again as a first-line fix for bolt crashes in shared/client runtime:
...
If low-level attach suppression is still required, keep it **dedicated-only** so client mannequin / preview pipelines remain untouched.

Do not reintroduce global client/shared `Agent.AttachWeaponToBone/Weapon` suppression unless a future investigation proves:
