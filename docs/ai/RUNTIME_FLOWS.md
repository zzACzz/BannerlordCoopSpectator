...
The launcher and client module do not issue `start_game`. Only the external runner may issue the minimum standard server bootstrap, and only after `Suppress`, dedicated loaded-hash validation, and the native `is ready! You can now enter console commands` acknowledgement. Milestone 2B.2C implements bounded asynchronous stdout/stderr capture, exact option/start-game/scene readbacks, singular structured command results, and exact PID-correlated native-log retention. Contract tests prove those runner mechanics in Windows PowerShell 5.1 and PowerShell 7; the next clean live rerun must still prove command acceptance, UDP visibility, client handoff, and connection in Bannerlord. The bootstrap opens no campaign fixture, advances no cooperative battle phase, consumes no result, and creates no L2/L3 evidence claim.
