...
The launcher and client module do not issue `start_game`. Only the external runner may issue the minimum standard server bootstrap, and only after `Suppress`, dedicated loaded-hash validation, and the native `is ready! You can now enter console commands` acknowledgement. Milestone 2B.2C implements bounded asynchronous stdout/stderr capture, exact option/start-game/scene readbacks, singular structured command results, and exact PID-correlated native-log retention. Before those steps, successful process creation must establish provisional cleanup ownership; bounded identity enrichment must resolve and validate the exact requested executable even when an immediate `Process.Path` read is null. Clean live run `m2b2c-live-feasibility-20260831-01` exposed this missing transition and therefore did not prove command acceptance, UDP visibility, client handoff, or connection. The bootstrap opens no campaign fixture, advances no cooperative battle phase, consumes no result, and creates no L2/L3 evidence claim.
