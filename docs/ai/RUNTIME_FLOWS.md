...
Current correction boundary (2026-09-09): the field-only opening guard now resolves `TaleWorlds.MountAndBlade.ListedServer.IIntermissionState` and preserves `IsNewTaskAssignable` plus one-shot mission ownership. Shared runner failure handling classifies exact crash reporters separately from cleanup-owned `Watchdog.exe`, uses a one-second support-process grace while retaining 15 seconds for primary roles, explicitly initializes the Windows PowerShell 5.1 hash command, and emits failure-only phase markers around evidence and cleanup. Full 24/24 contracts and both non-deploying builds passed; no native rerun occurred, so mission progression and automatic live-failure cleanup remain unverified. [Correction evidence and next boundary](BATTLE_TEST_AUTOMATION_M4_FIELD_SPAWN_SMOKE.md#13-bounded-failure-diagnostic-and-sourcecontract-correction-2026-09-09).

Clean rerun boundary (2026-09-09): public pair runner → child 1 → exact corrected dedicated process → schema-2 `WaitingForDedicatedReady` → repeated parent-liveness admission → `Cancelled` → automatic exact dedicated cleanup/native-log capture → parent cancellation → full outer restoration. Initial parent admission and all immutable run/nonce/PID bindings succeeded, but the later combined check did not record whether its hard 10-second heartbeat age or a shared-read fact failed. `ModuleReady`, control readiness, bootstrap, UDP ownership, field readiness, mission and child 2 were not reached. [Exact artifacts and safety postflight](BATTLE_TEST_AUTOMATION_M4_FIELD_SPAWN_SMOKE.md#14-clean-m4-l2-rerun-parentliveness-cancellation-2026-09-09).

Corrected parent-liveness flow (2026-09-09, source/L1 only): parent renews lease → each child-output stream receives at most a 100 ms processing slice → child reads manifest/lease/intent → pure admission result records every fact and heartbeat age → exact parent Runner identity is revalidated → accept; a transient unreadable snapshot enters a second pure decision and may reuse only the last exact acceptance inside the same ten-second window while the cached process remains live. Any immutable mismatch, stale/invalid heartbeat, non-Active lease, expired read grace or process loss → one failure-only rejection artifact containing the evaluated decision → `Cancelled`. The focused dual-shell burst/fallback harness, 24/24 contracts and both builds passed; no native process exercised this flow yet. [Implementation and evidence boundary](BATTLE_TEST_AUTOMATION_M4_FIELD_SPAWN_SMOKE.md#15-parentliveness-source-and-contract-correction-2026-09-09).
