...
Latest scoreboard dependency flow (2026-09-09, source/L1): direct CoopBattle or full TdmClone server behavior list → detect first `MissionScoreboardComponent` → detect any existing `MissionMultiplayerGameModeBaseClient` → pure contract requests insertion only when the server dependency is missing → insert one minimal bridge immediately before the scoreboard → assert invariant → allow native `AfterStart`, `OnClearScene`, and `OnRemoveBehavior` without suppression. Scoreboard-free minimal TdmClone does nothing. Reusable hideout construction defers generic insertion so its day/night wrappers retain the existing `MissionMultiplayerCoopBattleClient`; siege and other safe paths are unchanged. Published `a6fe74f`, focused tests, 24/24 and both CompileOnly builds passed with installed inventories unchanged. No native process exercised the corrected flow yet. [Source/L1 closure](BATTLE_TEST_AUTOMATION_M4_FIELD_SPAWN_SMOKE.md#19-scoreboard-server-dependency-source-and-contract-closure-2026-09-09).

Latest resolved failure flow (2026-09-09, recovered dump): CoopBattle battle-map dedicated stack → `MissionScoreboardComponent` present, no `MissionMultiplayerGameModeBaseClient` → `Mission.AfterStart` → scoreboard resolves `_mpGameModeBase=null` → server/recorder branch dereferences `_mpGameModeBase.RoundComponent` → managed `System.NullReferenceException` on OS thread `0x20f0` → outer WER APPCRASH `0xc0000005` → full dump written to `C:\dumps`. Field/land/village/sally-out append to the same server stack; full TdmClone server has the latent same dependency. Hideout wrappers and siege/other listed modes already provide or avoid the dependency. No unchanged rerun occurred after recovery of the existing dump. [Exact managed stack, object fields and cross-mode audit](BATTLE_TEST_AUTOMATION_M4_FIELD_SPAWN_SMOKE.md#18-recovered-full-dump-and-exact-managed-failure-diagnosis-2026-09-09).

Latest failure-finalization flow (2026-09-09, source/L1): primary Crash/Timeout retained → start one exact same-host PowerShell collector → provisionally register PID/path/parent/launch window → isolated limited-property CIM query → poll 50 ms under time/private-memory bounds → exact forced collector stop on limit/failure → validate schema/output/record count → reuse lightweight records for descendant discovery and fatal-helper correlation → publish crash/hang metadata without raw records → `FailureEvidenceCaptureCompleted` → exact runtime cleanup. A collector failure remains structured and cannot erase the primary failure artifact; disk publication failure remains a distinct runner error. Feasibility, DedicatedSpawnSmoke and Record cleanup share the bounded snapshot path; no battle adapter changed. Dual-shell focused contracts, 24/24 and both CompileOnly builds passed. No native process exercised this corrected flow yet. [Source/contract closure](BATTLE_TEST_AUTOMATION_M4_FIELD_SPAWN_SMOKE.md#17-bounded-failure-evidence-source-and-contract-closure-2026-09-09).

Latest corrected live flow (2026-09-09): clean pair runner → child 1 → exact staged dedicated process → corrected repeated parent admission → control readiness → seven bootstrap acknowledgements → confirmed `start_game` → one `start_mission` → `MissionOpening` → native APPCRASH `0xc0000005` → `FailureEvidenceCaptureStarted` → failure-evidence stall/high memory → stale-lease cancellation refused uncertain ownership → exact identity-checked manual Runner stop → parent exit 40 → full outer restoration. No parent-rejection artifact was produced, so the prior liveness blocker is closed. No dump, fault module, terminal child failure artifact, second attempt, materialization, early abort or L2 pass exists. [Exact evidence and next correction order](BATTLE_TEST_AUTOMATION_M4_FIELD_SPAWN_SMOKE.md#16-clean-published-liveness-corrected-rerun-native-mission-opening-crash-and-failure-evidence-stall-2026-09-09).

Observed field flow (2026-09-09): exact staged role loaded -> seven bootstrap acknowledgements -> WaitingForMissionCommandReady -> request expiry. No mission, agent observation, early abort or real result attempt occurred. The opening guard's IIntermissionState namespace disagrees with the exact native assembly. Separately, child failure handling stalled before normal cleanup; manual exact cleanup and automatic installed restoration completed. The second fresh process was Not Run. [Confirmed source boundary and evidence limits](BATTLE_TEST_AUTOMATION_M4_FIELD_SPAWN_SMOKE.md#12-first-clean-local-only-live-attempt-2026-09-09).

Prior correction boundary (2026-09-09): the field-only opening guard now resolves `TaleWorlds.MountAndBlade.ListedServer.IIntermissionState` and preserves `IsNewTaskAssignable` plus one-shot mission ownership. Shared runner failure handling classifies exact crash reporters separately from cleanup-owned `Watchdog.exe`, uses a one-second support-process grace while retaining 15 seconds for primary roles, explicitly initializes the Windows PowerShell 5.1 hash command, and emits failure-only phase markers around evidence and cleanup. Full 24/24 contracts and both non-deploying builds passed; no native rerun occurred during that source-only stage, so mission progression and automatic live-failure cleanup remained unverified. [Correction evidence and next boundary](BATTLE_TEST_AUTOMATION_M4_FIELD_SPAWN_SMOKE.md#13-bounded-failure-diagnostic-and-sourcecontract-correction-2026-09-09).

Previous clean rerun boundary (2026-09-09): public pair runner → child 1 → exact corrected dedicated process → schema-2 `WaitingForDedicatedReady` → repeated parent-liveness admission → `Cancelled` → automatic exact dedicated cleanup/native-log capture → parent cancellation → full outer restoration. Initial parent admission and all immutable run/nonce/PID bindings succeeded, but the later combined check did not record whether its hard 10-second heartbeat age or a shared-read fact failed. `ModuleReady`, control readiness, bootstrap, UDP ownership, field readiness, mission and child 2 were not reached. [Exact artifacts and safety postflight](BATTLE_TEST_AUTOMATION_M4_FIELD_SPAWN_SMOKE.md#14-clean-m4-l2-rerun-parentliveness-cancellation-2026-09-09).

Corrected parent-liveness flow (2026-09-09): parent renews lease → each child-output stream receives at most a 100 ms processing slice → child reads manifest/lease/intent → pure admission result records every fact and heartbeat age → exact parent Runner identity is revalidated → accept; a transient unreadable snapshot enters a second pure decision and may reuse only the last exact acceptance inside the same ten-second window while the cached process remains live. Any immutable mismatch, stale/invalid heartbeat, non-Active lease, expired read grace or process loss → one failure-only rejection artifact containing the evaluated decision → `Cancelled`. The focused dual-shell burst/fallback harness, 24/24 contracts and both builds passed at source/L1; section 16's later native process remained admitted through `MissionOpening` with no rejection artifact. [Implementation and evidence boundary](BATTLE_TEST_AUTOMATION_M4_FIELD_SPAWN_SMOKE.md#15-parentliveness-source-and-contract-correction-2026-09-09).

Local field-file flow (2026-09-09): explicit profile + validated configuration → local state/bridge path → runner CreateNew sentinel → server exact-byte admission → ordinary immutable roster/materialization/abort → suppressed result → local preservation evidence. State/selection/spawn and auxiliary bridges use the same path authority; phase keeps state/phase. Personal Documents are not resolved in this profile. Ordinary and non-smoke automation paths retain their prior behavior. Source/contracts and both builds passed; section 12 now records a failed live attempt before mission, with no L2 pass. [Full ownership and evidence boundary](BATTLE_TEST_AUTOMATION_M4_FIELD_SPAWN_SMOKE.md#11-local-only-file-isolation-2026-09-09).
...
Last automation-control source verification: **2026-09-09**
...
Milestone 3A passed synthetic exact-byte, corruption/schema/path, battle-type admission, 23-project contract, and both-project compile-only checks. Milestone 3B supplies and live-proves the aggregate-owned campaign process/environment/reproduction path. Clean run `m3b-live-capture-02` captured the real private fixture from a purpose-made campaign sample and verified exact byte/hash/provenance/qualification, protected-result isolation, and cleanup. Milestone 3C independently reviewed that payload, retained it outside Git, created the deterministic linked derivative and separate oracle, and passed hash-gated contract replay. See [BATTLE_TEST_AUTOMATION_M3_FIELD_FIXTURE.md](BATTLE_TEST_AUTOMATION_M3_FIELD_FIXTURE.md).
...
The launcher does not modify `LauncherData.xml`, select a save, automate UI, start a multiplayer client, issue `start_game`, or require mission/full-battle completion. Bannerlord can still mutate normal user save/configuration state after manual interaction, so live capture must use a purpose-made save copy. `m3b-dirty-block-01` proves dirty-source rejection without product launch; `m3b-contracts-01` passed all 23 contracts; `m3b-compile-01` passed both builds with installed inventories unchanged. Controlled transaction `m3b-stage-01` retained the complete installed client pre-image and changed only its DLL/PDB. The first live attempt exposed stale runner literals after the recorder had already produced valid private bytes; published `baf5c69` corrects and cross-contract guards them. Corrected `m3b-fix-pub-contracts-01` passed 23/23, `m3b-fix-pub-compile-01` passed both builds without installed mutation, and `m3b-live-capture-02` returned `Pass` with one exact private field fixture, unchanged protected result, and zero remaining product processes or listeners. That private capture remains outside Git and is linked by hash to the reviewed derivative.

### Milestone 3C offline sanitization and contract replay

```text
private m3b-live-capture-02 field payload
  -> require exact reviewed SHA-256 and source metadata
  -> independently audit counts, references, composition, equipment, mounts, and privacy
  -> New-CoopSanitizedFieldFixture.ps1
       refuse source overwrite, nonempty output, and implicit repository output
       replace campaign-derived identities with deterministic fixture identities
       retain static game IDs and replay-relevant combat state
       emit only sanitized payload + linked sanitized metadata
  -> compare Windows PowerShell 5.1 and PowerShell 7 bytes
  -> add separately authored fixture.oracle.json
  -> contract replay:
       require exact three-file allowlist and both provenance hashes
       reject path/account/credential patterns
       deserialize through BattleSnapshotMessage
       validate independent counts/references/equipment/mount facts
       admit ordinary SCN-001 and reject a non-field mutation
  -> no Bannerlord process, mission, client, or result
```

The committed derivative is 259,744 bytes with SHA-256 `B47D7AF7FA057C36CA8EF759A6D597C00007158A22E3A556AC57A1299579D49D`. Final canonical `m3c-sanitized-contracts-02` passed 23/23 after the committed encoding/oracle assertions were complete, and `m3c-sanitized-compile-01` built the client and dedicated projects below the run root with installed inventories unchanged. This closes Milestone 3 only at the offline exact-fixture/contract boundary. Dedicated mission startup and `PreBattleHold` are Milestone 4.
...
#### Field automation native mode initialization (2026-09-10, source contract)

`BuildServerMissionBehaviorsForCoopBattle` inserts one native `BattleMissionStarterLogic` immediately after `MissionMultiplayerCoopBattle` only when `CoopAutomationSpawnSmokeBridge.ClaimNativeModeInitialization` admits the factory invocation. Admission requires the active immutable `FieldDedicatedSpawnSmokeV1` fixture, unchanged exact profile, the observed `MultiplayerBattle` opening, matching `battle_terrain_029`, a non-null mission, both the dedicated-process classification and `GameNetwork.IsDedicatedServer`, and no earlier initialization claim. Requested but invalid or repeated claims throw; rejected claims on an active binding retain failure. Disabled automation or an absent profile adds nothing.

The native component owns `SetMissionMode(MissionMode.Battle, atStart: true)` during its `AfterStart`. This is engine-mode initialization, not a transition to cooperative `CoopBattlePhase.BattleActive`. `ObserveInitialized` subsequently accepts only the same claimed mission instance without prior failure; `Reset` clears the claim. The minimal scoreboard dependency bridge retains its separate responsibility.

This factory-only correction does not alter simulation delta, bot options, synchronized-peer admission, native spawning, reinforcement queues, reconnect, or result suppression. Ordinary field battles and other scenario/role paths receive no new initializer. Mode callbacks and zero-client progression are not runtime verified. See [mode-only implementation and evidence boundary](BATTLE_TEST_AUTOMATION_M4_FIELD_SPAWN_SMOKE.md#24-field-automation-native-mode-initialization-2026-09-10); the separately approved clock/peer admission follows below.

#### Field automation zero-client pre-battle admission (2026-09-10, source contract)

`CoopAutomationSpawnSmokeBridge.CanUseZeroClientPreBattle` is the shared decision for the two participant checks. It requires the admitted exact profile/fixture, the opened and initialized same mission and native-mode claim, no failure/end observation, dedicated role, matching field scene, native `Continuing` plus `Battle`, zero connected clients, matching snapshot, and one of `Loading`, `SideSelection`, `UnitSelection`, `Deployment`, or `PreBattleHold`. Unknown and active/ended phases are denied. `CoopAutomationZeroClientRuntime.CanAdvancePreBattle` reads those facts only behind profile/mission admission; it additionally requires `Mission.Current` identity, an active session, no native mission end, exact campaign/battle/instance/scene and field-scenario identity, and two populated snapshot sides. An unreadable runtime fact records failure; an unknown peer list is not accepted as zero clients. Connected clients include unsynchronized connections.

After fixture admission, before native bootstrap commands, `CoopAutomationDedicatedControlBridge.TryAcceptRequest` installs `CoopAutomationZeroClientClockPatch` with a separate Harmony owner. Dedicated identity is required at installation; an active server session is required only when applying the runtime exception. The transpiler requires one native participant-predicate call followed by the observed conditional zero-delta assignment and rejects an absent, duplicated or changed corridor. It retains the native call and original instructions/labels, inserting the shared admission check only at that call site in `MissionState.TickMission`. A true native result remains true. The existing pause, fixed-delta, time-speed and tick logic remain intact; no bot option, participant count or global participant predicate is changed.

`CoopMissionSpawnLogic.ShouldDeferBattleMapStartupRuntime` consults the same admission only at its exact-scene/no-synchronized-peer branch, after the unchanged mode and 1.5-second gates, and still requires `IsBattleSnapshotReadyForMaterialization`. It does not bypass snapshot readiness or create agents. The existing deterministic bootstrap side and zero-eligible-peer snapshot acknowledgement already support the admitted field path. Native army bootstrap, phase promotion from actual army readiness, reinforcements, player control and result suppression retain their owners.
...
Primary campaign, dedicated-server, and multiplayer-client roles retain the 15-second graceful-close budget. `RuntimeSupport` and `RuntimeFailureSupport` roles use one second before exact-identity forced termination because they have no dependable graceful-shutdown contract. This distinction changes timing only; PID, path, start-time, ancestry, and hash checks remain authoritative.
