...
Published resolver run `m2d-live-r3-01` loaded corrected client SHA-256 `1C500501CE25D4A520782F61B338F9D8D0A4C591A4748E80991A521B63379250`, resolved the exact `NetworkMain.GameClient`, and reached `WaitingForLobby` with native `LobbyState=Idle`. The exact client displayed the stock `Not Logged In` state and never reached `AtLobby`. Installed-runtime decompilation shows that stock `MPAuthenticationVM.ExecuteLogin()` delegates to public `LobbyState.TryLogin()`, which owns platform privilege checks, login-provider creation, and lobby connection. Revision 13 implements a narrow authentication adapter before server discovery: complete default-off profile only, main application tick only, exact active `LobbyState`, reference equality with the resolved game client, `Idle`, `IsLoggingIn=false`, one retained task, and explicit outcome publication. It does not click UI, store credentials, bypass platform authentication, or retry blindly. Because this gate precedes server selection, it is shared by every battle type. Source/contracts and compile-only output are verified; the new binary is not staged or runtime-verified.

The external aggregate remains responsible for evidence and cleanup. The runner now preserves terminal client status through thrown wait failures and a validated cleanup fallback, and inventories exact client-PID native logs separately from dedicated logs before cleanup completes. Neither evidence correction changes native login authority or campaign/battle authority.
