...
`scripts/Invoke-CoopTest.ps1` is the only current general runner entry point. For non-runtime `Doctor`, `Contracts`, and `CompileOnly`, source-prepared runtime `Feasibility`, aggregate-owned campaign `Record`, `Inspect`, and `Recover`, and exact-run `Cancel`, it owns:
...
Milestone 3B adds `scripts/Start-CoopFieldFixtureCapture.ps1` and the aggregate `Record` command. Exact installed-1.4.8 launcher inspection establishes the fixed direct campaign shape rather than reusing the multiplayer wrapper. The aggregate requires clean local/upstream source, explicit installed client identity, free runtime resources, `Suppress`, a fresh run root, and shared locks; the child validates that contract again before inheriting the nonce only through its environment. Provisional campaign ownership precedes fallible process enrichment. The aggregate accepts only exact run/fixture/hash/revision/game-version status and metadata, treats non-field `Skipped` as retryable within the same campaign process, stops after an accepted payload, discovers only exact descendants, performs identity-bound cleanup, and retains PID-correlated logs plus protected-result evidence. The first diagnostic capture exposed two stale independent-runner literals; correction `baf5c69` aligns them with the canonical C# constants and guards that parity. Clean run `m3b-live-capture-02` then recorded and independently hash/length/provenance/qualification-validated one private exact ordinary-field payload, preserved the protected result, and left no product process or port listener. This layer does not edit launcher configuration or saves, start a multiplayer client, require a full battle, sanitize the payload, provide an independent content oracle, or claim L2/L3. See [BATTLE_TEST_AUTOMATION_M3_FIELD_FIXTURE.md](BATTLE_TEST_AUTOMATION_M3_FIELD_FIXTURE.md).

Milestone 3C keeps the exact raw capture and its broader run root private, then creates a distinct shareable authority rather than relabeling the raw bytes. `scripts/New-CoopSanitizedFieldFixture.ps1` is pinned to the independently reviewed private SHA-256, refuses source overwrite/nonempty output/implicit repository output, replaces campaign-derived identities deterministically, preserves static game replay content, and emits only a sanitized payload plus linked metadata. A separately authored oracle owns expected counts, composition, references, equipment, and evidence limits. `CoopAutomationFixture.ContractTests` verifies the three-file repository allowlist, privacy patterns, both provenance hashes, production-schema deserialization, first-field admission, independent values, and non-field mutation rejection. The sanitizer is byte-stable across Windows PowerShell 5.1 and PowerShell 7. This is a contract-replay authority only; runtime mission ownership remains with Milestone 4.
...
| Aggregate-owned private field-fixture capture | `scripts/Invoke-CoopTest.ps1 -Command Record`, `scripts/Start-CoopFieldFixtureCapture.ps1` |
| Reviewed shareable field-fixture derivative | `scripts/New-CoopSanitizedFieldFixture.ps1`, `Tests/Fixtures/Automation/field-current` |
| Independent committed field-fixture oracle | `Tests/Fixtures/Automation/field-current/fixture.oracle.json`, enforced by `CoopAutomationFixture.ContractTests` |
