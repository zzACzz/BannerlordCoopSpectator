...
Clean corrected-selection run `m2e1-live-r1-01` then selected the exact run-owned server, completed the native join task, executed `GameNetwork.StartMultiplayerOnClient(...)` with the intended one-shot loopback rewrite, entered `InCustomGame`, logged `Join game successful`, received server `CreatePlayer`, and visibly displayed `Awaiting Server`. Formal state remained non-terminal `JoinAccepted`, so the runner returned `Timeout`. Installed-runtime IL inspection proved that the only handoff notification was attached to an absent historical four-argument lobby method; installed `1.4.8` uses a parameterless lobby override that calls the already-patched GameNetwork method. Correction `d1af692` makes that actual lowest-level patch the sole notifier after address rewrite. Correction `3fdcda3` retains the last validated non-terminal status in timeout reports without changing the outcome. Full 22/22 contracts passed. Clean published compile `m2e1-handoff-pub-compile-20260901-01` from `a7bd528` produced client SHA-256 `7CC2D759806D2F02D8BEBA15BCBC01EF61DDE4975004728F3D7E6C8332977E97`; controlled transaction `m2e1-handoff-stage-r1` installed only its DLL/PDB while retaining the complete prior 32-file client tree and preserving the dedicated tree and protected result. This closes the path/hash gate only; formal `NetworkHandoff` and `Connected` require one clean loaded-hash rerun. Because this gate precedes every mission adapter, the finding and correction are shared by every battle type.
