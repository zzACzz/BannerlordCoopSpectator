...
The dedicated server does not bind/list the local endpoint before native `start_game`. Therefore the external `Feasibility` runner may issue a minimum vanilla `TeamDeathmatch` bootstrap only after the dedicated loaded-hash, result-suppression, and native console-readiness gates. The clean `70a40db` rerun proved that `ModuleReady` is published from `OnSubModuleLoad` before the native `IGameNetworkHandler.OnHandleConsoleCommand` path is ready. Milestone 2B.2C drains dedicated stdout/stderr asynchronously, waits for the native `is ready! You can now enter console commands` marker, requires exact per-command option readbacks and native start-game/scene markers, and retains only exact PID-correlated native logs. Clean live run `m2b2c-live-feasibility-20260831-01` did not reach those gates because immediate `Process.Path` acquisition failed after successful creation but before provisional ownership or capture. Process creation must therefore establish provisional cleanup ownership before fallible identity enrichment and classify enrichment failure as internal. The markers, UDP listener, and client handoff remain unproved. This bootstrap has no campaign fixture, cooperative battle authority, campaign result publication, or L2/L3 evidence status. See [BATTLE_TEST_AUTOMATION_M2B2C_RUNNER_CORRECTION.md](BATTLE_TEST_AUTOMATION_M2B2C_RUNNER_CORRECTION.md).
