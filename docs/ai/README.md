...
Current field automation stage: **Milestone 3 complete; Milestone 4 clean published rerun proved the parent-liveness correction and reached `MissionOpening`, then exposed a native `0xc0000005` process failure and a separate live failure-evidence finalization stall/high-memory defect. Full restoration passed, attempt 2 was Not Run, and L2 remains open.** See [BATTLE_TEST_AUTOMATION_M4_FIELD_SPAWN_SMOKE.md](BATTLE_TEST_AUTOMATION_M4_FIELD_SPAWN_SMOKE.md) for exact provenance, evidence limits and next correction order.
