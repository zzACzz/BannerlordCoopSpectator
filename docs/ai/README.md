...
Current field automation stage: **Milestone 3 complete; Milestone 4 failure-evidence collection is now isolated and bounded at source/L1 after the clean live run reached `MissionOpening` and exposed native `0xc0000005`. The correction passed dual-shell focused contracts, 24/24 and both CompileOnly builds, but no post-correction native run or L2 pass exists.** See [BATTLE_TEST_AUTOMATION_M4_FIELD_SPAWN_SMOKE.md](BATTLE_TEST_AUTOMATION_M4_FIELD_SPAWN_SMOKE.md) for exact provenance, evidence limits and next correction order.
