...
Last controlled on-disk staging verification: **2026-09-01** (`m2e-client-stage-r1`, clean pushed revision `6ed40e8`; complete 32-file client pre-image retained; native-login client `8089FC9F...C1FBB` installed; dedicated tree and protected result unchanged; no product process launched; launcher validation deferred because Steam was not running)
...
- Post-M2B.2D doctor was `EnvironmentBlocked` by `InstalledDedicatedHashDiffersFromRepositoryOutput` and `RuntimeVersionCombinationNotYetVerified`. The first blocker was limited to the intentionally untouched ignored repository output; later live runs supplied explicit installed hashes and confirmed both loaded identities, native login, exact server selection, and native join. The handoff observer correction changes the client binary and therefore requires controlled client-only staging; the dedicated does not require restaging. Formal connection and L2–L5 runtime claims remain open.
