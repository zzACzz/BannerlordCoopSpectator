...
Last bounded runtime-feasibility verification: **2026-09-01** (`m2e-live-r1-01`, repository revision `aea5bdd`; exact dedicated/client loaded hashes, complete dedicated bootstrap, UDP ownership, one-shot native platform-login success, `AtLobby`, 52 successful server-list responses, graceful cleanup, and protected-state preservation confirmed; no entry passed every selector, while list-entry contents were not retained)
Last automation-control source/contract verification: **2026-09-01** (`e55f1bd`; focused join/runner contracts and canonical `m2e1-c1`, 22/22; scene `Map` no longer populates optional native `UniqueMapId`; no product build, game process, installed-module mutation, or staging)
...
Last clean live validation: **2026-09-01** (`m2e-live-r1-01`; corrected resolver, exact identities, dedicated control, UDP ownership, native platform login, authenticated lobby, terminal-status retention, PID-log capture, and automatic cleanup passed; corrected server selection, connection, campaign, and battle remain pending a new run)
Repository: `BannerlordCoopSpectator3`
Verification scope: static review of source, project files, module descriptors, tests, scripts, existing documentation, selected installed game assemblies, and ILSpy output, plus the separately documented Milestone 1 installed-runtime probe, post-M1 client-control validation, Milestone 2A contracts/compile-only proof, controlled `0.3.2` staging, Milestone 2B.1 runtime-safety source/contracts, published ownership/handoff corrections, Milestone 2B.2D dedicated control, and live runs through `m2e-live-r1-01`. The latest live run confirmed the staged dedicated and client hashes inside their exact processes, authoritative lifecycle readiness, all seven bootstrap acknowledgements, native `start_game`, exact UDP ownership, exact `NetworkMain.GameClient` resolution, one bounded native `LobbyState.TryLogin()` task, and transition to `AtLobby`. The client then received 52 successful custom-server-list responses, but no entry passed all active selectors; retained evidence did not include list-entry fields. Installed-runtime inspection found that the runner incorrectly used scene `Map` as native serialized `UniqueMapId`. Published correction `e55f1bd` removes that invalid optional filter and is contract-verified, but corrected selection, connection, campaign, and battle behavior remain runtime-unverified.
