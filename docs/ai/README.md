...
Last bounded runtime-feasibility verification: **2026-08-31** (`m2b2c-client-handoff-live-20260831-01`, clean revision `711f2ca`; exact dedicated loaded hash, ownership, service activity, graceful cleanup, and protected-state preservation confirmed; redirected readiness timed out before commands or client launch)
Last automation-control source/contract verification: **2026-08-31** (`m2b2d-control-contracts-final-20260831-01`, 22/22, and `m2b2d-control-compile-final-20260831-01`; final hardened structured dedicated lifecycle/request/seven-acknowledgement channel compiled; installed modules unchanged; no game process launched)
...
Last controlled on-disk staging verification: **2026-09-01** (`m2b2d-stage-20260901-01` and `m2b2d-poststage-doctor-20260901-01`, clean published revision `7628c85`; dedicated-only exact-tree transaction; no product process launched)
...
Last clean live validation: **2026-08-31** (`m2b2c-client-handoff-live-20260831-01`; corrected dedicated ownership/cleanup passed; native-console text was not observable through redirected streams or PID-native logs; no command, UDP listener, client, campaign, or battle followed)
Repository: `BannerlordCoopSpectator3`
Verification scope: static review of source, project files, module descriptors, tests, scripts, existing documentation, selected local server assemblies, and ILSpy output, plus the separately documented Milestone 1 installed-runtime probe, post-M1 client-control validation, Milestone 2A contracts/compile-only proof, controlled `0.3.2` staging, Milestone 2B.1 runtime-safety source/contracts, earlier bounded feasibility attempts, published Revision 10 ownership corrections, Milestone 2B.2D dedicated-control source/contracts/staging, and clean run `m2b2d-live-feasibility-20260901-01`. That run live-confirmed the staged dedicated hash, authoritative lifecycle readiness, all seven bootstrap acknowledgements, native `start_game`, exact UDP ownership, real client launch, and the client loaded hash. It stopped before lobby connection because the aggregate double-applied the local UTC offset while importing the client handoff. Revision 12 corrects that shared runner defect and the profile-dependent watchdog-log policy; focused dual-PowerShell tests and canonical run `m2d-c-01` passed 22/22. Campaign/battle behavior remains unverified.
...
- The exact `0.3.2` client and dedicated binaries compiled from clean pushed revision `12abf36` are installed and hash-confirmed on disk. Four live attempts confirmed the exact dedicated loaded hash with `ResultPolicy=Suppress`. Clean run `m2b2c-client-handoff-live-20260831-01` additionally live-verified the published provisional/verified dedicated ownership and graceful cleanup path, while timing out before command dispatch because the starter's native console did not forward readiness through redirected streams. Server discovery, client loaded-role acknowledgement, join handoff, and connection were not reached.
- The launcher and client module do not issue `start_game` or automate battle phases/results. The Milestone 2B `Feasibility` runner may authorize only the minimum vanilla `TeamDeathmatch` bootstrap after loaded-hash, suppression, and exact owned-role readiness. The default-off run/token/hash/process-bound dedicated lifecycle/request/acknowledgement channel is runtime-verified through all seven steps and UDP ownership. Real client launch and its loaded hash are also confirmed. Revision 12 must still pass a clean published-correction rerun through lobby handoff and connection. This bootstrap is not campaign or L2/L3 battle evidence.
