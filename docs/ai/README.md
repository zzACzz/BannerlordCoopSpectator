...
Last exact-fixture launcher/source verification: **2026-09-02** (`m3b-contracts-01`, 23/23; `m3b-compile-01`, both projects passed below the run root with installed inventories unchanged; dirty capture rejected without product launch; clean staging and real capture pending)
...
- `Tests/contract-tests.manifest.json` is the canonical 23-project inventory. Milestone 3B run `m3b-contracts-01` passed all 23 and `m3b-compile-01` passed both builds without installed mutation; this remains local source/contract evidence, not a clean-published or runtime fixture claim.
...
- Milestone 3A adds a default-off exact post-write `battle_roster.json` recorder and integrity/admission contracts. Milestone 3B adds `Start-CoopFieldFixtureCapture.ps1` plus aggregate `Record`, exact campaign ownership/cleanup, bounded `Skipped`/`Failed`/`Recorded` handling, protected-result verification, and a credential-free reproduction descriptor. `m3b-dirty-block-01` rejected dirty source without launching Bannerlord. No real campaign fixture, independent oracle, sanitized derivative, mission, full battle, or L2/L3 evidence exists yet; clean publication and client-only staging come first.
