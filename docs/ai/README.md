...
Last bounded runtime-feasibility verification: **2026-09-02** (`m2b3a-live-r1-01`, repository revision `5d6a6c6`; exact new dedicated/client loaded hashes, live schema-2 heartbeat/progress, complete bootstrap, native login, terminal `Connected`, graceful cleanup, protected-state preservation, and no crash/hang artifact confirmed; RUN-008 remains open because the artifact contained one concatenated lock instead of six independent locks)
Last automation-control source/contract verification: **2026-09-02** (`m2b3a-lockfix-c-01`; production resource-id builder, expected/acquired cardinality, port deduplication, and per-resource collision/release passed under PowerShell 5.1/7.6.4 and the full 22/22 inventory; runner-only correction not yet clean-published/live-proven)
...
Last clean live validation: **2026-09-02** (`m2b3a-live-r1-01`; client `A363B19B...C91C0075` and dedicated `2E1494BC...C0CA1414` loaded; live role health, exact server selection, terminal `Connected`, protected-state preservation, exact automatic cleanup, and passing-path crash/hang absence confirmed; per-resource lock isolation not confirmed)
Repository: `BannerlordCoopSpectator3`
Verification scope: static review of source, project files, module descriptors, tests, scripts, existing documentation, selected installed game assemblies, and ILSpy output, plus the separately documented Milestone 1 installed-runtime probe, post-M1 client-control validation, Milestone 2A contracts/compile-only proof, controlled `0.3.2` staging, Milestone 2B.1 runtime-safety source/contracts, published ownership/handoff corrections, Milestone 2B.2D dedicated control, and live runs through `m2b3a-live-r1-01`. The latest live run promoted the new client/dedicated identities to `ConfirmedLoadedHash`, retained the full bootstrap, completed native login and exact run-owned server selection, reached terminal `Connected`, published live role heartbeat/progress, cleaned both exact roles gracefully, released both ports, preserved the protected result, and created no crash/hang artifact. Artifact audit found that inline PowerShell construction collapsed six intended shared-lock ids into one combined lock. The runner-only correction is source/contract complete but requires clean publication and repeat live six-lock proof. Campaign and battle behavior remain unverified by this connection-only run.
