...
Verification scope: static review of source, project files, module descriptors, tests, scripts, existing documentation, selected local server assemblies, and ILSpy output, plus the separately documented Milestone 1 installed-runtime probe, post-M1 client-control validation, Milestone 2A contracts/compile-only proof, controlled `0.3.2` staging, Milestone 2B.1 runtime-safety source/contracts, two bounded live feasibility attempts, and Milestone 2B.2C native-output/readback runner correction. The latest live rerun confirms the dedicated loaded hash under `ResultPolicy=Suppress`, six discrete command writes, bounded descendant discovery, and exact cleanup. The new correction is non-runtime verified only: no later UDP listener or client has yet appeared, so client identity, connection, and campaign/battle behavior remain unverified.
...
- The launcher and client module do not issue `start_game` or automate battle phases/results. The Milestone 2B `Feasibility` runner may issue only the minimum vanilla `TeamDeathmatch` bootstrap after loaded-hash, suppression, and exact owned-process native console readiness. Milestone 2B.2C now requires option readbacks plus native game-start/scene evidence before starting the UDP deadline, preserves primary versus terminal outcomes, and captures exact PID logs. These controls are contract-verified but still require a clean live rerun. This bootstrap is not campaign or L2/L3 battle evidence.
