...
Current field automation stage: **Milestone 3 complete; Milestone 4 remains open. The event-tail correction passed 16 focused cases, but clean aggregate `m4fe-p1-c1` subsequently passed only 23/24 because the new test entry point rejected the aggregate's `--nologo` argument. This revision restores that invocation compatibility; both normal runner-project and focused invocations passed before publication.** See [exact regression, correction and validation boundary](BATTLE_TEST_AUTOMATION_M4_FIELD_SPAWN_SMOKE.md#22-aggregate-test-invocation-compatibility-correction-2026-09-10). Subsequent clean aggregate/build evidence belongs to its run reports until M4 closure. No post-correction Bannerlord verification or L2 pass is claimed.
