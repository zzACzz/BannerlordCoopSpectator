...
Last bounded runtime-feasibility verification: **2026-08-31** (`m2b2-live-feasibility-20260831-01`; exact dedicated loaded hash confirmed; server visibility, client launch, and campaign/battle execution were not reached; exact recovery completed)
...
Verification scope: static review of source, project files, module descriptors, tests, scripts, existing documentation, and selected local server assemblies, plus the separately documented Milestone 1 installed-runtime launch/cleanup probe, post-M1 isolated client-control validation, Milestone 2A full contracts/compile-only proof, historical controlled `0.3.2` installation, Milestone 2B.1 runtime-safety source/contracts/compile-only proof, Milestone 2B.2A clean-current-source controlled staging, and the first bounded live feasibility attempt with exact recovery. The latest installation retains its complete immediately preceding `0.3.2` pre-images, while the historical staging run retains the earlier `0.3.1` pre-images. The dedicated role has now reported the exact staged hash under `ResultPolicy=Suppress`; the malformed bootstrap prevented server visibility and client launch, so the client loaded hash, connection, and campaign/battle behavior remain unverified.
...
- The exact `0.3.2` client and dedicated binaries compiled from clean pushed revision `12abf36` are installed and hash-confirmed on disk. Live run `m2b2-live-feasibility-20260831-01` confirmed that the dedicated role loaded its exact expected hash with `ResultPolicy=Suppress`; server discovery, client loaded-role acknowledgement, join handoff, and connection were not reached.
- The launcher and client module do not issue `start_game` or automate battle phases/results. The Milestone 2B.1 `Feasibility` runner may issue only the minimum vanilla `TeamDeathmatch` bootstrap after loaded-hash and `Suppress` gates because the native server does not bind/list beforehand; that bootstrap is not campaign or L2/L3 battle evidence. The first live attempt exposed command-emission and process-tree discovery defects; their correction is source- and contract-verified, while the clean committed rerun is pending.
