...
Verification scope: static review of source, project files, module descriptors, tests, scripts, existing documentation, selected local server assemblies, and ILSpy output, plus the separately documented Milestone 1 installed-runtime probe, post-M1 client-control validation, Milestone 2A contracts/compile-only proof, controlled `0.3.2` staging, Milestone 2B.1 runtime-safety source/contracts, four bounded live feasibility attempts, published Revision 10 aggregate/client post-start ownership corrections, and Milestone 2B.2D dedicated-control source/contracts plus dedicated-only staging. The fourth live attempt, from clean `711f2ca`, live-verified dedicated ownership/cleanup and exposed the native-console observability gap. Published revision `7628c85` resolves that gap in source and its exact dedicated output is now staged, but the new binary is not runtime-verified. No UDP listener or client appeared in the latest live evidence, so client identity, connection, and campaign/battle behavior remain unverified.
...
- The launcher and client module do not issue `start_game` or automate battle phases/results. The Milestone 2B `Feasibility` runner may authorize only the minimum vanilla `TeamDeathmatch` bootstrap after loaded-hash, suppression, and exact owned-role readiness. Revision 11's default-off run/token/hash/process-bound dedicated lifecycle/request/acknowledgement channel exists in source/contracts and its exact dedicated binary is staged; redirected standard handles and exact PID logs are supplementary only. Runtime loading and the client handoff remain live-unverified because no later product process was launched. This bootstrap is not campaign or L2/L3 battle evidence.
...
- Post-M2B.2D doctor is `EnvironmentBlocked` by `InstalledDedicatedHashDiffersFromRepositoryOutput` and `RuntimeVersionCombinationNotYetVerified`. The first blocker is limited to the intentionally untouched ignored repository output from the earlier build; the installed dedicated tree exactly matches the clean run-owned published-revision build. No L2–L5 runtime claim exists.
