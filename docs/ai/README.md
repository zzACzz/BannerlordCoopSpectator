...
Last exact-fixture source/contract verification: **2026-09-02** (`m3a-contracts-03`, 23/23; `m3a-compile-03`, both projects passed below the run root with installed inventories unchanged; no Bannerlord fixture captured yet)
...
- `scripts/Invoke-CoopTest.ps1` owns fresh run roots and exposes non-runtime `Doctor`, `Contracts`, and `CompileOnly`, bounded `Feasibility`, read-only `Inspect`, fail-closed exact-run `Cancel`, and opt-in exact `Recover` control. Protocol 1.1 capability gates cover role health, cancellation, recovery, and failure evidence. The aggregate acquires canonical shared-resource locks before product launch, classifies `NoHeartbeat` separately from `NoProgress`, and correlates owned crash/modal/hang helpers without name-only matching. `scripts/CoopAutomationRunner.Core.ps1` supplies the fixed structured dedicated bootstrap request, strict readiness/status validation, bounded process observation, validated Windows path fallback, provisional/verified identity matching, PID-reuse-resistant cleanup, and pure in-memory descendant discovery from one bounded process snapshot. Terminal status survives failure exceptions/final cleanup reads; after `3fdcda3`, the last validated non-terminal client status also survives external timeout without changing the outcome. Separate exact client-PID native-log inventory is captured automatically.
- `Tests/contract-tests.manifest.json` is the canonical 23-project inventory. Milestone 3A run `m3a-contracts-03` passed all 23; this local source/contract evidence is not yet a clean-published or runtime fixture claim.
...
- Milestone 3A adds a default-off exact post-write `battle_roster.json` recorder and integrity/admission contracts. It passed 23/23 and both compile-only builds without installed mutation. No real campaign fixture, independent oracle, sanitized derivative, reproduction command, mission, or battle evidence exists yet.
