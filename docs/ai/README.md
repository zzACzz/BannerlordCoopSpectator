...
Last bounded runtime-feasibility verification: **2026-09-01** (`m2d-live-r3-01`, clean published revision `729fd2c`; exact dedicated bootstrap, UDP ownership, corrected client loaded hash and `NetworkMain` resolution, stock pre-login state, graceful cleanup, and protected-state preservation confirmed; native platform login timed out before server discovery)
Last automation-control source/contract verification: **2026-09-01** (`m2e-contracts-20260901-03`, 22/22, and `m2e-compile-20260901-02`; exact one-shot native platform login, terminal client status, and separate client PID-log evidence compiled and contract-verified; installed modules unchanged; no game process launched)
...
Last controlled on-disk staging verification: **2026-09-01** (`m2d-client-stage-r2` and `m2d-client-stage-validate-r1`, clean published revision `729fd2c`; complete client pre-image retained; fresh client `1C500...79250` installed; dedicated trees and protected result unchanged; no product process launched)
...
Last clean live validation: **2026-09-01** (`m2d-live-r3-01`; corrected client resolver, exact identities, dedicated control, UDP ownership, stock pre-login state, and automatic cleanup passed; native lobby login, server discovery, connection, campaign, and battle did not follow)
Repository: `BannerlordCoopSpectator3`
Verification scope: static review of source, project files, module descriptors, tests, scripts, existing documentation, selected installed game assemblies, and ILSpy output, plus the separately documented Milestone 1 installed-runtime probe, post-M1 client-control validation, Milestone 2A contracts/compile-only proof, controlled `0.3.2` staging, Milestone 2B.1 runtime-safety source/contracts, published ownership/handoff corrections, Milestone 2B.2D dedicated control, and live runs through `m2d-live-r3-01`. The latest run live-confirmed the staged dedicated hash, authoritative lifecycle readiness, all seven bootstrap acknowledgements, native `start_game`, exact UDP ownership, exact corrected client hash, schema-v4 handoff, and exact `NetworkMain.GameClient` resolution. It reached the stock `Not Logged In` state with `LobbyState=Idle`. Revision 13 source/contracts now implement the next default-off one-shot native login gate and runner evidence corrections, but that candidate is not installed; native platform login, server discovery, connection, campaign, and battle behavior remain runtime-unverified.
...
- The exact dedicated `0.3.2` binary and corrected client SHA-256 `1C500501CE25D4A520782F61B338F9D8D0A4C591A4748E80991A521B63379250` are installed and hash-confirmed on disk. Transaction `m2d-client-stage-r2` retained the complete prior client tree; live run `m2d-live-r3-01` confirmed both loaded identities and exact automatic cleanup.
- The launcher and client module do not issue `start_game` or automate battle phases/results. The Milestone 2B `Feasibility` runner may authorize only the minimum vanilla `TeamDeathmatch` bootstrap after loaded-hash, suppression, and exact owned-role readiness. The default-off run/token/hash/process-bound dedicated lifecycle/request/acknowledgement channel is runtime-verified through all seven steps and UDP ownership. The corrected installed client resolves `NetworkMain.GameClient`, but remains `Not Logged In` with `LobbyState=Idle`. Revision 13 source now permits one bounded native `LobbyState.TryLogin()` attempt before server discovery, without UI automation, credentials, bypass, or blind retry, and publishes explicit task/outcome evidence. This new source is not yet staged or runtime-verified. The bootstrap is not campaign or L2/L3 battle evidence.
...
- `Tests/contract-tests.manifest.json` is the canonical 22-project inventory; historical Milestone 2A run `m2a-contracts-20260831-07` passed its then-current 20, clean-revision run `m2b2-contracts-20260831-01` passed the then-current 21, and final native-login run `m2e-contracts-20260901-03` passed all current 22 after the last test update.
- `CoopCompileOnly=true` redirects all build/package state below the run root and disables installation deployment; final run `m2e-compile-20260901-02` compiled both version `0.3.2` assemblies after the last source-text update and proved installed module inventories unchanged, producing unstaged client candidate SHA-256 `6DE0B8AE1F38626FD11B7BDE59E86AD7AA106CD7E96596C7FECB2E137C4E8551`.
...
- Post-M2B.2D doctor was `EnvironmentBlocked` by `InstalledDedicatedHashDiffersFromRepositoryOutput` and `RuntimeVersionCombinationNotYetVerified`. The first blocker was limited to the intentionally untouched ignored repository output; the live run supplied explicit installed hashes and confirmed both loaded identities. Connection and L2–L5 runtime claims remain open.
