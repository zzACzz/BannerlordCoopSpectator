...
Last bounded runtime-feasibility verification: **2026-09-01** (`m2e1-handoff-live-r1-01`, repository revision `d0648ed`; exact dedicated/client loaded hashes, complete dedicated bootstrap, native login, exact server selection, native join, post-rewrite `NetworkHandoff`, terminal `Connected`, server `CreatePlayer`, graceful cleanup, and protected-state preservation confirmed; runner returned `Pass`)
...
Last clean live validation: **2026-09-01** (`m2e1-handoff-live-r1-01`; corrected client `7CC2D759...2977E97` and dedicated `BD328AAC...702A78` loaded; exact server selection, native join, actual GameNetwork client start, terminal `Connected`, `Join game successful`, server `CreatePlayer`, protected-state preservation, and exact automatic cleanup confirmed; runner returned `Pass`)
Repository: `BannerlordCoopSpectator3`
Verification scope: static review of source, project files, module descriptors, tests, scripts, existing documentation, selected installed game assemblies, and ILSpy output, plus the separately documented Milestone 1 installed-runtime probe, post-M1 client-control validation, Milestone 2A contracts/compile-only proof, controlled `0.3.2` staging, Milestone 2B.1 runtime-safety source/contracts, published ownership/handoff corrections, Milestone 2B.2D dedicated control, and live runs through `m2e1-handoff-live-r1-01`. The latest live run promoted the handoff-corrected client to `ConfirmedLoadedHash`, retained the exact dedicated identity and full bootstrap, completed native login and exact run-owned server selection, observed the post-rewrite GameNetwork handoff, reached terminal `Connected`, retained native join success and server `CreatePlayer`, returned `Pass`, cleaned both exact roles gracefully, released both ports, and preserved the protected result. This closes the two formerly blocked Milestone 1 capability rows. The broader Milestone 2B remains open after a source/test audit found missing role-liveness/progress evidence, shared runtime locks, aggregate cancellation proof, complete automated recovery contracts, and structured crash/modal/hang evidence. Campaign and battle behavior remain unverified by this connection-only run.
...
- The exact dedicated `0.3.2` binary remains `BD328...02A78`. Transaction `m2e1-handoff-stage-r1` installed handoff-corrected client SHA-256 `7CC2D759806D2F02D8BEBA15BCBC01EF61DDE4975004728F3D7E6C8332977E97`, retained the complete prior 32-file tree containing `8089...C1FBB`, and preserved the dedicated tree plus protected result. Run `m2e1-handoff-live-r1-01` promoted the new client to `ConfirmedLoadedHash`.
...
- Later live runs supplied explicit installed hashes and confirmed both loaded identities, native login, exact server selection, native join, formal handoff, and terminal connection. The Milestone 1 capability matrix is therefore fully `Confirmed`. Milestone 2B still requires a battle-type-independent safety-closure slice for role heartbeat/progress, shared machine locks, aggregate cancellation, complete recovery contracts, and structured crash/modal/hang evidence before fixture work begins.
