...
Last bounded runtime-feasibility verification: **2026-09-01** (`m2e1-live-r1-01`, repository revision `9c3930b`; exact dedicated/client loaded hashes, complete dedicated bootstrap, native login, exact server selection, native join, actual GameNetwork client start, `InCustomGame`, server `CreatePlayer`, graceful cleanup, and protected-state preservation confirmed; formal status remained non-terminal `JoinAccepted` and the runner returned `Timeout`)
Last automation-control source/contract verification: **2026-09-01** (`d1af692`, `3fdcda3`; focused join/runner contracts, canonical `m2e1-handoff-fix-contracts-20260901-01` at 22/22, and isolated compile `m2e1-handoff-fix-compile-20260901-01`; actual GameNetwork handoff observer and timeout-status retention corrected; installed modules unchanged)
...
Last controlled on-disk staging verification: **2026-09-01** (`m2e1-handoff-pub-compile-20260901-01` and `m2e1-handoff-stage-r1`, clean pushed revision `a7bd528`; complete 32-file client pre-image retained; handoff-corrected client `7CC2D759...2977E97` installed; dedicated tree and protected result unchanged; no product process launched)
...
Last clean live validation: **2026-09-01** (`m2e1-live-r1-01`; exact identities, dedicated control, native login, exact server selection, native join, actual GameNetwork client start, `InCustomGame`, `Join game successful`, server `CreatePlayer`, visible `Awaiting Server`, protected-state preservation, and automatic cleanup confirmed; formal status remained `JoinAccepted` and the runner returned `Timeout` because the staged observer targeted an absent historical lobby signature)
Repository: `BannerlordCoopSpectator3`
Verification scope: static review of source, project files, module descriptors, tests, scripts, existing documentation, selected installed game assemblies, and ILSpy output, plus the separately documented Milestone 1 installed-runtime probe, post-M1 client-control validation, Milestone 2A contracts/compile-only proof, controlled `0.3.2` staging, Milestone 2B.1 runtime-safety source/contracts, published ownership/handoff corrections, Milestone 2B.2D dedicated control, and live runs through `m2e1-live-r1-01`. The latest live run confirmed the then-staged dedicated/client hashes, complete dedicated bootstrap, successful native login, exact run-owned server selection, native join acceptance, actual `GameNetwork.StartMultiplayerOnClient(...)`, same-machine loopback rewrite, `InCustomGame`, `Join game successful`, server `CreatePlayer`, and visible `Awaiting Server`. It formally timed out at non-terminal `JoinAccepted` because only an absent historical lobby signature notified the controller. Published corrections `d1af692` and `3fdcda3` move observation to the actual GameNetwork boundary and retain last-valid timeout evidence. Clean published compile `m2e1-handoff-pub-compile-20260901-01` and controlled transaction `m2e1-handoff-stage-r1` installed the corrected client as `ConfirmedPathHashOnly`; formal `Connected`, campaign, and battle behavior remain runtime-unverified until a clean hash-pinned rerun confirms that loaded identity.
...
- The exact dedicated `0.3.2` binary remains `BD328...02A78`. Transaction `m2e1-handoff-stage-r1` installs handoff-corrected client SHA-256 `7CC2D759806D2F02D8BEBA15BCBC01EF61DDE4975004728F3D7E6C8332977E97`, retains the complete prior 32-file tree containing `8089...C1FBB`, and preserves the dedicated tree plus protected result. The new client is `ConfirmedPathHashOnly`; run `m2e1-live-r1-01` confirmed the retained prior client, not the newly installed correction, from inside the process.
- The launcher and client module do not issue `start_game` or automate battle phases/results. The Milestone 2B `Feasibility` runner may authorize only the minimum vanilla `TeamDeathmatch` bootstrap after loaded-hash, suppression, and exact owned-role readiness. The default-off run/token/hash/process-bound dedicated lifecycle/request/acknowledgement channel is runtime-verified through all seven steps and UDP ownership. Revision 13's bounded native `LobbyState.TryLogin()` attempt is runtime-confirmed through `AtLobby`, without UI automation, credentials, bypass, or blind retry. Revision 14 requires scene `Map` and native serialized `UniqueMapId` to remain distinct. Revision 15 records successful exact server selection/native join and requires the actual GameNetwork patch to own handoff observation after the final address rewrite. Formal `Connected` remains open until the corrected staged client is loaded and passes a clean rerun. The bootstrap is not campaign or L2/L3 battle evidence.

### Battle-test non-runtime foundation

- `scripts/Invoke-CoopTest.ps1` owns fresh run roots and exposes non-runtime `Doctor`, `Contracts`, and `CompileOnly`, plus bounded `Feasibility`, read-only `Inspect`, and opt-in exact `Recover` control. `scripts/CoopAutomationRunner.Core.ps1` supplies the fixed structured dedicated bootstrap request, strict readiness/status validation, bounded process observation, validated Windows path fallback, provisional/verified identity matching, PID-reuse-resistant cleanup, and pure in-memory descendant discovery from one bounded process snapshot. Terminal status survives failure exceptions/final cleanup reads; after `3fdcda3`, the last validated non-terminal client status also survives external timeout without changing the outcome. Separate exact client-PID native-log inventory is captured automatically.
- `Tests/contract-tests.manifest.json` is the canonical 22-project inventory; clean pushed native-login run `m2e-pub-contracts-r1` passed all current 22.
...
- Post-M2B.2D doctor was `EnvironmentBlocked` by `InstalledDedicatedHashDiffersFromRepositoryOutput` and `RuntimeVersionCombinationNotYetVerified`. The first blocker was limited to the intentionally untouched ignored repository output; later live runs supplied explicit installed hashes and confirmed both then-loaded identities, native login, exact server selection, and native join. The handoff observer correction is now installed through controlled client-only transaction `m2e1-handoff-stage-r1`; the dedicated did not require restaging. Formal connection and L2–L5 runtime claims remain open until the new client identity is loaded and the later milestone paths execute.
