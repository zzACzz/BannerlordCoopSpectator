...
Last live-runner correction verification: **2026-08-31** (aggregate correction `8f68d43` published; client handoff focused Windows PowerShell 5.1/7 coverage passed; `m2b2c-client-handoff-contracts-20260831-01` passed 22/22; `m2b2c-client-handoff-compile-20260831-01` passed; no product process launched; installed modules unchanged)
Last clean live validation: **2026-08-31** (`m2b2c-live-feasibility-20260831-01`; dedicated launch succeeded, immediate process-path acquisition failed before ownership/output capture, exact manual recovery completed, installed modules and protected result unchanged)
Repository: `BannerlordCoopSpectator3`
Verification scope: static review of source, project files, module descriptors, tests, scripts, existing documentation, selected local server assemblies, and ILSpy output, plus the separately documented Milestone 1 installed-runtime probe, post-M1 client-control validation, Milestone 2A contracts/compile-only proof, controlled `0.3.2` staging, Milestone 2B.1 runtime-safety source/contracts, three bounded live feasibility attempts, and Revision 10 aggregate/client post-start ownership corrections. The third live attempt confirmed a clean `e62f536` launch and exact dedicated loaded hash, but exposed the aggregate process-identity/ownership gap before output capture; source/contracts now close that gap and the analogous client-launch handoff. No UDP listener or client appeared in live evidence, so client identity, connection, and campaign/battle behavior remain unverified.
...
| [BATTLE_TEST_AUTOMATION_M2B2C_RUNNER_CORRECTION.md](BATTLE_TEST_AUTOMATION_M2B2C_RUNNER_CORRECTION.md) | Native-output readiness/readback, singular-result normalization, primary-outcome preservation, exact PID-log capture, and final source/contract evidence |
...
- The exact `0.3.2` client and dedicated binaries compiled from clean pushed revision `12abf36` are installed and hash-confirmed on disk. All three live attempts confirmed the exact dedicated loaded hash with `ResultPolicy=Suppress`; the clean `70a40db` rerun additionally confirmed six separate command writes and bounded cleanup, while the clean `e62f536` attempt exposed immediate process-path/ownership failure before command handling. Server discovery, client loaded-role acknowledgement, join handoff, and connection were not reached.
- The launcher and client module do not issue `start_game` or automate battle phases/results. The Milestone 2B `Feasibility` runner may issue only the minimum vanilla `TeamDeathmatch` bootstrap after loaded-hash, suppression, and exact owned-process native console readiness. Milestone 2B.2C requires option readbacks plus native game-start/scene evidence before starting the UDP deadline, preserves primary versus terminal outcomes, and captures exact PID logs. Revision 10 provisional ownership, bounded exact path fallback, internal-failure classification, and provisional cleanup are implemented and non-runtime verified in both the aggregate runner and client-launcher handoff, but remain live-unproven. This bootstrap is not campaign or L2/L3 battle evidence.
