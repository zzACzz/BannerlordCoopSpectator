...
Current field automation stage: **Milestone 3 complete; Milestone 4 parent/child liveness now has per-fact failure evidence and time-sliced output capture, verified at L1 by 24/24 contracts and both builds. A clean published native L2 rerun remains pending.** See [BATTLE_TEST_AUTOMATION_M4_FIELD_SPAWN_SMOKE.md](BATTLE_TEST_AUTOMATION_M4_FIELD_SPAWN_SMOKE.md) for profile, source map, validation artifacts and the same-process reset gap.
