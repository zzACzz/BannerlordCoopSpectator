...
Current field automation stage: **Milestone 3 complete; Milestone 4 has a clean-published source/L1 correction for the recovered `MissionScoreboardComponent` dependency failure. Direct CoopBattle and full TdmClone server stacks now receive a minimal `MissionMultiplayerGameModeBaseClient` dependency before the scoreboard, while already-safe hideout/siege paths are preserved. Controlled staging, native lifecycle confirmation and L2 remain open.** See [BATTLE_TEST_AUTOMATION_M4_FIELD_SPAWN_SMOKE.md](BATTLE_TEST_AUTOMATION_M4_FIELD_SPAWN_SMOKE.md) for exact provenance, evidence limits and the next live gate.
