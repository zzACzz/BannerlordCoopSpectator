...
- The launcher and client module do not issue `start_game` or automate battle phases/results. The Milestone 2B `Feasibility` runner may authorize only the minimum vanilla `TeamDeathmatch` bootstrap after loaded-hash, suppression, and exact owned-role readiness. Revision 11's default-off run/token/hash/process-bound dedicated lifecycle/request/acknowledgement channel now exists in source and contracts; redirected standard handles and exact PID logs are supplementary only. The new dedicated binary is not staged or runtime-verified, and the client handoff remains live-unverified because the client was not launched. This bootstrap is not campaign or L2/L3 battle evidence.
