...
The Milestone 2B safety foundation is not yet complete. The closure audit found no implementation/contract proof for the complete live-role heartbeat/progress projection or distinct `NoHeartbeat`/`NoProgress` classification; no exclusive shared-resource runtime locks beyond the fresh run-root lock and staging mutexes; no aggregate `Ctrl+C`/explicit cancellation proof; no complete automated `Recover` preview/revalidation/action/reporting matrix; and no owned crash/modal correlation, `FatalAutomationFailure`, or guaranteed `crash.json`/`hang.json` evidence path. Existing exact cleanup, synthetic post-start cleanup, and real abandoned-run recovery evidence remain valid but do not substitute for those missing contracts.

The next gate is a separately approved battle-type-independent Milestone 2B safety-closure implementation and verification slice. It must not open a campaign, cooperative mission, or battle and must not claim L2/L3 evidence. Milestone 3 must not begin until this residual matrix passes.
