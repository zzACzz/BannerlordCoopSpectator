...
The installed client remains the exact clean revision `12abf36` output documented in [BATTLE_TEST_AUTOMATION_M2B2_STAGING.md](BATTLE_TEST_AUTOMATION_M2B2_STAGING.md). Clean runner revision `70a40db` proved that `ModuleReady` preceded native console readiness and no UDP listener appeared. Milestone 2B.2C implemented and contract-tested native-output readiness, exact command/start readbacks, singular runner results, and PID-correlated native-log capture without changing game-side source. Clean live revision `e62f536` then exposed a null immediate `Process.Path` after successful creation, before ownership/capture; exact manual recovery left installed modules and the protected result unchanged. The provisional post-start ownership and bounded exact-identity correction was subsequently published in `8f68d43`, and the client-launcher ownership handoff was published in `711f2ca`. Clean run `m2b2c-client-handoff-live-20260831-01` proved dedicated ownership and cleanup but also proved that redirected standard streams are not a trustworthy sole readiness channel for this exact starter profile.

Milestone 2B.2D implements, contract-tests, and now stages the required run-scoped dedicated readiness/request/acknowledgement channel from published revision `7628c85`; see [BATTLE_TEST_AUTOMATION_M2B2D_DEDICATED_CONTROL.md](BATTLE_TEST_AUTOMATION_M2B2D_DEDICATED_CONTROL.md). The staged dedicated SHA-256 is `BD328AAC4F2A64C28D3EDCE28BCE3D72FF164BDAF817D9460B302ED538702A78`, while the client remains `B576B8EA0FB223126A65E062CB562FD15815DF8BA1ADDB1797506914B48D7928`. The next gate is a clean published-revision `Feasibility` run with both hashes explicit. Until that runtime gate passes, the project must preserve the no-client, no-campaign, and no-L2/L3 evidence boundary.
