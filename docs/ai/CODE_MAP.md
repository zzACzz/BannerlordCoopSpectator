...
- `Multiplayer/Automation/CoopLobbyAutomationDriver.cs`: narrow reflection adapter that resolves `TaleWorlds.MountAndBlade.NetworkMain` from its exact defining `TaleWorlds.MountAndBlade` assembly, distinguishes structural resolution failures from a not-yet-created `GameClient`, and performs custom-server discovery plus custom-game join. Revision 13 plans a one-shot adapter to stock `LobbyState.TryLogin()` before discovery; that login adapter is not implemented yet.
- `Multiplayer/Automation/CoopLobbyAutomationController.cs`: main-thread polling, exact local-server ownership gate, native join lifecycle, acknowledgement, and safe cancellation boundary. Its next planned state transition is explicit native platform-login evidence while `LobbyState` remains `Idle`.
