...
- `scripts/Invoke-CoopTest.ps1`: `Doctor`, full `Contracts`, client/dedicated `CompileOnly`, runtime `Feasibility`, existing-run `Inspect`, fail-closed exact `Cancel`, and opt-in exact `Recover`. It owns the run root, runner lock, nonce fingerprint, lease, schema-2 runner status, events, assertions, provisional-to-verified process identities, cleanup, logs, and reports. Protocol 1.1 adds `RoleHealthV1`, `CancellationV1`, `RecoveryV2`, and `FailureEvidenceV1`; `Feasibility` acquires canonical game/dedicated/bridge/profile/port locks before product launch and verifies release. A dedicated launch is provisionally inventoried before bounded exact path/start/parent enrichment. `Feasibility` waits for structured dedicated readiness and all seven acknowledgements before UDP/client work, then validates and registers the complete client handoff before fallible live revalidation. The current bootstrap requests exact run-unique name, port, and `TeamDeathmatch` game type but omits optional `UniqueMapId` because no authoritative serialized native value is available; it never substitutes scene `Map`. It preserves terminal/non-terminal client evidence without promoting timeout, distinguishes `NoHeartbeat` from `NoProgress`, inventories PID logs separately, and writes exact-correlated crash/hang evidence before cleanup.
