...
- `Multiplayer/Automation/CoopLobbyAutomationController.cs`: main-thread polling, one retained native platform-login task with no blind retry or false cancellation, explicit login outcomes, exact local-server ownership gate, native join lifecycle, acknowledgement, and safe cancellation boundary.
