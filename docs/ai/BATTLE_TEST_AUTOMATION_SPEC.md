...
**Current boundary (2026-09-10):** The approved mode initializer and exact zero-client clock/initial-peer admission were published as `438b434`; fresh clean contracts passed 24/24 and both CompileOnly builds passed. The [bounded live attempt](BATTLE_TEST_AUTOMATION_M4_FIELD_SPAWN_SMOKE.md#26-clean-zero-client-live-attempt-stops-before-bootstrap-2026-09-10) loaded the candidate but stopped with `NoHeartbeat` before dedicated control readiness or any bootstrap command. Later retained role health is fresh; the failing read and classification timestamp are not preserved, so transient unreadability versus a real heartbeat gap remains unresolved. Automatic timeout finalization, exact cleanup and complete temporary-install restoration passed; attempt two was Not Run. Mission/agent/early-abort assertions remain unverified, and Milestone 4 stays open. Preserve the [narrow pre-battle admission contract](RUNTIME_FLOWS.md#field-automation-zero-client-pre-battle-admission-2026-09-10-source-contract), production/client/active-battle gates, exact fixture, native-only spawning and result suppression. Further role-health implementation or a live rerun requires a revised approved plan.
