...
Revision: **20 — Milestone 3A exact campaign-roster recorder and rejection contracts implemented; runtime fixture still pending**
...
Implementation status on 2026-09-02: the 3A source/contract slice is complete. It records exact post-write `battle_roster.json` bytes only under the explicit run-scoped fixture profile, admits only the ordinary mixed SCN-001 field slice, retains immutable metadata/hash/provenance, and rejects corruption, schema mismatch, and path escape. Final runs `m3a-contracts-03` and `m3a-compile-03` passed 23/23 plus both compile-only projects with installed inventories unchanged. No real Bannerlord fixture has been captured, independently reviewed, sanitized, committed, or replayed, and no reproduction command or L2/L3 claim exists. See [BATTLE_TEST_AUTOMATION_M3_FIELD_FIXTURE.md](BATTLE_TEST_AUTOMATION_M3_FIELD_FIXTURE.md).
