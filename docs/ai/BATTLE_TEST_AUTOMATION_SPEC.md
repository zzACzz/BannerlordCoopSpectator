...
Revision: **13 — source-implemented bounded native platform-login gate after the live-proven exact client resolver**
Live-evidence source baseline: **`729fd2c325019ca026787b4ffcccf91beeced755`** (`729fd2c`)
...
The companion audit remains the source-fact baseline. Revisions 2–13 refine implementation requirements and ordering without changing the audit's historical source findings. The narrow client launch/join slice, Milestone 2A non-runtime foundation, Milestone 2B runner-safety foundation, and dedicated-control channel now exist. Their evidence boundaries are recorded in [BATTLE_TEST_AUTOMATION_CLIENT_JOIN_IMPLEMENTATION.md](BATTLE_TEST_AUTOMATION_CLIENT_JOIN_IMPLEMENTATION.md), [BATTLE_TEST_AUTOMATION_M2A_IMPLEMENTATION.md](BATTLE_TEST_AUTOMATION_M2A_IMPLEMENTATION.md), [BATTLE_TEST_AUTOMATION_M2B1_RUNTIME_FOUNDATION.md](BATTLE_TEST_AUTOMATION_M2B1_RUNTIME_FOUNDATION.md), [BATTLE_TEST_AUTOMATION_M2B2_FEASIBILITY.md](BATTLE_TEST_AUTOMATION_M2B2_FEASIBILITY.md), [BATTLE_TEST_AUTOMATION_M2B2C_RUNNER_CORRECTION.md](BATTLE_TEST_AUTOMATION_M2B2C_RUNNER_CORRECTION.md), and [BATTLE_TEST_AUTOMATION_M2B2D_DEDICATED_CONTROL.md](BATTLE_TEST_AUTOMATION_M2B2D_DEDICATED_CONTROL.md). Run `m2d-live-r3-01` live-proved the staged dedicated/client hashes, authoritative readiness, all seven bootstrap acknowledgements, `start_game`, exact UDP ownership, schema-v4 handoff, exact `NetworkMain.GameClient` resolution, protected-state preservation, and graceful cleanup. It stopped at the stock `Not Logged In` boundary with `LobbyState=Idle`; no server discovery, connection, campaign, cooperative mission, L2, or L3 evidence was produced. Revision 13 source now implements the exact one-shot native login boundary plus terminal client-status and exact client-PID log retention; contracts and compile-only evidence exist, but the new client binary has not been published, staged, or exercised in Bannerlord.
...
Implementation status: **Milestone 2B.1 source/contracts, Milestone 2B.2A staging, exact dedicated loaded identity, bounded process ownership/cleanup, result suppression, and the Revision 10 ownership corrections are evidenced. Milestone 2B.2D implements and live-proves the Revision 11 default-off structured dedicated-control channel through authoritative readiness, seven native acknowledgements, `start_game`, and exact UDP ownership. Revision 12 then established schema-v4 client ownership, UTC-safe aggregation, exact cleanup, and the evidence-backed watchdog policy. Published correction `729fd2c` resolves `NetworkMain` from its exact defining assembly. Fresh build `m2d-r13-pub-b1` passed both projects; controlled client-only staging `m2d-client-stage-r2` installed client SHA-256 `1C500501CE25D4A520782F61B338F9D8D0A4C591A4748E80991A521B63379250`, retained the exact prior 32-file tree, and preserved the dedicated installation and protected result. Clean run `m2d-live-r3-01` live-confirmed that corrected client hash, the dedicated hash, the entire bootstrap, UDP ownership, schema-v4 handoff, exact cleanup, and unchanged result state. It also proved the next shared prerequisite: the client reached `NetworkMain` and active multiplayer lobby state but remained `Idle` at the stock `Not Logged In` screen, so no server-list or join call could begin. Revision 13 now implements a one-shot, main-tick, exact-native `LobbyState.TryLogin()` gate with no credential handling or UI synthesis, request schema 3 login evidence, aggregate terminal-status preservation, and exact client-PID log inventory. Focused contracts passed in both supported PowerShell engines; final canonical run `m2e-contracts-20260901-03` passed 22/22 after the last test update; final compile-only run `m2e-compile-20260901-02` built both projects after the last source-text update with installed inventories unchanged and produced client candidate SHA-256 `6DE0B8AE1F38626FD11B7BDE59E86AD7AA106CD7E96596C7FECB2E137C4E8551`. Connection, campaign, cooperative mission, battle, L2, and L3 remain unproved until that candidate is published, transactionally staged, and live-verified.** See [BATTLE_TEST_AUTOMATION_M2B1_RUNTIME_FOUNDATION.md](BATTLE_TEST_AUTOMATION_M2B1_RUNTIME_FOUNDATION.md), [BATTLE_TEST_AUTOMATION_M2B2_STAGING.md](BATTLE_TEST_AUTOMATION_M2B2_STAGING.md), [BATTLE_TEST_AUTOMATION_M2B2_FEASIBILITY.md](BATTLE_TEST_AUTOMATION_M2B2_FEASIBILITY.md), [BATTLE_TEST_AUTOMATION_M2B2C_RUNNER_CORRECTION.md](BATTLE_TEST_AUTOMATION_M2B2C_RUNNER_CORRECTION.md), and [BATTLE_TEST_AUTOMATION_M2B2D_DEDICATED_CONTROL.md](BATTLE_TEST_AUTOMATION_M2B2D_DEDICATED_CONTROL.md).
