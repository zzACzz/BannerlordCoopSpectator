...
Revision: **22 — Milestone 3B clean staging and first exact private runtime fixture captured; independent review and replay still pending**
...
Implementation status on 2026-09-02: the 3A recorder and 3B campaign-capture control surface are complete, clean-published, staged client-only, and live-proven at the exact pre-mission campaign-roster boundary. The first diagnostic run produced valid bytes but exposed stale aggregate payload-kind/boundary literals; published correction `baf5c69` aligns the runner with the canonical C# constants and adds a cross-language guard. Corrected runs `m3b-fix-pub-contracts-01` and `m3b-fix-pub-compile-01` passed 23/23 and both compile-only projects with installed inventories unchanged. `m3b-live-capture-02` then returned `Pass` with exact staged client SHA-256 `2A1E17E4...250FFB4`, payload SHA-256 `ECF29661...073B9C7`, two populated sides, infantry, mounted, and hero/captain groups, unchanged protected result, and zero remaining product processes/port listeners. The raw fixture remains private, unreviewed, unsanitized, uncommitted, and unreplayed; no independent oracle or L2/L3 claim exists. See [BATTLE_TEST_AUTOMATION_M3_FIELD_FIXTURE.md](BATTLE_TEST_AUTOMATION_M3_FIELD_FIXTURE.md).
