...
Revision: **14 — native platform login live-proven; native map identity and optional unique-map selection separated**
Live-evidence repository baseline: **`aea5bddacce3644cbada95a7760d48d3e28b4e73`** (`aea5bdd`)
Current source correction: **`e55f1bd`**
...
The companion audit remains the source-fact baseline. Revisions 2–14 refine implementation requirements and ordering without changing the audit's historical source findings. The narrow client launch/join slice, Milestone 2A non-runtime foundation, Milestone 2B runner-safety foundation, and dedicated-control channel now exist. Their evidence boundaries are recorded in [BATTLE_TEST_AUTOMATION_CLIENT_JOIN_IMPLEMENTATION.md](BATTLE_TEST_AUTOMATION_CLIENT_JOIN_IMPLEMENTATION.md), [BATTLE_TEST_AUTOMATION_M2A_IMPLEMENTATION.md](BATTLE_TEST_AUTOMATION_M2A_IMPLEMENTATION.md), [BATTLE_TEST_AUTOMATION_M2B1_RUNTIME_FOUNDATION.md](BATTLE_TEST_AUTOMATION_M2B1_RUNTIME_FOUNDATION.md), [BATTLE_TEST_AUTOMATION_M2B2_FEASIBILITY.md](BATTLE_TEST_AUTOMATION_M2B2_FEASIBILITY.md), [BATTLE_TEST_AUTOMATION_M2B2C_RUNNER_CORRECTION.md](BATTLE_TEST_AUTOMATION_M2B2C_RUNNER_CORRECTION.md), and [BATTLE_TEST_AUTOMATION_M2B2D_DEDICATED_CONTROL.md](BATTLE_TEST_AUTOMATION_M2B2D_DEDICATED_CONTROL.md). Run `m2e-live-r1-01` live-proved the staged dedicated/client hashes, authoritative readiness, all seven bootstrap acknowledgements, `start_game`, exact UDP ownership, schema-v4 handoff, exact `NetworkMain.GameClient` resolution, one successful native `LobbyState.TryLogin()` task, transition to `AtLobby`, 52 successful custom-server-list responses, protected-state preservation, and graceful cleanup. The retained evidence proves only that no entry satisfied every requested selector; it does not retain list entries and therefore does not prove that the run-owned name/port identity was absent. Installed-runtime inspection found that the aggregate runner incorrectly supplied scene name `mp_tdm_map_001` as `UniqueMapId`, while the dedicated sends a separately serialized native `UniqueSceneId` or null. Published correction `e55f1bd` omits that unavailable optional filter, preserves the run-unique name/port/game-type and owned-host gates, and passed focused contracts plus canonical run `m2e1-c1` at 22/22 without launching a product process. Connection, campaign, cooperative mission, L2, and L3 evidence remain pending a clean live rerun.
...
Implementation status: **Milestone 2B.1 source/contracts, controlled staging, exact dedicated/client loaded identity, bounded ownership/cleanup, result suppression, authoritative dedicated control, native `start_game`, exact UDP ownership, schema-v4 client handoff, corrected `NetworkMain` resolution, and one-shot native platform login are live-evidenced. Clean run `m2e-live-r1-01` reached `AtLobby` and completed 52 custom-server-list responses, but no entry passed every selector and no join began. The retained list evidence was insufficient to claim server absence. Installed-runtime inspection then proved that the runner conflated scene `Map` with native serialized `UniqueMapId`; published runner-only correction `e55f1bd` removes that invalid optional filter while retaining the exact name/port/game-type and owned-host gates. Focused contracts passed, and canonical non-runtime run `m2e1-c1` passed 22/22 without a product launch. No DLL changed, so no new staging is required. Connection, campaign, cooperative mission, battle, L2, and L3 remain unproved until a clean live rerun verifies selection, join, handoff, and connected state.** See [BATTLE_TEST_AUTOMATION_M2B1_RUNTIME_FOUNDATION.md](BATTLE_TEST_AUTOMATION_M2B1_RUNTIME_FOUNDATION.md), [BATTLE_TEST_AUTOMATION_M2B2_STAGING.md](BATTLE_TEST_AUTOMATION_M2B2_STAGING.md), [BATTLE_TEST_AUTOMATION_M2B2_FEASIBILITY.md](BATTLE_TEST_AUTOMATION_M2B2_FEASIBILITY.md), [BATTLE_TEST_AUTOMATION_M2B2C_RUNNER_CORRECTION.md](BATTLE_TEST_AUTOMATION_M2B2C_RUNNER_CORRECTION.md), and [BATTLE_TEST_AUTOMATION_M2B2D_DEDICATED_CONTROL.md](BATTLE_TEST_AUTOMATION_M2B2D_DEDICATED_CONTROL.md).
