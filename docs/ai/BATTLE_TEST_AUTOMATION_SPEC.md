...
Implementation status: **Milestone 2B.1 source/contracts, Milestone 2B.2A staging, exact dedicated loaded identity, bounded process ownership/cleanup, result suppression, and the Revision 10 ownership corrections are evidenced on 2026-08-31. Clean run `m2b2c-client-handoff-live-20260831-01` proved exact dedicated ownership and cleanup but disproved redirected standard handles as the authoritative readiness channel. Milestone 2B.2D now implements the Revision 11 default-off structured dedicated-control channel in source: the exact native `InitialListedGameServerState.OnActivated` lifecycle event produces a run/token/hash/process-bound ready status; a fixed allowlisted request is claimed atomically; `GameNetwork.HandleConsoleCommand` applies the bootstrap from the main tick; native option/map/listed-server state produces exactly seven ordered acknowledgements. Focused Windows PowerShell 5.1/PowerShell 7 contracts passed, final full run `m2b2d-control-contracts-final-20260831-01` passed 22/22, and `m2b2d-control-compile-final-20260831-01` built both modules with installed inventories unchanged. The new dedicated binary is not staged or runtime-verified. UDP visibility, client launch, client loaded identity, lobby handoff, connection, campaign, mission, battle, L2, and L3 remain unproved.** See [BATTLE_TEST_AUTOMATION_M2B1_RUNTIME_FOUNDATION.md](BATTLE_TEST_AUTOMATION_M2B1_RUNTIME_FOUNDATION.md), [BATTLE_TEST_AUTOMATION_M2B2_STAGING.md](BATTLE_TEST_AUTOMATION_M2B2_STAGING.md), [BATTLE_TEST_AUTOMATION_M2B2_FEASIBILITY.md](BATTLE_TEST_AUTOMATION_M2B2_FEASIBILITY.md), [BATTLE_TEST_AUTOMATION_M2B2C_RUNNER_CORRECTION.md](BATTLE_TEST_AUTOMATION_M2B2C_RUNNER_CORRECTION.md), and [BATTLE_TEST_AUTOMATION_M2B2D_DEDICATED_CONTROL.md](BATTLE_TEST_AUTOMATION_M2B2D_DEDICATED_CONTROL.md).
