...
**Current boundary (2026-09-10):** The [test invocation compatibility correction](BATTLE_TEST_AUTOMATION_M4_FIELD_SPAWN_SMOKE.md#22-aggregate-test-invocation-compatibility-correction-2026-09-10) follows the passing event-tail cases and the first aggregate's 23/24 failure. Both runner-project invocation modes passed before publication. Clean aggregate/build verification is authorized as a subsequent gate with separately retained outcomes; a native run still requires its own approved plan. The recorded dispositions below remain historical, and Milestone 4 remains open.
