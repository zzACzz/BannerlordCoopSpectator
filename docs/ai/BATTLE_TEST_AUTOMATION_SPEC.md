...
Revision: **12 — live-proven dedicated control with UTC-safe client ownership and evidence-backed native-log policy**
Live-evidence source baseline: **`21042956b7928726dacb50c2de258437d5a65c6e`** (`2104295`)
...
Implementation status: **Milestone 2B.1 source/contracts, Milestone 2B.2A staging, exact prior dedicated loaded identity, bounded process ownership/cleanup, result suppression, and the Revision 10 ownership corrections are evidenced on 2026-08-31. Clean run `m2b2c-client-handoff-live-20260831-01` proved exact dedicated ownership and cleanup but disproved redirected standard handles as the authoritative readiness channel. Milestone 2B.2D implements the Revision 11 default-off structured dedicated-control channel in source: the exact native `InitialListedGameServerState.OnActivated` lifecycle event produces a run/token/hash/process-bound ready status; a fixed allowlisted request is claimed atomically; `GameNetwork.HandleConsoleCommand` applies the bootstrap from the main tick; native option/map/listed-server state produces exactly seven ordered acknowledgements. Published-revision run `m2b2d-prestage-contracts-20260901-01` passed 22/22, and `m2b2d-prestage-compile-20260901-01` built both modules with installed inventories unchanged. Controlled run `m2b2d-stage-20260901-01` installed dedicated SHA-256 `BD328AAC4F2A64C28D3EDCE28BCE3D72FF164BDAF817D9460B302ED538702A78` in both dedicated bins, retained the exact prior 218-file tree, and preserved the client and protected result. Published-revision run `m2b2d-live-feasibility-20260901-01` then confirmed the loaded dedicated hash, authoritative lifecycle readiness, all seven acknowledgements, native `start_game`, exact UDP ownership, real client launch, and loaded client hash. It stopped before lobby connection because the aggregate runner converted an already-deserialized UTC `DateTime` through a culture-dependent string and shifted it by the local offset. Revision 12 source now carries schema-v4 complete client identity, type-safe UTC normalization, retained pre-revalidation cleanup authority, and explicit optional-watchdog evidence; focused dual-PowerShell tests and canonical run `m2d-c-01` passed 22/22. Connection, campaign, cooperative mission, battle, L2, and L3 remain unproved until a clean published-revision rerun passes.** See [BATTLE_TEST_AUTOMATION_M2B1_RUNTIME_FOUNDATION.md](BATTLE_TEST_AUTOMATION_M2B1_RUNTIME_FOUNDATION.md), [BATTLE_TEST_AUTOMATION_M2B2_STAGING.md](BATTLE_TEST_AUTOMATION_M2B2_STAGING.md), [BATTLE_TEST_AUTOMATION_M2B2_FEASIBILITY.md](BATTLE_TEST_AUTOMATION_M2B2_FEASIBILITY.md), [BATTLE_TEST_AUTOMATION_M2B2C_RUNNER_CORRECTION.md](BATTLE_TEST_AUTOMATION_M2B2C_RUNNER_CORRECTION.md), and [BATTLE_TEST_AUTOMATION_M2B2D_DEDICATED_CONTROL.md](BATTLE_TEST_AUTOMATION_M2B2D_DEDICATED_CONTROL.md).
