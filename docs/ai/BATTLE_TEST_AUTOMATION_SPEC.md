...
Current M4 evidence addendum (2026-09-10): recovered `m4sd-p1-l1` proves loading the scoreboard-corrected dedicated candidate, but not a passing L2 run or complete automatic cleanup. The event-tail correction passed 16 focused cases. Clean aggregate `m4fe-p1-c1` then returned 23/24 because the test entry point rejected `--nologo`; this revision restores normal invocation compatibility and passed both runner-project and focused selections before publication. Subsequent clean aggregate/build outcomes are run-scoped evidence until M4 closure. Revision 34 requirements are unchanged. [Canonical regression, correction and validation boundary](BATTLE_TEST_AUTOMATION_M4_FIELD_SPAWN_SMOKE.md#22-aggregate-test-invocation-compatibility-correction-2026-09-10).
...
| 18 | Clean binaries staged and live role health/connection/cleanup confirmed; live artifact audit found one concatenated lock instead of six per-resource locks; exact production construction and cardinality contracts added before repeat live proof |
| 19 | Clean published 22/22 correction verification and repeat live run acquired/released six independent resources, repeated role health/connection/exact cleanup, and closed RUN-008 plus Milestone 2B without a campaign or battle claim |
| 20 | Default-off exact post-write campaign-roster recording, first-field admission, immutable metadata/hash/path validation, corruption/schema rejection, and 23/23 contracts implemented; no real fixture, oracle, sanitization, or reproduction claim yet |
...
| 26 | Clean ebf9cd3 passed fresh 24/24 contracts and both builds; first local-only live attempt reached seven bootstrap acknowledgements but failed before mission with a native readiness interface mismatch and a stalled failure handler; exact manual cleanup and full installed restoration passed, second attempt Not Run, L2 remains open |
| 27 | Exact ListedServer intermission readiness, fatal-helper classification, support-process cleanup grace, Windows PowerShell 5.1 hash-command initialization, and failure-finalization phase markers implemented and contract-verified; bounded failure diagnostics did not reproduce the prior stall or high memory, and native L2 remains open pending a fresh staged live run |
| 28 | Clean published c3ec15a passed fresh 24/24 contracts and both non-deploying builds; the first staged child loaded the exact corrected dedicated DLL but cancelled during server startup when the combined 10-second parent-liveness check rejected the still-running parent; automatic exact cleanup, non-fatal watchdog handling, full 218/32-file restoration, ports and locks passed; second child Not Run and L2 remains open |
| 29 | Parent admission now reports every manifest/lease/intent/heartbeat/process fact separately, tolerates only a transient unreadable shared snapshot within the existing ten-second last-verified window and a still-live exact cached parent identity, and writes one failure-only rejection artifact. Process-output capture now time-slices each stream so a burst cannot monopolize a controller heartbeat cycle. Synthetic high-output and admission cases passed in Windows PowerShell 5.1 and PowerShell 7; the canonical matrix passed 24/24 and both CompileOnly builds passed without installed mutation. No native rerun occurred and L2 remains open. |
| 30 | Clean published `6d38780` passed fresh 24/24 contracts and both CompileOnly builds. The controlled two-DLL transaction proved the parent-liveness correction in native startup and reached `MissionOpening`, then the dedicated process failed with Windows APPCRASH `0xc0000005`; the available WER report names no fault module and contains no dump, so the native root cause is unknown. The child then reproduced a live failure-evidence finalization stall with approximately 6.0 GiB private memory; fail-closed cancellation refused stale ownership, exact identity-checked manual runner termination was required, child terminal evidence remained incomplete, parent exited 40, and attempt 2 was Not Run. Full 218/32-file restoration, protected-result preservation, zero processes/ports, and eight exclusive lock probes passed. L2 remains open. |
| 31 | Full `Win32_Process` acquisition now runs only inside a short-lived isolated collector that projects five lightweight fields and is bounded by a 5-second deadline, 256-MiB private-memory limit, 4-MiB output limit, and 4,096-record limit. The collector is provisionally registered by exact PID/path/parent/launch window before it can block; timeout, memory, process, output and payload failures become structured evidence instead of blocking the runner. One capture is reused by descendant discovery, fatal-helper correlation and crash/hang publication; raw `CimInstance` objects and full records are not serialized into terminal evidence. Primary Crash/Timeout evidence is still published if optional descendant discovery fails, then exact cleanup continues. Focused tests passed in Windows PowerShell 5.1 and PowerShell 7, final `m4fe-c3` passed 24/24, and `m4fe-b2` built both modules without installed mutation or product launch. This is source/L1 only; native `0xc0000005`, dump capture and L2 remain open. |
| 32 | Preflight recovered the previously omitted full WER dump for exact dedicated PID 15968: `C:\dumps\DedicatedCustomServer.Starter.exe.15968.dmp`, 1,157,623,635 bytes, SHA-256 `E5348288E6FE61AAE7442AB32504E61E46E76DC2FA1526F46F56A6CF0E5CF124`. `dotnet-dump` identifies the only meaningful managed exception as `System.NullReferenceException` on OS thread `0x20f0` in the Harmony wrapper for `MissionScoreboardComponent.AfterStart`. Dynamic IL, the exact installed `TaleWorlds.MountAndBlade.dll`, and the live object agree: the method resolves `_mpGameModeBase` through `GetMissionBehavior<MissionMultiplayerGameModeBaseClient>()`, receives null, then dereferences `_mpGameModeBase.RoundComponent` because the process is server/recorder. The field battle-map server stack retains `MissionScoreboardComponent` but omits and explicitly removes `MissionMultiplayerCoopBattleClient`; the same full TdmClone server stack has the latent dependency, while hideout, siege, CoopTdm, hero-creator and campaign-map stacks already supply or avoid it. The existing WER configuration and actual dump prove capture availability. No registry, installed module, source or game process was changed, and no redundant native rerun occurred. A separately approved cross-mode source/contracts fix is required before the next live attempt. |
| 33 | Published correction `a6fe74f` adds a pure scoreboard dependency contract and a minimal server-only `MissionMultiplayerGameModeBaseClient` bridge. Direct CoopBattle inserts one bridge immediately before the first `MissionScoreboardComponent`; the full TdmClone server path enforces the same invariant, while scoreboard-free minimal TdmClone remains unchanged. Reusable hideout construction deliberately defers insertion so the already-safe day/night wrappers retain their established `MissionMultiplayerCoopBattleClient`; siege and other previously safe paths are unchanged. The bridge adds no client visuals, hot-path diagnostics, `AfterStart`, `OnClearScene`, or `OnRemoveBehavior` interception, so all native scoreboard lifecycle methods execute normally against a non-null dependency. Focused contracts passed, `m4sd-c1` passed 24/24, and `m4sd-b1` built both modules with 77/49 existing warnings, zero errors, no product process, and unchanged installed inventories. Candidate SHA-256 values are `A517CED31447A855D680945E979581080331AA7CA2F5C60E29DDA7D76C80C371` (client) and `74B03C00987F46AD76A6F21BAF807672FC699A402C3BE5EE5D52CB6CEC2BD4B7` (dedicated). This is source/L1 evidence only; staging, native lifecycle confirmation, materialization and L2 remain open. |
| 34 | Delivery is divided into one independently measurable defect/root cause per atomic task, with adjacent scenarios inspected for shared impact but distinct causes deferred. Full compliance and canonical-document audits occur at substage or milestone closure. Explicitly designated development/test installations may use `DeployPersistent`: the latest verified candidate remains installed, exact previous-state backup/restore is not required, and interrupted deployment is repaired by reapplying the complete intended artifact under the installation lock. Binary identity, atomic copy verification, cleanup, and non-module user-data protection remain mandatory. |

The companion audit remains the source-fact baseline. Revisions 2–34 refine implementation requirements and ordering without changing the audit's historical source findings. The narrow client launch/join slice, Milestone 2A non-runtime foundation, Milestone 2B runner-safety foundation, dedicated-control channel, Milestone 2B.3A safety closure, Milestone 3A exact campaign-roster recorder, Milestone 3B aggregate campaign-capture control surface, and Milestone 3C reviewed shareable fixture surface now exist. Their evidence boundaries are recorded in [BATTLE_TEST_AUTOMATION_CLIENT_JOIN_IMPLEMENTATION.md](BATTLE_TEST_AUTOMATION_CLIENT_JOIN_IMPLEMENTATION.md), [BATTLE_TEST_AUTOMATION_M2A_IMPLEMENTATION.md](BATTLE_TEST_AUTOMATION_M2A_IMPLEMENTATION.md), [BATTLE_TEST_AUTOMATION_M2B1_RUNTIME_FOUNDATION.md](BATTLE_TEST_AUTOMATION_M2B1_RUNTIME_FOUNDATION.md), [BATTLE_TEST_AUTOMATION_M2B2_FEASIBILITY.md](BATTLE_TEST_AUTOMATION_M2B2_FEASIBILITY.md), [BATTLE_TEST_AUTOMATION_M2B2C_RUNNER_CORRECTION.md](BATTLE_TEST_AUTOMATION_M2B2C_RUNNER_CORRECTION.md), [BATTLE_TEST_AUTOMATION_M2B2D_DEDICATED_CONTROL.md](BATTLE_TEST_AUTOMATION_M2B2D_DEDICATED_CONTROL.md), [BATTLE_TEST_AUTOMATION_M2B3_SAFETY_CLOSURE.md](BATTLE_TEST_AUTOMATION_M2B3_SAFETY_CLOSURE.md), and [BATTLE_TEST_AUTOMATION_M3_FIELD_FIXTURE.md](BATTLE_TEST_AUTOMATION_M3_FIELD_FIXTURE.md).
...
Clean revision `5d6a6c6` passed 22/22 contracts in `m2b3a-pub-c-01`; `m2b3a-pub-b-01` built client `A363...0075` and dedicated `2E14...1414` without product launch or installed mutation. Transaction `m2b3a-stage-01` retained complete pre-images and installed only the client DLL/PDB plus two dedicated DLLs. Clean run `m2b3a-live-r1-01` then live-confirmed exact loaded hashes, schema-2 heartbeat/progress, native login, terminal `Connected`, graceful exact cleanup, free ports, unchanged protected result, and no passing-path crash/hang artifact. Artifact audit found only one shared-lock record containing all six intended ids as a space-separated string. Cross-host reproduction proved PowerShell argument-expression construction, not the lock primitive, collapsed the production array. Revision 18 introduces an exact resource-id builder, explicit expected/acquired cardinality checks, and per-resource collision contracts. The live run remains valid for connection and role-health evidence but cannot close RUN-008 until the corrected script is clean-published and rerun.

Clean published revision `c6507e9` passed 22/22 contracts in `m2b3a-lockfix-pub-c-01`. Repeat run `m2b3a-live-r2-01` used that exact clean revision and the already staged binaries, returned `Pass`, and recorded six expected resource ids across six distinct lock paths followed by six verified releases. It repeated live role health, seven-step bootstrap, native login, terminal `Connected`, graceful exact cleanup, free ports, unchanged installed hashes and protected result, and passing-path crash/hang absence. RUN-008 and Milestone 2B are complete. The run opened no campaign, mission, or battle and makes no L2/L3 claim.
...
A selected subset MUST be reported as a subset and MUST NOT be described as the full contract suite. Relevant focused contracts MAY close an atomic implementation task when they satisfy TST-005; the full inventory remains the authority for substage, milestone, and release closure.
...
| `DeployPersistent` | Preferred direct-deployment mode for an explicitly designated development/test installation | Exclusive installation lock; exact target allowlist; atomic or fully verified copy; intended/staged/loaded hash chain; retain the successfully verified current candidate after the run; repair an interrupted deployment by reapplying the complete intended artifact |
| `DeployWithRestore` | Direct deployment to an installation whose prior module state must be preserved | Explicit opt-in, exclusive installation lock, exact pre-image backup, atomic/copy verification, and bounded restore with before/after hashes |
...
`DeployPersistent` MUST be enabled only for exact client and dedicated module roots that an environment profile or approved plan explicitly classifies as development-owned. A new or unknown machine remains protected by default. This classification applies only to the approved module paths; it does not authorize deletion or replacement of campaign saves, Documents data, Steam state, game configuration, unrelated modules, or other user data.

A successful `DeployPersistent` run MUST leave the verified intended candidate installed; restoring an older pre-run module version is neither required nor desirable. An interrupted or partially verified persistent deployment MUST mark the installation unavailable for further runtime runs, preserve the intended artifact and failure evidence, and repair the installation under the same exclusive lock by reapplying and verifying the complete intended artifact. It MUST NOT guess at or reconstruct an older state. Exact pre-image backup remains optional evidence for this mode, not a completion requirement.

### BLD-006 — End-to-end binary identity chain

The manifest MUST correlate the repository revision, compile output hash, staged/deployed hash, process-loaded assembly path, and hash reported by the loaded role. A mismatch is `EnvironmentBlocked`. A failed `DeployWithRestore` restoration or incomplete `DeployPersistent` repair is `RunnerInternalError`, preserves all recovery evidence, and blocks further runtime runs against that installation until explicit inspection and verified repair.
...
Primary product roles retain the normal 15-second graceful-close budget. Owned support roles that have no meaningful graceful-shutdown contract MAY use a shorter bounded budget; the current `RuntimeSupport` and `RuntimeFailureSupport` policy is one second before exact-identity forced termination. A shorter support budget MUST NOT weaken PID/path/start-time validation or be applied to the campaign host, dedicated server, or multiplayer client.
...
Ownership and fatal classification are separate decisions. A correlated `Watchdog.exe` remains an owned cleanup-support process but MUST NOT by itself promote a timeout to `Crash`; only an allowlisted fatal reporter such as the exact client/dedicated `CrashUploader.exe` or system `WerFault.exe` may do so. The same centralized policy MUST serve connection feasibility, field smoke, and later runtime scenarios.
...
The default fast command MAY select an impacted subset when a Git diff is available. Atomic-task and pre-commit verification MUST run the focused set required by TST-005 and report that it is a subset. Substage, milestone, release, and repository-wide completion verification MUST offer and execute the explicit full-contract option unless the approved closure criteria state why it is not applicable.
...
Implementation status: **Complete on 2026-09-02.** Clean published contracts `m2b3a-lockfix-pub-c-01` passed 22/22 from exact revision `c6507e9`. Repeat live run `m2b3a-live-r2-01` acquired and released the exact six independent shared resources, repeated exact loaded identities, schema-2 role health, seven-step bootstrap, native login, terminal `Connected`, result isolation, graceful exact cleanup, free ports, and passing-path crash/hang absence. Cancellation, simulated runner failure, RecoveryV2, and structured failure artifacts remain contract evidence; real fault injection is optional future evidence rather than a Milestone 2B blocker. No campaign, cooperative mission, battle, L2, or L3 evidence is claimed. See [BATTLE_TEST_AUTOMATION_M2B3_SAFETY_CLOSURE.md](BATTLE_TEST_AUTOMATION_M2B3_SAFETY_CLOSURE.md).
...
Implementation status on 2026-09-02: Milestone 3 is complete at the exact-fixture and contract-replay boundary. The 3A recorder and 3B campaign-capture control surface are clean-published, staged client-only, and live-proven at the exact pre-mission campaign-roster boundary. `m3b-live-capture-02` returned `Pass` with exact staged client SHA-256 `2A1E17E4...250FFB4`, private payload SHA-256 `ECF29661...073B9C7`, two populated sides, infantry, mounted, and hero/captain groups, unchanged protected result, and zero remaining product processes/port listeners. Milestone 3C independently audited that payload, kept the raw bytes and broader private run root outside Git, produced a byte-identical cross-PowerShell sanitized derivative with SHA-256 `B47D7AF7...579D49D`, added linked metadata plus a separate critical-value oracle, and passed hash-gated production-schema qualification and mutation rejection. Final canonical `m3c-sanitized-contracts-02` and `m3c-sanitized-compile-01` passed 23/23 and both compile-only projects with installed inventories unchanged. No mission, multiplayer connection, full battle, L2, or L3 claim exists. See [BATTLE_TEST_AUTOMATION_M3_FIELD_FIXTURE.md](BATTLE_TEST_AUTOMATION_M3_FIELD_FIXTURE.md).
...
- replay input bytes hash-match either the exact private capture or the explicitly linked reviewed derivative selected by the run contract, and provenance pins both hashes when a derivative is used;
...
**Current boundary (2026-09-10):** Published `ab80784` passed clean 24/24 contracts and both CompileOnly builds. The subsequent [zero-client startup diagnosis](BATTLE_TEST_AUTOMATION_M4_FIELD_SPAWN_SMOKE.md#23-zero-client-startup-contract-diagnosis-2026-09-10) established independent mode/clock/peer conditions incompatible with the prior field path. The separately approved [mode initializer](BATTLE_TEST_AUTOMATION_M4_FIELD_SPAWN_SMOKE.md#24-field-automation-native-mode-initialization-2026-09-10) and [initial clock/peer admission](BATTLE_TEST_AUTOMATION_M4_FIELD_SPAWN_SMOKE.md#25-zero-client-clock-and-initial-peer-admission-2026-09-10) address those source boundaries in the working tree. The narrow exception is authorized only for the admitted zero-client field mission through `PreBattleHold`, under the [canonical runtime conditions](RUNTIME_FLOWS.md#field-automation-zero-client-pre-battle-admission-2026-09-10-source-contract). Production/client/active-battle peer gates, real pause, exact fixture assertions, native-only spawning and result suppression remain required. Fake participants, forced battle phases and fixture relaxation remain forbidden. The diagnosis alone did not authorize this exception. No new Bannerlord run occurred; historical clock inputs remain unverified, the dispositions below remain historical, and Milestone 4 remains open.
...
**4B correction disposition (2026-09-09):** The field-only readiness observer now resolves the exact installed `TaleWorlds.MountAndBlade.ListedServer.IIntermissionState` contract while retaining the native `IsNewTaskAssignable` idle guard and one-shot command ownership. Shared failure handling now separates owned `Watchdog.exe` cleanup from fatal `CrashUploader.exe`/`WerFault.exe` classification, applies a one-second grace only to support roles, explicitly initializes `Get-FileHash` for the supported Windows PowerShell 5.1 host, and emits failure-only evidence/cleanup phase markers. Bounded synthetic diagnostics passed in PowerShell 7.6.5 and Windows PowerShell 5.1 with failure-evidence calls below 0.4 seconds and peak private memory below 200 MiB; the prior live stall and greater-than-4.6-GiB observation were not reproduced and therefore are not claimed fixed by reproduction. Focused cross-shell contracts passed, `m4b-readiness-runner-contracts-20260909-01` passed 24/24, and `m4b-readiness-runner-compile-20260909-01` built both modules with installed inventories unchanged and no product process. This closes only the source/L1 correction gate. Staging, native rerun, mission materialization, automatic live-failure cleanup, and L2 remain pending. See [the correction audit](BATTLE_TEST_AUTOMATION_M4_FIELD_SPAWN_SMOKE.md#13-bounded-failure-diagnostic-and-sourcecontract-correction-2026-09-09).

**4B clean-rerun disposition (2026-09-09):** Clean published revision `c3ec15a` passed `m4r27-c-01` at 24/24 and `m4r27-b-01` with both builds, no product launch and unchanged installed inventories. The controlled transaction staged only dedicated DLL SHA-256 `0F7FA25AED9C6C7F2D4B3C250D73991B26EAA29189C5703FE753CB12C0D85382`. Child `m4r27-live-01-01` loaded that exact identity but cancelled before module/control readiness because the repeated combined parent-liveness admission rejected the still-running parent after initial admission had succeeded. The check combines file readability, immutable run/nonce/PID identities, lease status, and a hard 10-second heartbeat age without recording the failed subcondition; the retained evidence does not distinguish a stale heartbeat from a transient shared-read failure, so neither is asserted as the exact cause. The child automatically stopped its exact dedicated role, captured native logs, left no process/port, preserved its run-local result, classified no fatal helper, and released all locks. The outer transaction restored all 218 dedicated files and preserved all 32 client files; eight independent lock probes passed. The second child was Not Run. This is runner cancellation evidence, not a field readiness, mission, automatic Crash/Timeout-finalization, or L2 result. A separately approved source/contract stage must instrument and correct the parent/child liveness contract without removing orphan protection or merely hiding the failure with an arbitrary delay. See [the clean-rerun audit](BATTLE_TEST_AUTOMATION_M4_FIELD_SPAWN_SMOKE.md#14-clean-m4-l2-rerun-parentliveness-cancellation-2026-09-09).

**4B parent-liveness source/contract disposition (2026-09-09):** The shared runner core now returns a structured `coop-spawn-smoke-parent-admission-v1` result with distinct manifest, lease, intent, nonce, RunId, attempt, PID, status, heartbeat-timeline and heartbeat-freshness facts. The child retains the exact initial parent role and may survive an unreadable shared snapshot only within the existing ten-second last-accepted window while that exact cached process identity remains live; immutable mismatches, a stale/invalid heartbeat, a non-Active lease and process-identity loss still fail closed. The pure `coop-spawn-smoke-parent-read-fallback-v1` decision records whether that narrow fallback was allowed, its measured age/deadline and exact rejection code. Rejection writes that decision inside one `coop-spawn-smoke-parent-rejection-v1` artifact and attaches the effective failure code to the exception only on the failure path. `Update-CoopProcessTextCapture` now applies a 100 ms per-stream processing slice, and the public parent uses it explicitly, so a large stdout/stderr burst cannot synchronously consume an unbounded heartbeat interval. No timeout was increased and no sleep was added. The focused harness directly passed the five-second allowed, eleven-second expired and exact-process-loss fallback cases, retained all 4,000 stdout plus 4,000 stderr lines, and passed in Windows PowerShell 5.1 and PowerShell 7. Final short-path run `m4pl-c3-01` passed 24/24 with `contracts.json` SHA-256 `4865EB701D1621DBEBF442C3D23A6D9FB091141616476D650D1EDFFC5E35CB3A`. `m4pl-b3-01` built both modules with 77/49 warnings, zero errors, no product process, byte-identical installed inventories SHA-256 `35D7049344D13EDCBFEAE2D3E389880AD7EBFA8464EE60AA8A8B83B96AEFBC86`, client SHA-256 `5FC9028AAD2BB2318E29A03018F761B6A5D136E2E5171110E768D3BB55D3821F`, and dedicated SHA-256 `0F7FA25AED9C6C7F2D4B3C250D73991B26EAA29189C5703FE753CB12C0D85382`. A preceding long-RunId matrix was rejected as evidence after one executable path exceeded the Windows limit; it drove no code change. This closes only the source/L1 correction gate. The exact prior live trigger remains unproven, and publication, staging, native rerun, readiness, mission and L2 remain separate.

**4B liveness-corrected live disposition (2026-09-09):** Clean published revision `6d38780af05975b76c27af914ad4e0d66b057ae4` passed `m4pl-p1-c1` at 24/24 and `m4pl-p1-b1` with both CompileOnly builds and unchanged installed inventories. Transaction `m4pl-p1-s1` staged only dedicated DLL SHA-256 `0F7FA25AED9C6C7F2D4B3C250D73991B26EAA29189C5703FE753CB12C0D85382`; child `m4pl-p1-l1-01` produced no parent-rejection artifact, reached control readiness, completed all seven bootstrap acknowledgements, confirmed `start_game`, issued exactly one `start_mission`, and advanced to `MissionOpening`. The dedicated process then generated Windows Application Error/Windows Error Reporting evidence for APPCRASH exception `0xc0000005`; the fault module is `unknown`, the archived report contains no dump, and no exact native root cause is claimed. During failure finalization the child emitted `FailureEvidenceCaptureStarted` but no completion event or crash/hang artifact, consumed approximately 431.8 CPU seconds and 6.0 GiB private memory, and stopped servicing its lease. Cooperative cancellation correctly refused uncertain stale ownership; after exact manifest/PID/start-time/path/hash/parent validation, manual force termination stopped only the child Runner. Parent outcome was `RunnerInternalError` with exit 40, child terminal evidence is incomplete, attempt 2 was Not Run, and no L2 pass is claimed. The transaction restored both original installed DLLs and all inventories; independent postflight found 218/218 dedicated and 32/32 client files unchanged, no product/runner process or required port owner, eight exclusively open locks, and the protected result byte-identical. See [the clean liveness-corrected live audit](BATTLE_TEST_AUTOMATION_M4_FIELD_SPAWN_SMOKE.md#16-clean-published-liveness-corrected-rerun-native-mission-opening-crash-and-failure-evidence-stall-2026-09-09).

**Revision 30 required correction order (the source/contracts step is completed below):** First, a separately approved shared runner source/contract stage MUST make failure-evidence finalization bounded and MUST NOT retain or serialize a full heavyweight live `CimInstance` snapshot. It MUST inspect every consumer, including Feasibility and dedicated spawn smoke, preserve non-fatal watchdog handling and exact WER/crash-uploader correlation, keep cancellation and lease servicing responsive, avoid high-memory growth, and guarantee terminal artifacts plus exact cleanup. Only after that correction is clean-published may a fresh staged live rerun enable or otherwise make available dump capture to localize the native `0xc0000005` at `MissionOpening`. Do not change battle adapters, relax fixture/identity gates, or retry the native run blindly. Any implementation requires a separate approved plan.

**4B bounded failure-evidence source/contract disposition (2026-09-09):** Published revision `c100cb8` moves every full `Win32_Process` acquisition used by descendant discovery, fatal-helper correlation and runtime failure publication into `Get-CoopBoundedProcessSnapshotCore`. Its isolated child projects only PID, parent PID, executable path, command line and creation time, then exits; the runner enforces 5 seconds, 256 MiB private memory, 4 MiB output and 4,096 records. `Get-CoopBoundedRuntimeProcessSnapshot` provisionally records the collector's exact PID, shell path, parent PID and launch window before waiting, so recovery retains ownership if the runner itself terminates. The same bounded result is reused throughout finalization. Failed optional collection is recorded in terminal crash/hang evidence without raw records; the primary Crash/Timeout remains eligible for publication even if descendant discovery supersedes the runner outcome, and cleanup follows. Watchdog remains non-fatal and WER/CrashUploader path/PID correlation is unchanged. Focused success/timeout/memory/failure/live-snapshot and failed-collector publication tests passed in Windows PowerShell 5.1 and PowerShell 7. Final `m4fe-c3` passed 24/24 with `contracts.json` SHA-256 `67C32070A89CDFF6144C96D59B12787254B2FCEC2E6484902FC9B80C8EEC96A9`. `m4fe-b2` built both modules with 77/49 warnings, zero errors, no product process, and byte-identical installed inventories SHA-256 `B817EEA278FFFC398A61E0FD92C65EB07314E0A7474D11FC417492C39B8B1E6B`; its compile report SHA-256 is `73994167DA9C96B14915399A6A5CF8E37D909D12B65141B9E7BB27349A7A7E05`. This closes the source/L1 failure-finalization gate only. No game process or live failure exercised the new boundary; native `0xc0000005`, dump capture, materialization, attempt 2 and L2 remain open. See [the source/contract closure](BATTLE_TEST_AUTOMATION_M4_FIELD_SPAWN_SMOKE.md#17-bounded-failure-evidence-source-and-contract-closure-2026-09-09).

**4C recovered-dump diagnosis (2026-09-09):** The previously omitted full WER dump for exact PID 15968 was present in `C:\dumps` and is correlated by PID and creation time to `m4pl-p1-l1-01`. Its sole meaningful managed exception is a `System.NullReferenceException` in `MissionScoreboardComponent.AfterStart`: `_mpGameModeBase` is null immediately before the unguarded `RoundComponent` access. This matches the exact installed engine method and the field server behavior list, which retains the scoreboard while excluding the required `MissionMultiplayerGameModeBaseClient`. The external APPCRASH `0xc0000005` is therefore no longer the lowest known failure boundary. The planned unchanged rerun was deliberately Not Run because it would reproduce a now-localized defect without testing a correction. No WER setting, installed file, source file or product process changed. The next implementation MUST enforce the scoreboard/game-mode-client dependency across every construction path, include the latent full TdmClone server path, preserve already-safe hideout/siege/other mode ordering, and test removal/cleanup lifecycle behavior before another native attempt. See [the recovered-dump diagnosis](BATTLE_TEST_AUTOMATION_M4_FIELD_SPAWN_SMOKE.md#18-recovered-full-dump-and-exact-managed-failure-diagnosis-2026-09-09).

**4D scoreboard dependency source/contract disposition (2026-09-09):** Published revision `a6fe74f` implements the required pure dependency decision, minimal server bridge, ordered insertion and fail-closed assertion for direct CoopBattle and full TdmClone server stacks. The existing hideout wrappers retain their own established bridge; siege, CoopTdm, hero-creator, campaign-map and scoreboard-free minimal TdmClone paths remain unchanged. No Harmony suppression was added: native scoreboard `AfterStart`, `OnClearScene`, and `OnRemoveBehavior` remain executable with the required non-null base-client dependency. Focused contracts passed, `m4sd-c1` passed 24/24 (`contracts.json` SHA-256 `22C5CA7630C3453DBF203F46538044EFA1BD6326E6EF43DA238AC18961E2A557`), and `m4sd-b1` built client `A517CED3...80C371` and dedicated `74B03C00...2BD4B7` with 77/49 warnings, zero errors, no product process and unchanged installed inventories. No module was staged and no native process ran. The next stage requires separate approval for controlled staging and one bounded live attempt; L2 remains open. See [the source/L1 closure](BATTLE_TEST_AUTOMATION_M4_FIELD_SPAWN_SMOKE.md#19-scoreboard-server-dependency-source-and-contract-closure-2026-09-09).
...
| Process orchestration | `scripts/Invoke-CoopTest.ps1`; `scripts/Start-CoopBattleTestClient.ps1`; `scripts/Start-CoopFieldFixtureCapture.ps1`; `run_battle_test_client.bat`; dedicated helper and command classes; `scripts/CoopDevLoop.ps1` is reference only, not the runner base |
