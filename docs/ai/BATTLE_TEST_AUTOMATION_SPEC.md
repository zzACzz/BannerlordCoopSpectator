...
Revision: **15 — native server selection/join live-proven; authoritative GameNetwork handoff observation corrected**
Live-evidence repository baseline: **`9c3930b18c89b274d48dbb7f739fe054a8090dc6`** (`9c3930b`)
Current source corrections: **`d1af692`**, **`3fdcda3`**
...
Implementation status: **Milestone 2B.1 source/contracts, controlled staging, exact dedicated/client loaded identity, bounded ownership/cleanup, result suppression, authoritative dedicated control, native `start_game`, exact UDP ownership, schema-v4 client handoff, corrected `NetworkMain` resolution, one-shot native platform login, exact server selection, and native custom-game join are live-evidenced. Clean run `m2e1-live-r1-01` reached the run-owned server, executed the real `GameNetwork.StartMultiplayerOnClient(...)` path, entered `InCustomGame`, logged native join success and `CreatePlayer`, and visibly waited for a mission. The formal controller status remained non-terminal `JoinAccepted`, so the bounded runner correctly returned `Timeout`. Installed-runtime IL inspection traced this evidence gap to a notification owned only by an absent historical lobby signature. Published source corrections `d1af692` and `3fdcda3` move handoff observation to the actual GameNetwork boundary and retain last-valid timeout status evidence. Focused tests, full 22/22 contracts, and isolated client/dedicated compilation passed; installed trees remained unchanged. The changed client DLL now requires controlled staging. Formal `NetworkHandoff`/`Connected`, campaign, cooperative mission, battle, L2, and L3 remain unproved until the staged correction passes a clean live rerun.** See [BATTLE_TEST_AUTOMATION_M2B1_RUNTIME_FOUNDATION.md](BATTLE_TEST_AUTOMATION_M2B1_RUNTIME_FOUNDATION.md), [BATTLE_TEST_AUTOMATION_M2B2_STAGING.md](BATTLE_TEST_AUTOMATION_M2B2_STAGING.md), [BATTLE_TEST_AUTOMATION_M2B2_FEASIBILITY.md](BATTLE_TEST_AUTOMATION_M2B2_FEASIBILITY.md), [BATTLE_TEST_AUTOMATION_M2B2C_RUNNER_CORRECTION.md](BATTLE_TEST_AUTOMATION_M2B2C_RUNNER_CORRECTION.md), and [BATTLE_TEST_AUTOMATION_M2B2D_DEDICATED_CONTROL.md](BATTLE_TEST_AUTOMATION_M2B2D_DEDICATED_CONTROL.md).
