...
Revision: **21 — Milestone 3B exact campaign-capture runner and launcher contracts implemented; clean staging and runtime fixture still pending**
...
Implementation status on 2026-09-02: the 3A recorder and 3B campaign-capture control surface are complete in source/contracts. The recorder retains exact post-write `battle_roster.json` bytes only under the explicit run-scoped fixture profile, admits only the ordinary mixed SCN-001 field slice, preserves immutable metadata/hash/provenance, and rejects corruption, schema mismatch, and path escape. The aggregate `Record` command now verifies clean local/upstream identity, explicit installed hash, shared runtime locks, exact installed-1.4.8 `/singleplayer` module arguments, runner-owned campaign process identity, bounded status/cleanup, protected-result isolation, and a nonce/credential-free reproduction descriptor. Runs `m3b-contracts-01` and `m3b-compile-01` passed 23/23 plus both compile-only projects with installed inventories unchanged; `m3b-dirty-block-01` proved dirty-source rejection without product launch. No real Bannerlord fixture has been captured, independently reviewed, sanitized, committed, or replayed, and no L2/L3 claim exists. Clean publication, client-only staging, and live capture remain pending. See [BATTLE_TEST_AUTOMATION_M3_FIELD_FIXTURE.md](BATTLE_TEST_AUTOMATION_M3_FIELD_FIXTURE.md).
