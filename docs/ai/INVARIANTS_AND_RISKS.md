...
Scoreboard dependency invariant (2026-09-09, diagnosed/pending implementation): every mission stack containing `MissionScoreboardComponent` while `GameNetwork.IsServerOrRecorder` can be true MUST also contain a valid `MissionMultiplayerGameModeBaseClient` before `AfterStart`, or MUST remove the complete scoreboard contract only when all dependent server components and clear/remove lifecycle calls remain safe. Guarding or swallowing only the observed `AfterStart` exception is insufficient because the exact engine class also dereferences `_mpGameModeBase.RoundComponent` in `OnRemoveBehavior` and `OnClearScene`. The recovered PID-15968 full dump proves the current CoopBattle battle-map server violates this invariant; full TdmClone server has the latent same dependency. Any correction MUST audit all behavior factories and scenario wrappers, preserve already-safe hideout/siege ordering, and add default-off/no-hot-path contract evidence before a native rerun. [Dump proof and cross-mode audit](BATTLE_TEST_AUTOMATION_M4_FIELD_SPAWN_SMOKE.md#18-recovered-full-dump-and-exact-managed-failure-diagnosis-2026-09-09).
