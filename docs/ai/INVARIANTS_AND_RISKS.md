...
Last dedicated/client-control runtime verification: **2026-09-01** (`m2e1-live-r1-01`; exact selection, native join, `InCustomGame`, `Join game successful`, `CreatePlayer`, and visible `Awaiting Server` confirmed; formal Connected rerun pending)
...
- The default-off normal-lobby automation path has real client-launch, loaded-hash, exact `NetworkMain` resolution, successful one-shot native login, `AtLobby`, exact server selection, native join acceptance, actual GameNetwork client start, `InCustomGame`, server `CreatePlayer`, client logs, and cleanup evidence. Formal status remained `JoinAccepted` because the then-loaded client observed handoff only through an absent historical lobby signature. Corrections `d1af692` and `3fdcda3` are source/contract/compile verified, and controlled transaction `m2e1-handoff-stage-r1` installed clean published client `7CC2D759...2977E97` while retaining the complete pre-image. Formal `NetworkHandoff` and `Connected` remain runtime-unverified until a clean rerun confirms the new loaded hash.
