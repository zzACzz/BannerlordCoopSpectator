...
Last automation-control source/contract verification: **2026-09-02** (`m2b3a-lockfix-c-01`; protocol 1.1 role health, cancellation, RecoveryV2, failure evidence, exact resource-id construction/cardinality, and per-resource lock collision/release; 22/22 dirty-source verification)
...
Last dedicated/client-control runtime verification: **2026-09-02** (`m2b3a-live-r1-01`; exact loaded identities, live role health, terminal `Connected`, exact cleanup, protected-state preservation, and passing-path crash/hang absence confirmed; per-resource shared locks remain pending repeat live proof)
...
- Milestone 2B.3A implements the previously missing shared runner controls: schema-2 role heartbeat/progress/state-revision/error projection, distinct `NoHeartbeat`/`NoProgress`, canonical shared locks, real console and explicit cancellation contracts, RecoveryV2, and exact owned crash/modal/hang evidence paths. Clean run `m2b3a-live-r1-01` confirms role health, identity, connection, cleanup, result isolation, and passing-path crash/hang absence. Its lock artifact contained one concatenated id instead of six independent resources; therefore RUN-008 remains open. The runner-only source correction builds ids independently and validates expected/acquired cardinality, but until a clean published repeat `Feasibility` run records and releases six locks, treat any combined/missing lock artifact or uncorrelated helper process as a blocker rather than falling back to old behavior.
