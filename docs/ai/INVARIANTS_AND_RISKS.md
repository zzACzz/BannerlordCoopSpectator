...
- The default-off normal-lobby automation path has real client-launch, loaded-hash, exact `NetworkMain` resolution, successful one-shot native login, `AtLobby`, exact server selection, native join acceptance, actual GameNetwork client start, `InCustomGame`, server `CreatePlayer`, client logs, and cleanup evidence. Formal status remained `JoinAccepted` because the staged client observed handoff only through an absent historical lobby signature. Corrections `d1af692` and `3fdcda3` are source/contract/compile verified, but the changed client is not installed; formal `NetworkHandoff` and `Connected` remain runtime-unverified until controlled staging and a clean rerun.
