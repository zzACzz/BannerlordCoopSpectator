...
Last automation-control source/contract verification: **2026-09-02** (Milestone 2B.3A protocol 1.1 role health, shared locks, cancellation, RecoveryV2, and failure evidence; `m2b3a-final-c-01`, 22/22 with verified runner-lock release; dirty source verification)
Last controlled client staging verification: **2026-09-01** (`m2e1-handoff-pub-compile-20260901-01`, `m2e1-handoff-stage-r1`; clean `a7bd528` client `7CC2D759...2977E97` installed with complete pre-image; no product process launched)
Last dedicated/client-control runtime verification: **2026-09-01** (`m2e1-handoff-live-r1-01`; exact loaded identities, selection, native join, post-rewrite `NetworkHandoff`, terminal `Connected`, native success, `CreatePlayer`, exact cleanup, and protected-state preservation confirmed)
...
- Milestone 2B.3A implements the previously missing shared runner controls: schema-2 role heartbeat/progress/state-revision/error projection, distinct `NoHeartbeat`/`NoProgress`, canonical shared locks, real console and explicit cancellation contracts, RecoveryV2, and exact owned crash/modal/hang evidence paths. These controls are source/contract verified but not yet staged or Bannerlord-runtime confirmed. Until a clean published live `Feasibility` run proves them, treat missing heartbeat/lock-release artifacts or an uncorrelated helper process as a blocker rather than falling back to old timeout/name-only behavior.
