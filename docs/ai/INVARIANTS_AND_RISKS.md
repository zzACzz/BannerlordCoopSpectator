...
Last exact-fixture launcher/source verification: **2026-09-02** (`m3b-contracts-01`, 23/23; `m3b-compile-01`, both projects passed; dirty-source launch rejected; exact singleplayer arguments, ownership, private-artifact, cleanup, and reproduction contracts passed; clean staging/runtime fixture pending)
...
The remaining high-risk boundary is user state, not the recorder hot path: Bannerlord can write normal configuration/log/save data after the user loads or starts a campaign. The launcher itself edits no save and the live procedure must use a purpose-made copy. The raw fixture and metadata can contain campaign identifiers and remain private outside Git until independent content/privacy review. A valid payload hash and qualification count still prove only the pre-mission file boundary; they do not prove mission loading, multiplayer connection, natural battle completion, writeback, or L2/L3.
