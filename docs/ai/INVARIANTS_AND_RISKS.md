...
Current field verification boundary (2026-09-10): `m4zclive-l1-01` returned `Timeout/NoHeartbeat` before control readiness/bootstrap although the later retained role-status snapshot has a recent heartbeat. That historical run did not retain the failed read/decision; do not assert a dead server, a proven false alarm, a mission failure or a dependency-load root cause. The subsequent source/contract correction retains the rejected observation separately from later finalization snapshots; it does not recover historical data or establish native progression. Automatic bounded timeout finalization, graceful exact cleanup, full temporary-install restoration and preservation of all five dumps passed for the historical run. Native-crash handling and mission progression remain unverified. [Live evidence](BATTLE_TEST_AUTOMATION_M4_FIELD_SPAWN_SMOKE.md#26-clean-zero-client-live-attempt-stops-before-bootstrap-2026-09-10), [focused correction](BATTLE_TEST_AUTOMATION_M4_FIELD_SPAWN_SMOKE.md#27-exact-role-health-rejection-source-and-contract-correction-2026-09-10).

Role-health evidence invariant (2026-09-10): preserve the rejected read and its classifier UTC before finalization; a later readable/fresh status MUST NOT replace that observation or justify retroactive acceptance. Missing/not-visible, read/parse failure and a stale timestamp remain distinct evidence even when their existing outcome is `NoHeartbeat`. Keep identity/capability/schema rejection and the five-second heartbeat/180-second progress defaults unchanged. Diagnostic projection must remain bounded, detached and failure-only, and its own failure MUST NOT supersede the primary rejection. [Canonical field contract, activation, cost and limitations](BUILD_TEST_DEBUG.md#exact-role-health-rejection-evidence-2026-09-10).

Scoreboard dependency invariant (2026-09-09, implemented at source/L1 in `a6fe74f`): every mission stack containing `MissionScoreboardComponent` while `GameNetwork.IsServerOrRecorder` can be true MUST also contain a valid `MissionMultiplayerGameModeBaseClient` before `AfterStart`, or MUST remove the complete scoreboard contract only when all dependent server components and clear/remove lifecycle calls remain safe. Direct CoopBattle and full TdmClone now use a pure decision plus one minimal server bridge inserted immediately before the scoreboard, followed by a fail-closed assertion. Existing game-mode client dependencies suppress insertion, and scoreboard-free stacks do nothing. Reusable hideout construction retains its established wrapper bridge; siege and other safe modes remain unchanged. Never guard or swallow only `AfterStart`: the exact engine class also dereferences `_mpGameModeBase.RoundComponent` in `OnRemoveBehavior` and `OnClearScene`. No default-on hot-path diagnostic or lifecycle Harmony suppression is permitted. Focused tests, 24/24 and both CompileOnly builds passed; native lifecycle regression and L2 remain unverified. [Implementation, evidence and next gate](BATTLE_TEST_AUTOMATION_M4_FIELD_SPAWN_SMOKE.md#19-scoreboard-server-dependency-source-and-contract-closure-2026-09-09).

Failure-finalization invariant (2026-09-09, source/L1): a full live `CimInstance` process set MUST NOT exist inside the Runner or enter terminal JSON. Process acquisition MUST use the exact provisionally owned isolated collector, project only the five required scalar fields, and enforce total time, private-memory, output-size and record-count limits. One validated lightweight capture SHOULD be reused for descendant discovery, fatal-helper correlation and crash/hang publication. Optional collector failure MUST remain structured, MUST NOT erase the primary Crash/Timeout artifact, and MUST NOT prevent exact cleanup; destination write failure remains separately fatal. CrashUploader/WerFault require exact path plus owned-tree or command-line PID correlation; Watchdog remains non-fatal. These rules affect shared Feasibility, DedicatedSpawnSmoke and Record cleanup, not any battle adapter. Dual-shell focused contracts, 24/24 and both CompileOnly builds passed; real live failure finalization remains unverified and native `0xc0000005`/L2 remain open. [Implementation and evidence](BATTLE_TEST_AUTOMATION_M4_FIELD_SPAWN_SMOKE.md#17-bounded-failure-evidence-source-and-contract-closure-2026-09-09).

Latest native field boundary (2026-09-09): clean `m4pl-p1-l1-01` proves the corrected parent-liveness contract through control readiness, seven acknowledgements, confirmed `start_game`, one `start_mission`, and `MissionOpening`. It then proves only a dedicated APPCRASH `0xc0000005`; no dump or fault module exists, so no native root cause may be asserted. The subsequent shared failure-evidence finalizer stalled at approximately 6.0 GiB private memory, stopped servicing its lease, and required exact identity-checked manual Runner termination; child terminal evidence is incomplete, attempt 2 was Not Run, and L2 remains open. Before another native attempt, bounded failure finalization MUST be established across Feasibility and all shared battle paths: do not retain/serialize a full heavyweight live `CimInstance`, keep watchdog handling non-fatal, preserve exact WER/CrashUploader correlation, service cancellation/lease, bound memory/time, and guarantee terminal artifacts plus cleanup. Only then enable or verify dump capture for one fresh rerun. Full 218/32-file restoration, protected result, processes, ports and 8/8 lock probes passed. [Canonical evidence](BATTLE_TEST_AUTOMATION_M4_FIELD_SPAWN_SMOKE.md#16-clean-published-liveness-corrected-rerun-native-mission-opening-crash-and-failure-evidence-stall-2026-09-09).

Previous live field boundary (2026-09-09): the first attempt exposed an opening-readiness namespace mismatch and a stalled child failure path; original installations/resources were restored, but no mission or L2 pass was reached. The readiness literal is now corrected to the exact ListedServer interface while retaining the native idle guard and one-shot mission command. Shared failure handling now separates fatal reporters from cleanup-owned Watchdog, shortens only support-role cleanup, initializes the Windows PowerShell 5.1 hash command, and records failure-evidence/cleanup phases. At that stage bounded diagnostics did not reproduce the prior stall or high-memory observation, so automatic live-failure cleanup and native L2 remained unverified. [Failure audit and correction boundary](BATTLE_TEST_AUTOMATION_M4_FIELD_SPAWN_SMOKE.md#13-bounded-failure-diagnostic-and-sourcecontract-correction-2026-09-09).

Previous clean field blocker (2026-09-09): `m4r27-live-01-01` loaded the corrected exact dedicated DLL but cancelled before readiness when `Assert-CoopSpawnSmokeParent` rejected a repeated liveness check. Initial admission and immutable parent/child run, nonce and PID bindings succeeded; the combined rejection did not retain its failed subcondition. Preserve orphan detection and exact binding, but require per-fact rejection evidence and a measured freshness contract that tolerates supported startup scheduling. Do not remove the guard, add a blind sleep, extend mission timeouts, or infer a readiness/battle defect from this earlier orchestration failure. Automatic cleanup, watchdog non-fatal semantics, full restoration, ports and locks passed. [Clean rerun audit](BATTLE_TEST_AUTOMATION_M4_FIELD_SPAWN_SMOKE.md#14-clean-m4-l2-rerun-parentliveness-cancellation-2026-09-09).

Parent-liveness correction invariant (2026-09-09): output capture MUST remain time-sliced and lossless at completion; a high-volume child MUST NOT monopolize the parent lease-renewal loop. Parent admission MUST retain distinct manifest/lease/intent/heartbeat/process facts. A transient unreadable snapshot MAY use the last exact acceptance only through the pure bounded fallback classifier, within the configured heartbeat window and only while the cached exact process identity remains live. Identity mismatch, non-Active status, invalid/stale heartbeat, expired read grace and process loss MUST fail closed and SHOULD retain the failure-only rejection artifact, including the fallback decision when evaluated. Never increase the mission timeout, remove the heartbeat, accept a PID without start-time identity, or treat source/L1 verification as native L2. [Correction evidence](BATTLE_TEST_AUTOMATION_M4_FIELD_SPAWN_SMOKE.md#15-parentliveness-source-and-contract-correction-2026-09-09).
...
Milestone 4A's native-agent observer is retained only under explicit default-off FieldDedicatedSpawnSmokeV1. Native reads remain potentially runtime-sensitive. Never retry start_mission, relax the pinned fixture/hero identities, bypass production/active-battle peer gates or claim same-process reset from two fresh processes. The separately approved 2026-09-10 exception permits only the bound zero-client field mission's initial clock/peer admission through `PreBattleHold`; its canonical conditions and lifecycle are in [RUNTIME_FLOWS.md](RUNTIME_FLOWS.md#field-automation-zero-client-pre-battle-admission-2026-09-10-source-contract). Preserve real pause, exact mission/snapshot identity, native-only spawning and result suppression. Unknown state, clients, failure, mission replacement/end or active battle must not receive the exception. The targeted native IL corridor is version-sensitive: fail installation before native commands when it does not match, and remove only its dedicated Harmony owner. Native progression and cleanup remain unverified. See [M4 admission, isolation, cleanup and verification boundaries](BATTLE_TEST_AUTOMATION_M4_FIELD_SPAWN_SMOKE.md).

Last source verification: **2026-09-09** (`a6fe74f`; `m4sd-c1` 24/24; `m4sd-b1` client/dedicated CompileOnly passed with installed inventories unchanged)
Last automation-control source/contract verification: **2026-09-09** (`m4b-readiness-runner-contracts-20260909-01`; exact fatal-helper classification, role-specific cleanup grace, Windows PowerShell 5.1 hash initialization, failure-phase markers, and field readiness; 24/24)
...
`NetworkHandoff` evidence must originate from the actual `GameNetwork.StartMultiplayerOnClient(string,int,int,int)` patch after the final one-shot address rewrite. Optional version-specific lobby reflection patches must not be the sole observer and must not emit a duplicate notification. `Connected` additionally requires both `GameNetwork.IsClient` and `GameNetwork.IsSessionActive`; native join success alone must not fabricate the terminal acknowledgement.
...
The Milestone 2B.2D source path enforces this invariant: the external runner owns one fixed allowlisted command intent, the variable server-name field accepts only ASCII letters, digits, dot, underscore, or hyphen, and the exact dedicated role publishes run/token/hash/process-bound atomic acknowledgements for readiness, applied options, usable-map acceptance, and start-game progression. The control path remains disabled by default, exposes no generic production command surface, and performs no per-tick file I/O or logging when automation is disabled. Run `m2b2d-live-feasibility-20260901-01` runtime-confirmed the loaded hash, readiness, all seven acknowledgements, native `start_game`, and UDP ownership.

### 19. JSON dates and optional native logs must not destroy ownership or outcome evidence
...
Exact dedicated and client `rgl_log` plus `rgl_log_errors` evidence remains required for every started owned role. A role-PID watchdog log is profile-dependent: capture and validate it when present, otherwise record explicit optional `NotProduced`. Dedicated and client inventories must remain distinct; a terminal client status must survive a thrown wait failure into the aggregate report, and a timeout after valid progress must retain the last run/token-validated non-terminal status with `IsTerminal=false`. Evidence retention must not supersede the timeout outcome. Missing optional evidence must not supersede the primary runtime outcome, while a present stale or identity-incompatible file remains a runner evidence failure.

Owned-process correlation must never be treated as proof of fatality by itself. Exact client/dedicated `CrashUploader.exe` and system `WerFault.exe` paths are fatal reporters; `Watchdog.exe` is an owned support process unless independent fatal evidence exists. Support-role cleanup may use a one-second graceful budget, while primary campaign/dedicated/client roles retain 15 seconds. Both paths must preserve exact PID/path/start-time validation and forced-termination evidence. Failure evidence and cleanup must emit separate phase markers so a future stall can be localized without enabling production hot-path diagnostics.
...
- The default-off normal-lobby automation path has real client launch, exact loaded hashes, exact `NetworkMain` resolution, successful one-shot native login, `AtLobby`, exact server selection, native join acceptance, post-rewrite GameNetwork handoff, terminal `Connected`, server `CreatePlayer`, separate PID-native logs, and exact graceful cleanup evidence. Clean run `m2e1-handoff-live-r1-01` loaded corrected client `7CC2D759...2977E97` and closed the formal connection gate.
...
- Milestone 2B.3A implements the shared runner controls: schema-2 role heartbeat/progress/state-revision/error projection, distinct `NoHeartbeat`/`NoProgress`, canonical shared locks, real console and explicit cancellation contracts, RecoveryV2, and exact owned crash/modal/hang evidence paths. Clean run `m2b3a-live-r1-01` confirmed role health, identity, connection, cleanup, result isolation, and passing-path crash/hang absence while exposing a concatenated lock id. The runner-only correction builds ids independently and validates expected/acquired cardinality. Clean aggregate `m2b3a-lockfix-pub-c-01` passed 22/22, and `m2b3a-live-r2-01` recorded and released the exact six resources independently. RUN-008 is closed. The 2026-09-09 correction further makes watchdog ownership non-fatal, centralizes exact fatal reporters, and distinguishes support versus primary cleanup grace; `m4b-readiness-runner-contracts-20260909-01` passed 24/24. Continue treating any later combined/missing lock artifact, uncorrelated helper process, or missing failure-phase marker as a blocker rather than falling back to old behavior.
...
Milestone 3B keeps campaign capture outside production authority. Only the aggregate `Record` command may supply the fixture environment. It requires clean local/upstream identity, an explicit installed client hash, a fresh exact run root, `Suppress`, all shared runtime locks, and no pre-existing product process. The child command line contains only the fixed singleplayer module list; the nonce remains environment-only, the server-password variable is removed, and neither appears in launch/reproduction artifacts. A non-field `Skipped` status may be followed by another selection in the same owned campaign process, but no status may be adopted from another run or fixture. Exact descendant discovery and cleanup must never degrade into name-based process killing.

The remaining high-risk boundary is user state, not the recorder hot path: Bannerlord can write normal configuration/log/save data after the user loads or starts a campaign. The launcher itself edits no save and the live procedure must use a purpose-made copy. Run `m3b-live-capture-02` now proves that the exact private pre-mission file boundary can be captured and cleaned up under the stated controls. Its raw fixture and metadata can contain campaign identifiers and remain private outside Git until independent content/privacy review. A valid payload hash and qualification count still prove only that boundary; they do not prove mission loading, multiplayer connection, natural battle completion, writeback, or L2/L3.
