...
Current field verification boundary (2026-09-10): `m4zclive-l1-01` returned `Timeout/NoHeartbeat` before control readiness/bootstrap although the later retained role-status snapshot has a recent heartbeat. The exact failed read and decision time were not retained; do not assert a dead server, a proven false alarm, a mission failure or a dependency-load root cause from that artifact. `Read-CoopJsonShared` currently collapses absence/read/parse failure to null, and the role-health classifier also uses `NoHeartbeat` for stale data. Preserve this distinction in the next approved investigation; do not disable the guard or extend timeouts to mask it. Automatic bounded timeout finalization and graceful exact cleanup passed for this run; native-crash handling and mission progression remain unverified. Full temporary-install restoration and all five original dumps were verified. [live evidence and next boundary](BATTLE_TEST_AUTOMATION_M4_FIELD_SPAWN_SMOKE.md#26-clean-zero-client-live-attempt-stops-before-bootstrap-2026-09-10).
