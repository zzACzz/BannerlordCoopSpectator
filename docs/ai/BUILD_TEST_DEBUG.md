...
Last automation-control verification: **2026-09-01** (`m2e1-handoff-live-r1-01`; exact corrected client/dedicated loaded identities, formal `Connected`, `Pass`, graceful exact cleanup, free ports, and protected-state preservation)
...
The next automation step is not another identical connection run. Before Milestone 3, implement and contract-test full dedicated/client heartbeat and progress projections with distinct `NoHeartbeat`/`NoProgress`, exclusive shared-resource runtime locks, aggregate `Ctrl+C`/explicit cancellation, the complete `Recover` preview/revalidation/action/reporting matrix, and owned crash/modal/hang evidence including structured `FatalAutomationFailure` plus `crash.json`/`hang.json`. These paths are battle-type-independent and must remain default-off or run-profile gated.
