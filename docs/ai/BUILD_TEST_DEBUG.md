...
Do not extend the timeout or modify battle adapters for that result. Installed-runtime IL inspection proves `LobbyGameStateCustomGameClient.StartMultiplayer()` is parameterless and calls `GameNetwork.StartMultiplayerOnClient(string,int,int,int)`. The old reflection patch expected an absent four-argument lobby method and was the only handoff notifier. Published correction `d1af692` makes the actual GameNetwork patch notify after the final address rewrite; `3fdcda3` carries the last validated non-terminal status into a timeout report without changing the outcome. Focused tests passed, canonical run `m2e1-handoff-fix-contracts-20260901-01` passed 22/22, and pre-publication compile `m2e1-handoff-fix-compile-20260901-01` built both modules with installed inventories unchanged. Its client SHA-256 `049850693D98C300BAE2F5D297C1FD4F4A350E91854078D4AB103CCBE0B4ECF2` is not the staged identity. Fresh clean-published run `m2e1-handoff-pub-compile-20260901-01` from `a7bd528` passed all four compile-only assertions and produced selected client DLL `7CC2D759806D2F02D8BEBA15BCBC01EF61DDE4975004728F3D7E6C8332977E97` plus PDB `92D3135A10D05B663169F24D4BC18BB948497BDFCF09C50350344EB23BF7ABAE`; the dedicated output remained `BD328...02A78`. Targeted ILSpy comparison found the two changed patch types identical between the pre-publication and clean-published DLLs. Transaction `m2e1-handoff-stage-r1` installed only the clean DLL/PDB pair, retained the complete previous 32-file tree, preserved the dedicated tree and protected result, and launched no product process. The next authoritative step is one clean hash-pinned feasibility rerun. Require formal `NetworkHandoff` and terminal `Connected`; do not claim a campaign, mission, L2, or L3 result.
