...
The staging transaction itself established `ConfirmedPathHashOnly`. Live run `m2b2-live-feasibility-20260831-01` later established `ConfirmedLoadedHash` for the dedicated module only: PID `141096` reported state `ModuleReady`, exact hash `1A9723A3249582FABCF08D3778C10CF448944B928549E6D9582FA7F3C0770626`, and `ResultPolicy=Suppress`. The malformed bootstrap did not make the server visible, so the client was not launched and its loaded hash, correlated handoff, and connection remain unverified. Do not use this as L2/L3 battle evidence. See [BATTLE_TEST_AUTOMATION_M2B2_FEASIBILITY.md](BATTLE_TEST_AUTOMATION_M2B2_FEASIBILITY.md).
...
The current `0.3.2` binaries include the game-side runtime foundation and remain bound to clean pushed revision `12abf36` by the Milestone 2B.2A staging record. The runner correction changes only repository automation scripts and contracts, so the installed DLLs do not require restaging. A fresh live `Feasibility` rerun must use the committed correction revision before it can establish client loaded identity or connectivity.
