...
Current M4 safety boundary (2026-09-10): this revision records the event-tail correction that removes provider metadata before failure-report JSON serialization. `m4fe-fix-c1/checks/execution-02` passed 8 focused cases in each of Windows PowerShell 5.1 and PowerShell 7; only the contract project was built. Installed DLL hashes are unchanged and no test worker remains. The full suite, module builds and native failure cleanup remain unverified for this correction. `DeployPersistent` remains unimplemented and is not a prerequisite for the runner fix. [Exact command, evidence and remaining gate](BATTLE_TEST_AUTOMATION_M4_FIELD_SPAWN_SMOKE.md#21-event-tail-serialization-correction-and-focused-contracts-2026-09-10). Older next-action paragraphs below describe historical boundaries; L2 remains open.
