...
Current M4 source-validation boundary (2026-09-10): the event-tail correction passed its focused tests, while the first clean aggregate `m4fe-p1-c1` returned 23/24 when `CoopAutomationRunner.ContractTests.Main` rejected the forwarded `--nologo` argument. This revision accepts that exact legacy invocation; `m4arg-c1` passed the normal runner-project invocation and the 16-case focused selection in both supported shells. [Commands, retained failure and correction evidence](BATTLE_TEST_AUTOMATION_M4_FIELD_SPAWN_SMOKE.md#22-aggregate-test-invocation-compatibility-correction-2026-09-10). The authorized subsequent clean gates use `m4fe-p2-c1` and `m4fe-p1-b1`; consult their own reports for outcomes. Native failure cleanup and L2 remain unverified; `DeployPersistent` remains a separate unimplemented requirement.
