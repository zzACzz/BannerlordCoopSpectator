...
Current M4 boundary (2026-09-10): the [native mode initializer](BATTLE_TEST_AUTOMATION_M4_FIELD_SPAWN_SMOKE.md#24-field-automation-native-mode-initialization-2026-09-10) and [zero-client clock/initial-peer admission](BATTLE_TEST_AUTOMATION_M4_FIELD_SPAWN_SMOKE.md#25-zero-client-clock-and-initial-peer-admission-2026-09-10) passed focused source/contracts and both CompileOnly builds. Latest focused evidence is 432 assertions, metadata-only verification of the exact installed clock IL, and client/dedicated builds with 77/49 warnings, zero errors and unchanged installed inventories. The approved next gate is publication, fresh clean 24-project Contracts and both CompileOnly builds, private transaction self-tests, then one bounded public DedicatedSpawnSmoke invocation with two fresh attempts conditional on prior success and cleanup. Its explicit DeployWithRestore transaction replaces only the two dedicated-module CoopSpectator DLLs and preserves existing crash dumps before launch; use the new gate's exact revision/hashes, never historical helper pins. Native progression, failure cleanup and L2 remain unverified; `DeployPersistent` remains separately unimplemented.
