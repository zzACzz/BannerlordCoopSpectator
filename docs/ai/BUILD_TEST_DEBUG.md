...
The current `0.3.2` binaries include the game-side runtime foundation and remain bound to clean pushed revision `12abf36` by the Milestone 2B.2A staging record. Milestone 2B.2C changes no game-side source, so restaging is not required. Final compile-only run `m2b2c-compile-only-20260831-02` passed with installed inventories unchanged. Before another live run, implement and contract-test immediate provisional ownership, bounded path acquisition with exact requested-path validation, internal-failure classification, and cleanup when identity enrichment fails. Do not proceed to campaign automation first.
