...
The current `0.3.2` binaries include the game-side runtime foundation and remain bound to clean pushed revision `12abf36` by the Milestone 2B.2A staging record. Milestone 2B.2C changes no game-side source, so restaging is not required. Final compile-only run `m2b2c-compile-only-20260831-02` passed with installed inventories unchanged. After review and a clean committed/pushed correction revision, the next separately approved action is one fresh connection-only live rerun; do not proceed to campaign automation first.
