...
Final compile-only run `m2e-compile-20260901-02` built both product projects after the last source-text update without changing installed client/dedicated inventories or launching Bannerlord. The current client candidate SHA-256 is `6DE0B8AE1F38626FD11B7BDE59E86AD7AA106CD7E96596C7FECB2E137C4E8551`; verify the exact hash from the run manifest before staging. The builds retained the existing warning baseline (77 client, 49 dedicated), reported zero errors, and emitted no diagnostic against the changed automation source files. This source evidence does not authorize installation or a live run.
