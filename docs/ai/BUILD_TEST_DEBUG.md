...
Milestone 2B.2D implements that channel in source. `CoopAutomationDedicatedControlBridge` binds the public `InitialListedGameServerState.OnActivated` event through reflection, performs bounded request polling only for a validated enabled profile, invokes `GameNetwork.HandleConsoleCommand` from `SubModule.OnApplicationTick`, and verifies the native option/map/listed-server state. `m2b2d-focused-dev-20260831-03` passed the focused runtime and runner contracts in Windows PowerShell 5.1 and PowerShell 7.6.4; final hardened run `m2b2d-control-contracts-final-20260831-01` passed 22/22; `m2b2d-control-compile-final-20260831-01` built both modules and proved all installed inventories unchanged. The dedicated output SHA-256 is `BD328AAC4F2A64C28D3EDCE28BCE3D72FF164BDAF817D9460B302ED538702A78`. Do not run `Feasibility` against this contract until a separately approved staging operation installs the exact published dedicated build.
