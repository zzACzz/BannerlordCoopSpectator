...
`Feasibility` requires a fresh `RunId`, exact installed client/dedicated hashes, Steam, no pre-existing Bannerlord/dedicated/crash-reporter process, and an unowned requested UDP port. It enables fail-closed `Suppress`, launches the exact dedicated role, requires its loaded hash, and intends to authorize the minimum vanilla `TeamDeathmatch`/`start_game` bootstrap required for native server visibility before verifying the exact UDP owner and launching the client through `-UseExistingRunContract`. The current redirected standard-handle implementation is blocked before command dispatch on the exact starter profile and must not be retried unchanged. Even after its control channel is corrected, this command is connectivity evidence only, never L2/L3 battle evidence.
...
The Milestone 2B.2C source redirects and continuously drains dedicated stdout/stderr. After loaded-hash confirmation it attempts to require the exact native `InitialListedGameServerState.OnActivate` ready message, then exact option/start-game/scene readbacks before beginning the UDP deadline. Synthetic capture contracts passed under Windows PowerShell 5.1 and PowerShell 7, and final full run `m2b2c-full-contracts-20260831-03` passed 22/22. Clean live evidence later proved that those synthetic pipes do not model the exact starter's native-console behavior. Fixed startup sleeps remain forbidden, but redirected standard handles are now supplementary rather than authoritative.
