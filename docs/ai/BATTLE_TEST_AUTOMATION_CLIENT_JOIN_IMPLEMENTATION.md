...
The aggregate and client-launcher post-start ownership prerequisites are implemented, published, and live-verified for the dedicated process path only. The dedicated readiness/request/acknowledgement boundary is now implemented and contract-tested but not staged or runtime-verified. The remaining gates are:

1. review and publish the Milestone 2B.2D source change under separate approval;
2. stage the exact dedicated output under a separate approved and recoverable transaction while preserving the prior installation tree;
3. invoke `Feasibility` from the clean published revision with explicit expected hashes and require lifecycle readiness, all seven dedicated acknowledgements, the run-owned UDP endpoint, and then both loaded roles;
