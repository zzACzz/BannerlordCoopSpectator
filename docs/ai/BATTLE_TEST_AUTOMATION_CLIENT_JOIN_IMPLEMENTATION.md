...
Status: **Corrected client staged and live-confirmed through exact NetworkMain resolution; native platform login is the next shared connection gate**
...
| RUN-001/003/004/005 narrow client protocol | `Satisfied at the non-runtime contract layer; partially runtime exercised` | Fixed `RunId` root; protocol 1.0 source/target role identity; sequence/command/token/hash identity; strict atomic status; launcher refuses reused request/status; general manifest/lease/recovery/file-fault contracts pass; `m2d-live-r3-01` confirmed live role registration, acknowledgement, timeout, and exact cleanup | Source, contract test, runtime evidence | Multiplayer client; battle-type independent | Specification, code map, build guide, M2A report updated | Native login, server-list request, lobby handoff, and connection remain unverified; aggregate failure reporting did not retain the terminal client status object in `feasibility.json` |
...
| PROC-001 client launch slice | `Partially runtime satisfied` | Immediate provisional PID/path/parent/window identity; bounded exact observation; atomic schema-v4 complete-identity handoff; exact cleanup on post-start failure; cross-PowerShell JSON-date contracts; `m2d-live-r3-01` confirmed real launch, loaded hash, aggregate identity acceptance, and graceful automatic cleanup | Source, contract test, runtime evidence | Multiplayer client | Build guide, code map, correction report, and this report updated | Native login, lobby selection, network handoff, and connection remain open; client PID-native logs should be captured automatically rather than recovered manually after cleanup |
...
| CLI-008 run-scoped native-lobby intent | `Partially Satisfied` | (a) default-off complete profile; (b) fresh RunId/token/hash-bound request; (c) exact server filters; (d) run-scoped owned process plus UDP association gate; (e) native list/join APIs; (f) secret excluded from CLI/artifacts; (g) atomic state acknowledgements; (h) launcher/client do not issue `start_game` or use UI automation; (i) exact dedicated lifecycle and seven acknowledgements; (j) native `start_game` plus run-owned UDP endpoint; (k) real client launch and loaded hash; (l) Revision 12 schema-v4 exact identity, UTC-safe import, retained cleanup authority, and required/optional native-log inventory; (m) exact `NetworkMain` defining-assembly resolution live-confirmed by `m2d-live-r3-01`; (n) Revision 13 requires one bounded native `LobbyState.TryLogin()` attempt with explicit outcome evidence before server discovery | Source, contract test, on-disk staging, dedicated/client partial runtime evidence | Multiplayer client and associated local dedicated server; battle-type independent | Specification Revision 13 and affected living documents updated | Native platform login, lobby selection, network handoff, and connection require implementation and a new clean live rerun; the minimum vanilla bootstrap is not L2/L3 battle evidence |
| TST-004 safe default | `Satisfied` for this slice | No complete automation environment means the controller returns without work; feature flag is independent of verbose diagnostics; fresh client and dedicated compile-only builds passed with installed inventories unchanged; the corrected exact client was later staged transactionally and live-confirmed | Source, contract compilation, compile-only build, runtime evidence | Production multiplayer client | Architecture and risks updated | The planned native-login action must remain complete-profile-only, one-shot, and disabled by default |

## 5. Remaining gates

The dedicated readiness/request/acknowledgement boundary, native bootstrap, UDP ownership, real client process creation, schema-v4 aggregate handoff, exact client cleanup, corrected client loaded hash, exact `NetworkMain` resolution, and stock pre-login lobby state are runtime-confirmed. The remaining gates are:

1. implement the Revision 13 one-shot native `LobbyState.TryLogin()` boundary on the main application tick, gated by the complete default-off profile, exact active `LobbyState`, reference equality with the resolved `NetworkMain.GameClient`, `Idle`, and `IsLoggingIn=false`;
2. publish explicit login waiting/success/fault/cancel/privilege-denied/still-idle evidence without UI automation, credentials, bypasses, or blind retries;
3. preserve the terminal `client-join.status.json` object in aggregate failure reporting and automatically retain exact client-PID native-log inventory;
4. pass focused contracts and clean compile-only validation, publish the correction, and transactionally stage only the changed client artifacts with a retained pre-image;
5. invoke `Feasibility` with explicit installed client/dedicated hashes and require native login, exact lobby state, server-list selection, join request/result, network handoff, connected session, unchanged global result state, PID-log inventory, and exact automatic cleanup evidence;
6. retain the run root for inspection and make no campaign, L2, or L3 claim.
