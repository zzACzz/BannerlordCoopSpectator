...
Milestone 2B.2C completed the source and contract prerequisites for another connectivity-only rerun. The remaining gates are:

1. review, commit, and push the verified runner-only correction; no game-side binary restaging is required because Milestone 2B.2C changes no module source;
