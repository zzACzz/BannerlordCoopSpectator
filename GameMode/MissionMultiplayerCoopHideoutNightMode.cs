...
{
    public sealed class MissionMultiplayerCoopHideoutNightMode
        : MissionBasedMultiplayerGameMode
    {
        private const string BattleMissionShell = "MultiplayerBattle";
        public const string GameModeId = CoopHideoutAmbushContract.GameModeId;

        public MissionMultiplayerCoopHideoutNightMode(string name) : base(name)
        {
        }
        ...
        public override void StartMultiplayerGame(string scene)
        {
            ...
            {
                ModLogger.Info(
                    "CoopHideoutNight: rejected mission start because the requested scene is not a supported hideout scene. " +
            ...
            {
                ModLogger.Info(
                    "CoopHideoutNight: rejected mission start because the active battle snapshot is not a nighttime ambush scenario. " +
            ...
            ModLogger.Info(
                "CoopHideoutNight: opening isolated nighttime hideout mission. " +
                "Scene=" + normalizedScene +
                " ScenarioSource=" + scenarioSource +
            ...
            out string source)
        {
            if (GameNetwork.IsServer)
            {
                ...
                BattleSnapshotRuntimeState.SetCurrent(
                    snapshot,
                    "CoopHideoutNight server pre-open battle roster");
            ...
            if (GameNetwork.IsClientOrReplay)
            {
                        ...
                        contract.ScenarioContext?.ScenarioKind))
                {
                    source =
                        ...
                        ",ScenarioKind=" +
                        (contract.ScenarioContext?.ScenarioKind ?? "missing");
                    return null;
                }
            ...
            Mission mission)
        {
                ...
                !list.Any(behavior => behavior is MissionMultiplayerGameModeBaseClient))
            {
                ...
                ModLogger.Info(
                    "CoopHideoutNight: inserted dedicated MissionMultiplayerGameModeBaseClient bridge before MissionCustomGameServerComponent for MissionScoreboardComponent lifecycle compatibility.");
            }
                ...
                !list.Any(behavior => behavior is AgentHumanAILogic))
            {
                ...
                ModLogger.Info(
                    "CoopHideoutNight: inserted the current AgentHumanAILogic into the isolated server behavior stack.");
            }
                ...
                !list.Any(behavior => behavior is AgentHumanAILogic))
            {
                throw new InvalidOperationException(
                    "CoopHideoutNight requires AgentHumanAILogic before agent materialization.");
            }
                ...
                !list.Any(behavior => behavior is CoopHideoutStealthPatrolController))
            {
                ...
                ModLogger.Info(
                    "CoopHideoutNight: inserted the isolated hideout stealth and patrol controller into the server behavior stack.");
            }

            if (!list.Any(behavior => behavior is CoopHideoutAmbushNetworkController))
                list.Add(new CoopHideoutAmbushNetworkController());
            ...
            if (!GameNetwork.IsServer &&
                !list.Any(behavior => behavior is CoopSpectator.UI.CoopHideoutAmbushStealthView))
            {
                list.Add(new CoopSpectator.UI.CoopHideoutAmbushStealthView());
            }

            if (!GameNetwork.IsServer &&
                !list.Any(behavior => behavior is CoopSpectator.UI.CoopHideoutCallTroopsNameMarkerView))
            {
                list.Add(new CoopSpectator.UI.CoopHideoutCallTroopsNameMarkerView());
            }

            if (!GameNetwork.IsServer &&
                !list.Any(behavior => behavior is CoopSpectator.UI.CoopHideoutAmbushCallTroopsCinematicView))
            {
                list.Add(new CoopSpectator.UI.CoopHideoutAmbushCallTroopsCinematicView());
            }
            ...
            ModLogger.Info(
                "CoopHideoutNight: isolated behavior stack prepared. " +
