...
namespace CoopSpectator.GameMode // Простір імен для кастомного MP game mode CoopBattle
{
    ...
    public sealed class MissionMultiplayerCoopBattleMode : MissionBasedMultiplayerGameMode
    {
        ...
        public override void StartMultiplayerGame(string scene)
        {
            ...
            {
                ListedShellMissionSessionState.ReleaseSceneBoundActiveTokenForFreshBattleStartup(
                    scene,
        ...
        private static List<MissionBehavior> BuildServerMissionBehaviorsForCoopBattle(Mission mission, bool isDedicated)
        {
            ...
            {
                ModLogger.Info(
                    "CoopBattle server: injected CoopMissionNetworkBridge into the initial dedicated mission stack so first-entry peers can bootstrap battle snapshot transport before synchronized. " +
                    "CoopMissionSpawnLogic remains on deferred observer attach until native mission-ready state stabilizes.");
            }
            else
            {
        ...
        private static void ValidateClientStackSanity(List<MissionBehavior> list)
        {
            ...
            int removedLegacyEquipmentNetwork = 0;
            int removedLegacyVisualSpawn = 0;
            for (int i = list.Count - 1; i >= 0; i--)
            {
                MissionBehavior behavior = list[i];
                if (behavior == null)
                ...
                if (!string.Equals(typeName, "MultiplayerMissionAgentVisualSpawnComponent", StringComparison.Ordinal))
                    continue;

                list.RemoveAt(i);
                removedLegacyVisualSpawn++;
                ModLogger.Info("CoopBattle client validation: removed legacy MultiplayerMissionAgentVisualSpawnComponent from custom runtime stack.");
            }

            ModLogger.Info(
                "CoopBattle client validation: completed. " +
                "RemovedLegacyEquipmentNetwork=" + removedLegacyEquipmentNetwork +
                " RemovedLegacyVisualSpawn=" + removedLegacyVisualSpawn);
        }
