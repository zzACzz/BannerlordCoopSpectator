...
{
    /// <summary>
    /// Owns only the ordinary FieldBattle client startup pacing state.
    /// SallyOut keeps its existing state and code path in BattleMapSpawnHandoffPatch.
    /// </summary>
    internal static class ExactFieldBattleInitialMaterializationRuntime
    {
        ...
        {
            diagnostics = "field-battle-initial-materialization-disabled";
            if (!ExperimentalFeatures.EnableExactFieldBattleInitialMaterializationRuntime)
                return false;
        ...
        public static bool ShouldPaceInitialClientCreateAgent(
            ...
            out string reason)
        {
            reason = "not-field-battle-client-startup";
            ...
            {
                reason = "field-battle-already-active";
                return false;
            }

            if (!IsValidatedScenario(mission, out string scenarioDiagnostics))
            {
                reason = "field-battle-scenario-not-validated:" +
            ...
            reason =
                "validated-field-battle-initial-materialization MaterializedMap={" +
            ...
            string source)
        {
            ...
            {
                ModLogger.Info(
                    "ExactFieldBattleInitialMaterializationRuntime: adjusted adaptive client replay pacing. " +
