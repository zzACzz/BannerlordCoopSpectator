...
- Exact campaign-scene armies now spawn visually in correct campaign-like zones.
- Hero spawn / battle / victory already work on exact campaign scenes.
...
- Exact campaign commander/general-control handoff is now the active frontier: server-side general state mostly works, client-side commander control is partial, and campaign-style command UI is still not migrated.
...
- After exact-scene armies, battle flow, and aftermath became stable, the current remaining gap shifted again: commander/general control on the client is only partially working. Small battles can already issue some move orders, but the client still uses MP order-UI semantics, while campaign-style commander UX is a separate follow-up layer.
