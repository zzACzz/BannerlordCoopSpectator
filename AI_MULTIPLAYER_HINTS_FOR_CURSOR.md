...
- **Етап 1–2:** Підготовка, Spectator mode (broadcaster, UI, блокування) — виконано.
...
- **Етап 3.2:** Клієнт приєднується через **Custom Server List**; запуск клієнта з модом — через **батник** `run_mp_with_mod.bat`. Join і екран Awaiting Server працюють. **Підтверджено:** коли хост у синглплеєрі входить у битву — на дедик-сервері теж починається битва (GET /Manager/start_mission). Далі — тест **повного циклу з клієнтом**: клієнт на Awaiting Server → перехід у місію при start_mission і повернення при end_mission.
