    ...
    { // Починаємо блок класу
        ...
        private sealed class BattleResultCharacterAggregate
        {
            ...
            public int DesiredCount => IsHero
                ...
                : Math.Max(0, SnapshotCount - Math.Max(0, KilledCount));
        ...
        private static bool TryGetUnsupportedCoopMissionReason(out string reason, out string summary)
        {
            ...
            try
            {
                ...
                if (LordsHallCampaignBattleAdapter.IsCampaignStage(battle, encounterSettlement) &&
                    !LordsHallCampaignBattleAdapter.TryValidateActiveMission(
                        battle,
                        encounterSettlement,
                        mission,
                        out _,
                ...
                {
                    reason = "lords-hall-contract-invalid";
                    summary += " LordsHallDiagnostics={" + lordsHallDiagnostics + "}";
                    return true;
                }
        ...
        private static bool TryValidateSupportedDayHideoutAssaultMission(
            Mission mission,
            out string diagnostics)
        {
            diagnostics = "Mission=missing";
            if (mission?.MissionBehaviors == null)
                return false;

            const string dayControllerType =
                "SandBox.Missions.MissionLogics.Hideout.HideoutMissionController";
            const string ambushControllerType =
                "SandBox.Missions.MissionLogics.Hideout.HideoutAmbushMissionController";

            MissionBehavior dayController = mission.MissionBehaviors.FirstOrDefault(behavior =>
                string.Equals(
                    behavior?.GetType().FullName,
                    dayControllerType,
                    StringComparison.Ordinal));
            bool hasDayController = dayController != null;
            bool hasAmbushController = mission.MissionBehaviors.Any(behavior =>
                string.Equals(
                    behavior?.GetType().FullName,
                    ambushControllerType,
                    StringComparison.Ordinal));
            bool hasBossFightEntity = false;
            try
            {
                hasBossFightEntity = mission.Scene?.FindEntityWithTag(
                    CoopHideoutBossPhaseContract.BossFightEntityTag) != null;
            }
            catch
            {
            }
            bool hasSelectedRosterContract = dayController?.GetType().GetMethod(
                "GetAllTroopsForSide",
            ...
            try
            {
                ...
                if (!Enum.TryParse(playerSideText, ignoreCase: true, out BattleSideEnum playerSide))
                {
                    string snapshotPlayerSide = snapshot.Sides
                        .FirstOrDefault(side => side?.IsPlayerSide == true)?.SideText;
                    if (!Enum.TryParse(snapshotPlayerSide, ignoreCase: true, out playerSide))
                    {
                        diagnostics = "player-side-unresolved:" + (playerSideText ?? "null");
                        return false;
                    }
                }
                ...
                if (playerSideSnapshot?.Parties == null)
                {
                    diagnostics = "player-side-snapshot-missing";
                    return false;
                }
                ...
                foreach (BattlePartySnapshotMessage party in playerSideSnapshot.Parties)
                {
                    ...
                    if (party != null)
                    {
                        party.Troops = retainedTroops;
                        party.TotalManCount = retainedTroops.Sum(troop => troop.Count);
                    }
                }

                playerSideSnapshot.Parties = playerSideSnapshot.Parties
                    .Where(party => party?.Troops?.Count > 0)
                    .ToList();
                playerSideSnapshot.Troops = playerSideSnapshot.Parties
                    .SelectMany(party => party.Troops)
                    .ToList();
                playerSideSnapshot.TotalManCount = playerSideSnapshot.Troops.Sum(troop => troop.Count);
                if (!playerSideSnapshot.Parties.Any(party =>
                        string.Equals(
                            party.PartyId,
                ...
                diagnostics =
                    ...
                    " projected=" + playerSideSnapshot.TotalManCount +
                    " parties=" + playerSideSnapshot.Parties.Count;
                ModLogger.Info(
                    "BattleDetector: projected vanilla hideout selected roster into isolated coop snapshot. " +
                    "Diagnostics={" + diagnostics + "}.");
                return playerSideSnapshot.TotalManCount == selectedTotal;
            }
            catch (Exception ex)
            {
        ...
        private void TryConsumeBattleResultWritebackAudit()
        {
            try
            {
                ...
                else if (LordsHallResultBridge.IsLordsHallResult(result))
                {
            ...
            CoopBattleResultBridgeFile.BattleResultSnapshot result)
        {
            ...
            foreach (CoopBattleResultBridgeFile.BattleResultEntrySnapshot entry in result.Entries.Where(entry => entry != null))
            {
                ...
                aggregate.ActiveCount += Math.Max(0, entry.ActiveCount);
            ...
            BattleResultWritebackSummary summary)
        {
            ...
            foreach (CoopBattleResultBridgeFile.BattleResultEntrySnapshot enemyEntry in result.Entries.Where(item =>
                         ...
                         !string.Equals(item.PartyId, partyId, StringComparison.OrdinalIgnoreCase)))
            {
                int casualtyCount = Math.Max(0, enemyEntry.KilledCount + enemyEntry.UnconsciousCount);
                if (casualtyCount <= 0)
                    continue;
            ...
            BattleResultWritebackSummary summary)
        {
            ...
            if (LordsHallResultBridge.IsLordsHallResult(result) &&
            ...
            string resultKey)
        {
            if (result?.Entries == null ||
                ...
                !LordsHallResultBridge.ShouldIncludeInFinalSiegeDefenderPreview(result))
            {
                return;
            }
            ...
            string resultKey)
        {
            ...
            try
            {
                ...
                BattleSideEnum defeatedSide = winnerSide.GetOppositeSide();
                var defeatedPartyIds = new HashSet<string>(
                    result.Entries
                        .Where(entry =>
                            entry != null &&
                ...
                foreach (BattleResultCharacterAggregate aggregate in aggregates.Values
                             .Where(item =>
                                 item != null &&
                                 defeatedPartyIds.Contains(item.PartyId ?? string.Empty))
                             .OrderBy(item => item.PartyId, StringComparer.OrdinalIgnoreCase)
                             .ThenBy(item => item.HeroId, StringComparer.OrdinalIgnoreCase)
                             .ThenBy(item => item.OriginalCharacterId, StringComparer.OrdinalIgnoreCase)
                             .ThenBy(item => item.CharacterId, StringComparer.OrdinalIgnoreCase))
                ...
                ModLogger.Info(
                    ...
                    " DefeatedSide=" + defeatedSide +
                    " DefeatedParties=" + defeatedPartyIds.Count +
                    " TroopsAdjusted=" + summary.RegularTroopsAdjusted +
                    " HeroDeaths=" + summary.HeroDeathsApplied +
                    " HeroWounds=" + summary.HeroWoundsApplied +
                    " Unresolved=" + summary.UnresolvedPartyAggregates +
            ...
            try
            {
                ...
                foreach (KeyValuePair<string, int> pair in _cachedHostAftermathMainPartyPreviewHeroHitPointsByHeroId)
                {
                    ...
                    int desiredHitPoints = Math.Min(
                        hero.MaxHitPoints,
                        Math.Max(1, pair.Value + Math.Max(0, battleEndHealing)));
        ...
        private static bool ShouldUseNativeFinalLordsHallAftermath(
            CoopBattleResultBridgeFile.BattleResultSnapshot result)
        {
            ...
            CoopBattleResultBridgeFile.BattleResultSnapshot result)
        {
            ...
            Settlement settlement =
                ...
                battle?.MapEventSettlement ??
                MobileParty.MainParty?.CurrentSettlement;
            _pendingLordsHallSettlementId =
                settlement?.StringId ??
                BattleSnapshotRuntimeState.GetCurrent()?.ScenarioContext?.SiegeContext?.SettlementId ??
                string.Empty;
            ...
            string resultKey)
        {
            ...
            {
                PartyBase mainParty = MobileParty.MainParty?.Party;
                if (mainParty == null || TaleWorlds.CampaignSystem.Campaign.Current?.Models?.PartyHealingModel == null)
                    return;
                ...
                foreach (KeyValuePair<string, int> pair in _cachedHostAftermathMainPartyPreviewHeroHitPointsByHeroId)
                {
                    ...
                    int healing = TaleWorlds.CampaignSystem.Campaign.Current.Models.PartyHealingModel
        ...
        private static void TryResetPendingLordsHallStageStateIfOrphaned()
        {
            ...
            Settlement settlement =
                ...
                battle?.MapEventSettlement ??
                MobileParty.MainParty?.CurrentSettlement;
            if (LordsHallResultBridge.ShouldPreservePendingStage(
                    settlement,
                    battle,
                    _pendingLordsHallSettlementId))
            {
            ...
            ModLogger.Info(
                "BattleDetector: cleared orphaned lords-hall continuation state. " +
                "PendingSettlement=" + (_pendingLordsHallSettlementId ?? "null") +
                " CurrentSettlement=" + (settlement?.StringId ?? "null") +
                " BattleType=" + (battle?.EventType.ToString() ?? "none") + ".");
            ClearPendingLordsHallStageState();
        }
        ...
        { // Починаємо блок методу
            ...
            message.Troops = BuildLegacyTroopsFromSnapshot(message.Snapshot) ?? BuildPartyTroopStacksSafe(message.Snapshot);
        ...
        private static List<TroopStackInfo> BuildPartyTroopStacksSafe(BattleSnapshotMessage snapshot = null) // Будуємо список стеків військ з партії (best-effort)
        { // Починаємо блок методу
        ...
        private static BattleSnapshotMessage BuildFallbackBattleSnapshot(string mapScene, string playerSideText)
        {
            ...
            var mainParty = new BattlePartySnapshotMessage
            {
                ...
                Troops = BuildPartyTroopStacksSafe(snapshot)
            };
        ...
        private static BattleSideSnapshotMessage BuildBattleSideSnapshot(
            object sideObject,
            string sideId,
            string sideText,
            string playerSideText,
            BattleSnapshotMessage snapshot)
        {
            ...
            foreach (object partyObject in EnumerateBattleParties(sideObject))
            {
                ...
                BattlePartySnapshotMessage partySnapshot = BuildBattlePartySnapshot(partyObject, sideId, snapshot);
            ...
            string sideId)
        {
            ...
            try
            {
                ...
                if (LordsHallCampaignBattleAdapter.IsCampaignStage(battle, encounterSettlement))
                {
                    ...
                    {
                        ModLogger.Info(
                            "BattleDetector: rejected lords-hall participant order because side id is unknown. " +
                            "SideId=" + (sideId ?? "null") + ".");
                        return orderedEntryIds;
                    }

                    bool exactOrderBuilt = LordsHallCampaignBattleAdapter.TryBuildParticipantEntryOrder(
                        Mission.Current,
                        side,
                        sideSnapshot,
                    ...
                    ModLogger.Info(
                        "BattleDetector: lords-hall exact native participant order resolved. " +
                        "SideId=" + sideId +
        ...
        private static BattlePartySnapshotMessage BuildBattlePartySnapshot(
            object partyObject,
            string sideId,
            BattleSnapshotMessage snapshot)
        {
            ...
            List<TroopStackInfo> troops = BuildTroopStacksFromPartySafe(partyBase, sideId, snapshot);
            if (troops.Count == 0)
            {
                ModLogger.Info(
            ...
            string partyId = TryResolveSnapshotPartyId(partyBase);
        ...
        private static BattlePartyModifierSnapshotMessage TryBuildPartyModifierSnapshot(object partyObject, object partyBase)
        {
            ...
            var modifiers = new BattlePartyModifierSnapshotMessage
            {
                ...
                LeaderMedicineSkill = TryGetCharacterSkillValue(leaderHero, "Medicine"),
        ...
        private static List<TroopStackInfo> BuildTroopStacksFromPartySafe(
            object partyBase,
            string sideId,
            BattleSnapshotMessage snapshot)
        {
            ...
            BattleSnapshotMessage snapshot = null)
        {
            ...
            List<CombatEquipmentVariantSnapshot> equipmentVariants = BuildCombatEquipmentVariants(
                characterObject,
                totalCount,
                snapshot);
            ...
            for (int variantIndex = 0; variantIndex < activeVariantCount; variantIndex++)
            {
                ...
                if (variant != null)
                    ...
                    ApplyCombatEquipmentSnapshot(stack, characterObject, snapshot);
        ...
        private static List<CombatEquipmentVariantSnapshot> BuildCombatEquipmentVariants(
            object characterObject,
            int totalCount,
            BattleSnapshotMessage snapshot = null)
        {
            List<CombatEquipmentVariantSnapshot> sampledVariants = BuildRandomBattleEquipmentVariants(
                characterObject,
                totalCount,
                snapshot);
            ...
            {
                CombatEquipmentVariantSnapshot variant = BuildCombatEquipmentVariantSnapshot(equipment, snapshot);
        ...
        private static List<CombatEquipmentVariantSnapshot> BuildRandomBattleEquipmentVariants(
            object characterObject,
            int totalCount,
            BattleSnapshotMessage snapshot = null)
        {
        ...
        private static string FormatPartyModifierSummary(BattleSideSnapshotMessage side, BattlePartySnapshotMessage party)
        {
            ...
            if (modifiers != null)
            {
                ...
                if (modifiers.LeaderMedicineSkill > 0)
                    parts.Add("LeaderMedSkill=" + modifiers.LeaderMedicineSkill);
        ...
        private static bool HasPartyModifierData(BattlePartyModifierSnapshotMessage modifiers)
        {
            ...
            return
                ...
                modifiers.LeaderMedicineSkill > 0 ||
        ...
        private static CampaignSceneSelectionContext TryBuildCampaignSceneSelectionContextSafe()
        {
            ...
            try
            {
                ...
                if (LordsHallCampaignBattleAdapter.IsCampaignStage(battle, encounterSettlement))
                {
                    bool contractValid = LordsHallCampaignBattleAdapter.TryValidateActiveMission(
                        battle,
                        encounterSettlement,
                        mission,
                        out string expectedScene,
                        out string lordsHallDiagnostics);
                    context.BattleSceneName = !string.IsNullOrWhiteSpace(expectedScene)
                        ? expectedScene
                        : missionSceneName;
        ...
        private static string TryResolveSnapshotPartyId(object partyBase)
        {
            string partyId =
                TryGetStringId(partyBase) ??
                TryGetStringId(TryGetPropertyValue(partyBase, "MobileParty"));
            if (!string.IsNullOrWhiteSpace(partyId))
                return partyId;
            ...
            Settlement settlement =
                ...
                battle?.MapEventSettlement ??
                MobileParty.MainParty?.CurrentSettlement;
            if (LordsHallCampaignBattleAdapter.IsCampaignStage(battle, settlement) &&
                partyBase is PartyBase nativePartyBase)
            {
                return nativePartyBase.Id;
            }
