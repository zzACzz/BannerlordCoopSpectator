...
using CoopSpectator.Patches; // Runtime patch surface
...
{ // ÐŸÐ¾Ñ‡Ð¸Ð½Ð°Ñ”Ð¼Ð¾ Ð±Ð»Ð¾Ðº Ð¿Ñ€Ð¾ÑÑ‚Ð¾Ñ€Ñƒ Ñ–Ð¼ÐµÐ½
    ...
    { // ÐŸÐ¾Ñ‡Ð¸Ð½Ð°Ñ”Ð¼Ð¾ Ð±Ð»Ð¾Ðº ÐºÐ»Ð°ÑÑƒ
        ...
        // Preview patch is safe to keep; AgentDisplayNamePatch remains disabled because even battle-only Agent.Name*
        // hooks still regress mannequin/preview pipelines. The remaining class/stats and exact bootstrap/runtime
        // clusters stay disabled until re-validated separately.
        ...
        private static void TryRegisterCoopBattleForClient()
        {
            ...
            try
            {
                if (TaleWorlds.MountAndBlade.Module.CurrentModule != null)
                {
                    ...
                    GameModeOverridePatches.SetTeamDeathmatchOverride(new MissionMultiplayerListedShellMode(CoopGameModeIds.OfficialTeamDeathmatch));
                    ...
                    ModLogger.Info("[CoopSpectator] TeamDeathmatch listed ingress override armed on client. Module.StartMultiplayerGame(TeamDeathmatch, scene) will enter explicit listed shell mode.");
                }
                else
                {
            ...
            try
            {
                ...
                if (EnableManualHarmonyApply)
                {
                    if (EnableManualPatchLobbyClient)
                    {
                        ...
                        ListedShellClientWrapperOwnershipPatch.Apply(harmony);
                        ...
                        if (EnableManualPatchMissionFlowBattleRuntime)
                        {
                            ...
                            if (EnableManualPatchMissionFlowBattleRuntimeBootstrapGates)
                            {
                                if (EnableManualPatchMissionFlowBattleRuntimeLateJoinBootstrapGate)
                                {
                                    ...
                                    LateJoinPeerStateReplayOwnershipPatch.Apply(harmony);
                                }
                                else
                                {
                                ...
                                {
                                    PendingBattleLoadMissionTransportTokenPatch.Apply(harmony);
                                    ListedShellBaseNetworkTransportOwnershipPatch.Apply(harmony);
                                }
                                else
                                {
